...
freeze = dict(recipe_sha256=digest(Path(__file__)), manifest_sha256=bundle.manifest_sha256,
              ownership_session=session, daemon_paths=dict(binary=paths.binary,pid=paths.pid,log=paths.log),
...
try:
    with acquire_radio_lock(serial):
        try:
            ...
            write_json(root/'stage-attempt.json', dict(scratch=scratch,session=session,
                daemon_paths=dict(binary=paths.binary,pid=paths.pid,log=paths.log),
                remote_directory=bundle.remote_directory,manifest_sha256=bundle.manifest_sha256))
