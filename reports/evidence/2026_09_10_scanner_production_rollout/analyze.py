...
def stats(values):
    return dict(count=len(values), mean=float(np.mean(values)), p99=float(np.percentile(values, 99)), maximum=float(max(values))) if values else dict(count=0)
events = receipt.events[:receipt.complete_visit_count]
steady = events[1:]
summary = dict(session_id=session_id, manifest_sha256=capture.manifest_sha256,
    full_iq_verified=True, terminal_state=receipt.terminal.state,
    source_seconds=receipt.duty_denominator_sample_count/rate,
    valid_seconds=receipt.valid_sample_count/rate, valid_duty_percent=receipt.valid_duty_ppm/10000,
    complete_visits=receipt.complete_visit_count, receiver_ids=list(receipt.plan.geometry.receiver_ids),
    sample_rate_hz=rate, bandwidth_hz=receipt.plan.geometry.bandwidth_hz,
    glrt_results=len(rows), delivery_complete=glrt.evidence.delivery_complete,
    ...
    unclassified_samples=receipt.unclassified_sample_count, unreceived_tail_samples=receipt.unreceived_tail_sample_count)
responses = {}
...
for name, value in (("summary.json", summary), ("public-glrt.json", glrt.model_dump(mode="json")), ("deployed-api.json", responses)):
...
axes[0].scatter(times[~positive], targets[~positive], s=5, color="#8a9ba8", label="No qualifying positive; absence not established")
axes[0].scatter(times[positive], targets[positive], s=9, color="#087e66", label="Lightweight GLRT candidate")
axes[0].set_yticks(range(8), [f"CH{i%4+1}{'L' if i<4 else 'U'}" for i in range(8)])
axes[0].invert_yaxis()
axes[0].set_title(f"Production 2.5 MS/s · {session_id}\nActual 120 ms visits; RX1 detection, both receivers recorded")
axes[0].legend(fontsize=8, ncol=2, loc="upper right")
bins = np.arange(0, 301, 10)
duty, coverage = [], []
for lo, hi in zip(bins[:-1], bins[1:]):
    duty.append(float(np.maximum(0, np.minimum(times+.12, hi)-np.maximum(times, lo)).sum()/(hi-lo)*100))
...
axes[1].plot((bins[:-1]+bins[1:])/2, coverage, label="Full temporal-screen coverage", color="#ba6b1b")
axes[1].axhline(90, color="gray", linestyle=":", label="90% capture target")
axes[1].set_ylim(0, 105)
axes[1].set_ylabel("Percent per 10 s bin")
axes[1].legend(fontsize=8, ncol=3)
axes[2].scatter([(r.valid_start-origin)/rate for r in evaluated], [r.cpu_ms for r in evaluated], s=4, color="#6a58a5", alpha=.5, label="ARM worker CPU / evaluated dwell")
