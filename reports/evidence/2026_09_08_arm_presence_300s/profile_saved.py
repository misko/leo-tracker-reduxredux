...
freeze=dict(records=records,recipe_sha256=digest(Path(__file__)),protocol_sha256=digest(protocol_path),
...
build_executable(out/'profile-arm',compiler=compiler,cflags=flags+('-mcpu=cortex-a9','-mfpu=neon','-mfloat-abi=hard')+fftw['cflags'],
                 ldflags=fftw['ldflags'],dependencies=fftw['dependencies'])
build_executable(out/'profile-desktop',cflags=flags+desktop['cflags'],ldflags=desktop['ldflags'],dependencies=desktop['dependencies'])
...
paths={'profile-arm':out/'profile-arm','libfftw3.so.3':library,**{r['probe']:source/r['probe'] for r in records}}
...
with acquire_radio_lock(serial):
    ...
    for index,record in enumerate(records):
        command=f'LD_LIBRARY_PATH={remote} nice -n 10 {remote}/profile-arm {remote}/{record["probe"]} 5 --nuisance'
        raw=remote_checked(f'run-{index:02}',command,timeout=20)
        arm=[json.loads(line) for line in raw.splitlines()]
        desk=subprocess.run([str(out/'profile-desktop'),str(source/record['probe']),'1','--nuisance'],check=True,text=True,capture_output=True)
        ...
        rows.append(dict(record=record,desktop=expected,arm=arm))
        print(f'Profiled {index+1}/16 selected saved slices, five ARM repetitions',flush=True)
    check('after')
...
summary={}
for rate in (2500000,5000000):
    subset=[row for r in rows if r['record']['rate_hz']==rate for row in r['arm']]
    ...
    summary[str(rate)]={k:dict(zip(('mean','p50','p99','max'),[float(np.mean(v)),*np.percentile(v,[50,99,100]).tolist()],strict=True)) for k,v in fields.items()}
