    ...
    try:
        ...
        selected = sorted(
            next(
                ...
                and e.valid_start_counter - receipt.terminal.first_counter >= rate * 30
            )
        ...
        with ExitStack() as stack:
            ...
            confirmations = {
                edge: stack.enter_context(NativePresence(library, rate, edge)) for edge in dwells
            }
            ...
            for visit in selected:
                ...
                for repetition in range(4):
                    ...
                    fragment = np.ascontiguousarray(
                        iq[window * (rate // 50) : (window + 1) * (rate // 50)]
                    )
                    confirmation = Result()
                    assert (
                        c.library.leo_presence_run_ci16(
                            c.workspace, pointer(fragment), len(fragment), ct.byref(confirmation)
                        )
                        == 0
                    )
                    assert not differences(
                        numerical(unpack(result.confirmations[0])), numerical(unpack(confirmation))
                    )
                    ...
                    row = dict(
                        ...
                        iq_sha256=digest,
                        target=record.event.target_index,
                        edge=edge,
...
for rate in (2500000, 5000000):
    ...
    for name in ("conversion", "coarse", "fine", "fractional", "total"):
        components[f"confirmation_{name}"] = [
            r["dwell"]["confirmations"][0][f"{name}_cpu_ms"] for r in group
        ]
    for name in (
        "acquisition_fft",
        "conditioned",
    ...
    summary[rate] = {
        name: dict(mean_ms=float(np.mean(v)), p99_ms=float(np.percentile(v, 99)), maximum_ms=max(v))
...
with (output / "profile.json").open("x") as handle:
    json.dump(
        dict(
            method=(
                "first saved visit at/after 30 source seconds per target; RX1; "
                "one warm-up and three measured repetitions; unchanged native flags and host FFTW; "
                "separate confirmation parity checked; desktop timing is not ARM timing"
            ),
            rows=rows,
            summary=summary,
        ),
