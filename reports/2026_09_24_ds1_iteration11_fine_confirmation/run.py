#!/usr/bin/env python3
"""Reference-free symmetric fine confirmation centered only on the sealed iteration-10 winner."""
...
LEVELS_KM = (0.048828125, 0.0244140625, 0.01220703125)
TOP_BASINS = 3
...
def load_iteration10(path: Path) -> dict[str, Any]:
    ...
    if value.get("complete") is not True or value.get("reference_used_for_fit") is not False:
        raise ValueError("iteration-10 must be a sealed reference-free inference")
    if tuple(value.get("common_tau_grid_s", ())) != TAUS:
        raise ValueError("iteration-10 tau grid does not match the fixed continuation grid")
    winner = value.get("winner", {})
    if set(winner.get("best_exact_by_group", {})) != set(GROUPS):
        raise ValueError("iteration-10 shared winner is incomplete")
    if not all(
        row["exact_comparison"]["exact_sgp4_gate"]["passed"]
        for row in winner["best_exact_by_group"].values()
    ):
        raise ValueError("iteration-10 winner did not pass exact gates")
    return value
...
def scan(task: tuple[str, dict[str, float]]) -> dict[str, Any]:
    ...
    iteration10 = load(ITER10_RUN, f"i11_i10_{group}_{os.getpid()}")
    iteration10.TAUS = TAUS
    return iteration10.scan((group, point))
...
def exact(task: tuple[str, dict[str, Any], dict[str, Any]]) -> dict[str, Any]:
    ...
    os.environ.setdefault("OPENBLAS_NUM_THREADS", "1")
    os.environ.setdefault("OMP_NUM_THREADS", "1")
    iteration10 = load(ITER10_RUN, f"i11_exact_{group}_{os.getpid()}")
    return iteration10.exact((group, point, tau_row))
...
def main() -> None:
    ...
    iteration10 = load_iteration10(args.iteration10)
    origin = {
        name: float(iteration10["winner"][name]) for name in ("latitude_deg", "longitude_deg")
    }
    parents = [origin]
    levels = []
    ...
    payload = {
        "schema": "ds1-iteration11-symmetric-fine-confirmation/v1",
        "complete": True,
        "partition": "train",
        ...
        "origin": origin,
        "origin_policy": (
            "sealed iteration-10 winner only; no post-seal reference error or "
        ...
        "bindings": {
            ...
            "iteration10_inference": digest(args.iteration10),
        },
    }
