...
def exact_descendants(east: float, north: float, spacing_km: float = 12.5) -> np.ndarray:
    cells = int(round(100.0 / spacing_km))
    if spacing_km <= 0 or not np.isclose(cells * spacing_km, 100.0):
        raise ValueError("validation spacing must divide the 100 km parent")
    offsets = (np.arange(cells) + 0.5) * spacing_km - 50.0
    return np.asarray(
...
def run(args: argparse.Namespace) -> dict:
    finest_spacing = getattr(args, "finest_spacing_km", 12.5)
    if (
        not 0 < args.radius_km <= 500
        or not 1 <= args.workers <= 12
        or args.budget_points < 100
        or not args.cities
        or len(set(args.cities)) != len(args.cities)
        or any(city not in CITIES for city in args.cities)
    ):
        raise ValueError("invalid bounded search configuration")
    levels = tuple(100.0 / 2**index for index in range(5) if 100.0 / 2**index >= finest_spacing)
    if levels[-1] != finest_spacing:
        raise ValueError("finest spacing must be an aligned halving of 100 km")
    ...
    with context.Pool(
        ...
        for city in args.cities:
            ...
            reference_points = exact_descendants(coarse.east_km, coarse.north_km, finest_spacing)
...
def main() -> None:
    ...
    parser.add_argument("--finest-spacing-km", type=float, choices=(25.0, 12.5, 6.25), default=12.5)
    parser.add_argument(
