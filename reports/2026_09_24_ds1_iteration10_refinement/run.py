#!/usr/bin/env python3
"""Reference-free symmetric refinement centered only on the sealed iteration-9 winner."""
...
ITER9 = ROOT / "reports/2026_09_24_ds1_iteration9_joint_refinement/inference.json"
ITER9_RUN = ROOT / "reports/2026_09_24_ds1_iteration9_joint_refinement/run.py"
GROUPS = ("20260921_00", "20260921_16")
TAUS = (-1.25, -1.0, -0.75, -0.5, -0.25)
LEVELS_KM = (0.1953125, 0.09765625, 0.048828125)
TOP_BASINS = 3
...
def load_iteration9(path: Path) -> dict[str, Any]:
    value = json.loads(path.read_text())
    if value.get("complete") is not True or value.get("reference_used_for_fit") is not False:
        raise ValueError("iteration-9 must be a sealed reference-free inference")
    if tuple(value.get("common_tau_grid_s", ())) != TAUS:
        raise ValueError("iteration-9 tau grid does not match the fixed continuation grid")
    winner = value.get("winner", {})
    if set(winner.get("best_exact_by_group", {})) != set(GROUPS):
        raise ValueError("iteration-9 shared winner is incomplete")
    if not all(
        row["exact_comparison"]["exact_sgp4_gate"]["passed"]
        for row in winner["best_exact_by_group"].values()
    ):
        raise ValueError("iteration-9 winner did not pass exact gates")
    return value
...
def joint_key(row: dict[str, Any]) -> tuple[float, float, float]:
    return (
        ...
        float(row["north_km_from_iteration9"]),
        float(row["east_km_from_iteration9"]),
    )
...
def scan(task: tuple[str, dict[str, float]]) -> dict[str, Any]:
    ...
    iteration9 = load(ITER9_RUN, f"i10_i9_{group}_{os.getpid()}")
    iteration9.TAUS = TAUS
    return iteration9.scan((group, point))
...
def exact(task: tuple[str, dict[str, Any], dict[str, Any]]) -> dict[str, Any]:
    ...
    os.environ.setdefault("OPENBLAS_NUM_THREADS", "1")
    os.environ.setdefault("OMP_NUM_THREADS", "1")
    iteration9 = load(ITER9_RUN, f"i10_exact_{group}_{os.getpid()}")
    return iteration9.exact((group, point, tau_row))
        ...
        output.append(
            {
                **{
                    ...
                    for name in (
                        "latitude_deg",
                        "longitude_deg",
                        "east_km_from_iteration9",
                        "north_km_from_iteration9",
...
def main() -> None:
    ...
    parser.add_argument("--iteration9", type=Path, default=ITER9)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--workers", type=int, default=4)
    args = parser.parse_args()
    if args.output.exists() or not 1 <= args.workers <= 4:
        raise ValueError("existing output or workers must be 1..4")
    iteration9 = load_iteration9(args.iteration9)
    origin = {name: float(iteration9["winner"][name]) for name in ("latitude_deg", "longitude_deg")}
    parents = [origin]
    levels = []
    ...
    payload = {
        "schema": "ds1-iteration10-symmetric-joint-refinement/v1",
        "complete": True,
        "partition": "train",
        "reference_used_for_fit": False,
        "observation_policy": "all_qualified_observations",
        "iteration9_input": {"path": str(args.iteration9), "sha256": digest(args.iteration9)},
        "origin": origin,
        "origin_policy": (
            "sealed iteration-9 winner only; no post-seal reference error or "
            "direction used for proposal construction or selection"
        ),
        ...
        "bindings": {
            ...
            "iteration9_driver": digest(ITER9_RUN),
            "iteration9_inference": digest(args.iteration9),
        },
    }
