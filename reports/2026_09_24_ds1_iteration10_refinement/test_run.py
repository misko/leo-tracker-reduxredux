...
def module():
    ...
    spec = importlib.util.spec_from_file_location("i10test", path)
...
def test_symmetric_lattice_has_center_and_opposite_offsets():
    ...
    rows = m.lattice(origin, 0.1953125, origin)
    assert len(rows) == 9
    assert sorted({round(row["east_km_from_iteration9"], 8) for row in rows}) == pytest.approx(
        [-0.1953125, 0.0, 0.1953125], abs=1e-7
    )
    assert sorted({round(row["north_km_from_iteration9"], 8) for row in rows}) == pytest.approx(
        [-0.1953125, 0.0, 0.1953125], abs=1e-7
    )
...
def test_equal_weight_combination_and_independent_taus():
    ...
    point = {
        "latitude_deg": 1.0,
        "longitude_deg": 2.0,
        "east_km_from_iteration9": 0.0,
        "north_km_from_iteration9": 0.0,
    }
    scans = [
