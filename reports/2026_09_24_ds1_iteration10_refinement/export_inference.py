#!/usr/bin/env python3
"""Export reference-free iteration-10 finalist artifacts."""
...
def main() -> None:
    ...
    for rank, candidate in enumerate(data["exact_finalists"], 1):
        ...
        rows.append(
            {
                "exact_rank": rank,
                "east_km_from_iteration9": candidate["east_km_from_iteration9"],
                "north_km_from_iteration9": candidate["north_km_from_iteration9"],
    ...
    with (args.output_dir / "iteration10-inference-finalists.csv").open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(rows[0]))
        writer.writeheader()
        writer.writerows(rows)
    figure, axis = plt.subplots(figsize=(5, 4), constrained_layout=True)
    scatter = axis.scatter(
        [row["east_km_from_iteration9"] for row in rows],
        [row["north_km_from_iteration9"] for row in rows],
        c=[row["balanced_exact_capped_loss"] for row in rows],
        s=75,
        cmap="viridis",
    )
    axis.scatter(
        rows[0]["east_km_from_iteration9"],
        rows[0]["north_km_from_iteration9"],
        marker="*",
        s=210,
        c="crimson",
    )
    axis.set(xlabel="east from iteration 9 (km)", ylabel="north from iteration 9 (km)")
    figure.colorbar(scatter, ax=axis, label="balanced exact capped loss")
    figure.savefig(args.output_dir / "iteration10-inference-finalists.png", dpi=160)
