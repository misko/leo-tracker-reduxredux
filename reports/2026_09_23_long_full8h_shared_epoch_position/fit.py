#!/usr/bin/env python3
"""Fit position and conditional per-scan epoch effects on full 00Z TRAIN."""
...
def score(single, tracks, point, etas, scale, include_evaluation=False):
    ...
    rows = [
        track_score(single, track, receiver, up, etas[track["session_id"]], include_evaluation)
        for track in tracks
    ]
    ...
    for iteration in range(1, 51):
        ...
        for track in tracks:
            if not active[track["track_id"]]:
                continue
            sat, eta = track["session_id"], etas[track["session_id"]]
...
def fit_arm(single, tracks, prior_name, baseline, scale):
    ...
    satellites = sorted({track["session_id"] for track in tracks})
    grouped = {
        satellite: [track for track in tracks if track["session_id"] == satellite]
...
def run(args):
    ...
    if len(sessions) != 72 or baseline["session_ids"] != sessions:
        raise ValueError("baseline sessions must exactly match frozen 72-scan TRAIN cohort")
    baseline_bindings = {row["session_id"]: row for row in baseline["bindings"]["sessions"]}
    ...
    for baseline_search in baseline["searches"]:
        ...
        zero = {track["session_id"]: 0.0 for track in tracks}
    ...
    inference = {
        "schema": "conditional-full8h-shared-epoch-position/v1",
    ...
    for arm in results["arms"]:
        baseline_search = next(row for row in baseline["searches"] if row["prior"] == arm["prior"])
