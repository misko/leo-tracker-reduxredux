#!/usr/bin/env python3
"""Close a DS1 basin after one full-catalogue reassociation, then freeze it."""
...
ASSOCIATION_UPDATE = HERE / "frozen-association-update.json"
...
def main() -> None:
    ...
    update = json.loads(ASSOCIATION_UPDATE.read_text())
    if update.get("passed") is not True or update.get("reference_used") is not False:
        raise ValueError("association update must pass and be reference-free")
    for group in GROUPS:
        sealed_groups[group]["track_associations"] = update["dynamic_result"][
            "per_group"
        ][group]["track_associations"]
    origin = {
    ...
    output = {
        "schema": "ds1-iteration17-reacquire-then-freeze-basin/v1",
        "complete": complete,
        ...
        "association_update": {
            "path": str(ASSOCIATION_UPDATE),
            "sha256": digest(ASSOCIATION_UPDATE),
        },
        "association_policy": "full-catalogue reacquisition once at parent, then frozen",
        "origin": origin,
