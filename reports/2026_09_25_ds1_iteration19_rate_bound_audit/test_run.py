...
def fit(loss: float, *, boundary_count: int = 0) -> dict:
    return {
        "selection_objective": loss,
        "exact_full_observation_capped_loss": loss,
        "converged": True,
        "rate_boundary_count": boundary_count,
    }
...
def test_combine_rejects_numerically_guarded_rate_boundary() -> None:
    m = module()
    point = {
        "latitude_deg": 1.0,
        "longitude_deg": 2.0,
    ...
    audits = [
        {**point, "group_id": "20260921_00", "rate_only": fit(0.02, boundary_count=1)},
        {**point, "group_id": "20260921_16", "rate_only": fit(0.10)},
    ]
    with pytest.raises(ValueError, match="no converged coordinate"):
        m.combine(audits, [point])
