#!/usr/bin/env python3
"""Reference-free exact replay and nuisance-bound gate for iteration 19."""
...
ITERATION19_RUNNER = Path(__file__).with_name("run.py")
...
def main() -> None:
    ...
    runner = load_module(ITERATION19_RUNNER, "i19_qualify_runner")
    runner.configure_driver(driver)
    ...
    for group in GROUPS:
        ...
        rate_boundary_count = sum(
            abs(value) >= RATE_BOUND_S_H - RATE_BOUNDARY_MARGIN_S_H
            for value in rates
        )
        ...
        gates.append(
            {
                ...
                "rate_boundary_count": rate_boundary_count,
                "rate_boundary_margin_s_h": RATE_BOUNDARY_MARGIN_S_H,
                "rates_beyond_control_bound_count": sum(
                    abs(value) > 0.25 for value in rates
                ),
    ...
    output = {
        "schema": "ds1-iteration19-widened-rate-bound-qualification/v1",
