#!/usr/bin/env python3
"""Reference-free local refinement of every sealed DS2 joint winner."""
...
RADII_KM = (4.6875, 9.375, 18.75)
LEVELS_KM = (1.5625, 0.78125, 0.390625)
...
def boundary_triggered(radial_km: float, radius_km: float) -> bool:
    """Require one complete first-level cell between a winner and the edge."""
    return radial_km >= radius_km - LEVELS_KM[0] - 1e-9


def levels_for_radius(radius_km: float) -> tuple[float, ...]:
    """Keep expanded windows bounded by prepending reusable coarse scales."""
    levels = []
    level = radius_km / 3.0
...
def run_one(coarse_task: dict[str, Any], cache_root: Path) -> dict[str, Any]:
    coarse_path = Path(coarse_task["output_path"])
    if not valid(coarse_path):
        raise ValueError(f"coarse artifact is not sealed: {coarse_path}")
    coarse = json.loads(coarse_path.read_text())
    point = coarse["estimated_position"]
    label = coarse_task["task_id"].removeprefix("joint-all20__")
    rows = []
    final_path = None
    exhausted = True
    ...
    for attempt, radius in enumerate(RADII_KM, 1):
        ...
        output = HERE / "inference_refined" / f"joint-all20__{label}__r{radius:g}.json"
        task = {
            **coarse_task,
            "task_id": f"joint-all20-refined__{label}__attempt-{attempt}",
            "group_id": "ds2-sept24-all-reference-free-local-refinement",
            "prior": {
                "name": f"sealed-{label}-winner-symmetric-{radius:g}km",
            ...
            "refinement_parent_sha256": digest(coarse_path),
            "reference_coordinate_present": False,
        }
        task_path = HERE / "tasks_refined" / f"{task['task_id']}.json"
        ...
        if not valid(output):
            ...
            if result.returncode != 0 or not valid(output):
                raise RuntimeError(
                    f"refinement failed for {label} attempt {attempt}: {result.stderr[-4000:]}"
                )
        ...
        # One complete first-level cell between the winner and boundary is the
        # deterministic interior criterion. Otherwise expand symmetrically.
    ...
    return {
        "method": label,
        "coarse_artifact": str(coarse_path),
        "coarse_sha256": digest(coarse_path),
        "centre_policy": "each method's own sealed coarse winner; no reference coordinate",
...
def main() -> None:
    ...
    tasks = [row for row in plan["tasks"] if row["task_id"].startswith("joint-all20__")]
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as pool:
        rows = list(pool.map(lambda task: run_one(task, args.cache_root), tasks))
    document = {
        "schema": "ds2-reference-free-local-refinement-index/v1",
        "complete": True,
        "reference_coordinate_present": False,
    ...
    path = HERE / "refinement-index.json"
