...
def describe(path):
    ...
    normalizer = np.sum(window**2)
    total = np.zeros(FFT)
    segments = 0
    energy = 0.0
    lags = {lag: [] for lag in (9000, 10000, 11000)}
    for start in range(0, len(iq) - 131072 + 1, 131072):
        ...
        energy += float(np.sum(abs(block) ** 2))
        segments += len(block)
        for lag in lags:
            lags[lag].append(lag_power(x, lag))
    ...
    result = dict(
        ...
        analyzed_samples=segments * FFT,
        omitted_tail_samples=len(iq) - segments * FFT,
        iq_sha256=digest(path),
        windowed_complex_power=float(psd.sum() * RATE / FFT),
        central_power_fraction={
            str(hz): float(psd[abs(frequency) < hz].sum() / psd.sum())
            for hz in (250000, 500000, 1000000)
        },
        lag_power_quantiles={
            str(lag): np.quantile(values, [0, 0.5, 0.9, 1]).tolist() for lag, values in lags.items()
        },
        lag_seconds={str(lag): lag / RATE for lag in lags},
        spectral_flatness=float(
            np.exp(np.mean(np.log(np.maximum(psd, np.finfo(float).tiny)))) / np.mean(psd)
        ),
    )
...
def main():
    ...
    tone = np.exp(2j * np.pi * 0.03125 * np.arange(131072))
    ...
    result = dict(
        scope="saved_IQ_descriptive_spectra_and_lag_power",
        new_rf_samples=0,
        sample_rate=RATE,
        fft_samples=FFT,
        frequency_bin_hz=RATE / FFT,
        calibrated_rf_power=False,
        ground_truth_signal_labels=False,
    ...
    for label in [*paths, "positive", "control"]:
        ...
        row, frequency, psd = describe(path)
        assert row["iq_sha256"] == expected
        result["cases"][label] = row
        curves[label] = psd
    result["source_sha256"] = digest(Path(__file__))
    with (BASE / "saved-visit-spectra-v2.json").open("x") as f:
    ...
    for label, psd in curves.items():
        axes[0].plot(
            frequency / 1e6, 10 * np.log10(psd / psd.mean()), label=label, lw=1, alpha=0.75
        )
    axes[0].set(
        xlabel="Frequency relative to tuned center (MHz)",
        ylabel="PSD / mean PSD (dB)",
    ...
    for label, row in result["cases"].items():
        axes[1].plot(
            [3.6, 4, 4.4],
            [row["lag_power_quantiles"][str(l)][1] for l in (9000, 10000, 11000)],
            "o-",
            label=label,
        )
    axes[1].set(
        xlabel="Lag (ms); 4 ms is three nominal frames",
        ylabel="Median squared normalized lag correlation",
        yscale="log",
    )
