...
def main():
    ...
    track = next(
        r["measurements"] for r in track["cases"] if r["label"] == "positive" and r["feedback"]
    )
    ...
    for case in manifest["cases"]:
        ...
        supported = [
            r
            for r in track
            if offset + 22 <= r.get("start", -1) < offset + 3333 and r.get("rejection") == 0
        ]
        if not supported:
            continue
        ...
        for f in range(11):
            for epoch in range(3333):
                value = int(grid[f, epoch])
                left = int(grid[f, epoch - 1]) if epoch else 0
        ...
        for row in candidates:
            if any(
                min(abs(row[0] - p[0]), 3333 - abs(row[0] - p[0])) < 20 and abs(row[1] - p[1]) <= 1
                for p in selected
            ):
                continue
            selected.append(row)
        ...
        for row in supported:
            ...
            hits = [
                ...
                if min(abs(p[0] - target), 3333 - abs(p[0] - target)) <= 8
            ]
            targets.append(
                dict(
                    frame=row["frame"],
                    pilot_offset=row["start"] - offset,
        ...
        print(
            case["number"], [(r["frame"], r["first_candidate_rank"]) for r in targets], flush=True
        )
    with (BASE / "positive-proposal-coverage-v1.json").open("x") as stream:
        json.dump(
            dict(
                scope="coarse_proposal_rank_near_reviewed_supported_timing",
                cases=cases,
                maximum_candidates_examined=512,
                coarse_frequency_not_a_requirement=True,
