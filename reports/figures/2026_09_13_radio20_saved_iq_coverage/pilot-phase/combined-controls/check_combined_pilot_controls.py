...
def evaluate(iq,first,cfo,refs):
    frames=np.arange(8)*9;grid=np.linspace(cfo-1000,cfo+1000,401)
    trained=[]
    for frame in frames[:4]:
        at=first+int(frame)*10000//3;z=iq[at+32:at+3268]
        assert len(z)==3236
        best=None
        for quarter in range(-64,65):
            whole,phase=divmod(quarter,4);ref=refs[phase,32-whole:3268-whole]
            ...
            row=dict(frame=int(frame),offset_quarters=quarter,cfo_hz=float(grid[k]),power=power)
            if best is None or power>best['power']:best=row
        trained.append(best)
    ...
    for frame in frames[4:]:
        ...
        quarter=round(float(np.polyval(timing,frame))*4);whole,phase=divmod(quarter,4)
        if 32-whole<0 or 3268-whole>3300:
            return dict(training=trained,score=None,reason='reference_extent_exceeded')
        z=iq[at+32:at+3268];ref=refs[phase,32-whole:3268-whole]
    ...
    return dict(training=trained,heldout=held,score=float(np.mean([r['power'] for r in held])),
                minimum_heldout_power=min(r['power'] for r in held),reason='evaluated')
...
def main():
    ...
    raw=np.fromfile(BASE/'direct-references.ci16',dtype='<i2').reshape(4,3300,4).astype(float)
    assert digest(BASE/'direct-references.ci16')=='78b50e1aea5c350889b0798fc691491299925932e496a918cd5fbd3b9bc4faf2'
    refs=raw[:,:,0]+1j*raw[:,:,1]
    manifest=json.loads((BASE/'paced-original-seed-input-v1/manifest.json').read_text())
    out=BASE/'combined-pilot-controls-v1';out.mkdir(exist_ok=False)
    result=dict(scope='same_C_selected_proposal_four_training_four_heldout_mean_power',new_rf_samples=0,
                ...
                source_sha256=digest(Path(__file__)),bench_sha256=digest(binary),cases=[])
    for label in ('positive','control'):
        path=BASE/f'paced-original-seed-input-v1/{label}.ci16';assert digest(path)==manifest[label]['sha256']
        ...
        for number,offset in enumerate(range(0,len(raw)-447851+1,447851)):
            root=out/f'{label}-{number}';root.mkdir();cut=raw[offset:offset+447851]
            cut.tofile(root/'iq.ci16')
            run=subprocess.run([str(binary),str(BASE/'coarse-bank.ci16'),str(root/'iq.ci16'),
                                str(BASE/'direct-references.ci16')],capture_output=True,timeout=12)
            (root/'stdout.jsonl').write_bytes(run.stdout);run.check_returncode();assert not run.stderr
            rows=[json.loads(line) for line in run.stdout.splitlines()]
            initial=next(r for r in rows if r['kind']==3);assert initial['frame']==initial['reference_phase']==0
            ...
            result['cases'].append(dict(label=label,number=number,source_offset=offset,source_sha256=manifest[label]['sha256'],
                input_sha256=digest(root/'iq.ci16'),journal_sha256=digest(root/'stdout.jsonl'),
                proposal=rows[0],first=initial['first'],resolver_cfo_hz=frequency,**scored))
