...
def main():
    output = EVIDENCE / 'adjacent-arm-v1-results'; output.mkdir(exist_ok=False)
    build = json.loads((EVIDENCE / 'adjacent-arm-build-v1.json').read_text())
    files = dict(probe='adjacent-bench-arm-v1', refs='direct-references.ci16',
    ...
    try:
        lease = authority.claim(('radio_pluto_5d4d',), task_id='adjacent-arm-v1', task_kind=CaptureTaskKind.QUALIFICATION)
    ...
    with lease, acquire_radio_lock(ENDPOINT[0]):
        ...
        try:
            ...
            completed = run(shlex.join([remote+'/probe', remote+'/iq', remote+'/refs', remote+'/training']))
            terminal = True
            (output / 'stdout.jsonl').write_bytes(completed.stdout)
            (output / 'stderr.txt').write_bytes(completed.stderr)
            evidence.update(exit_code=completed.returncode, stdout_sha256=hashlib.sha256(completed.stdout).hexdigest())
            completed.check_returncode(); evidence['status'] = 'complete_review_pending'
        except BaseException as error:
