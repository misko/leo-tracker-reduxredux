...
def main():
    ...
    expected = next(
        r["measurements"] for r in prior["cases"] if r["label"] == "positive" and r["feedback"]
    )
    statistics = {}
    for name, path in [
        ("host", BASE / "adjacent-host-bench-v1.jsonl"),
        ("arm", output / "stdout.jsonl"),
    ]:
        rows = [json.loads(line) for line in path.read_text().splitlines()]
        summary = rows.pop()
        assert summary["summary"] and len(rows) == 1768
        ...
        statistics[name] = dict(
            compute_seconds=summary["compute_ns"] / 1e9,
            mean_ms=float(elapsed.mean() / 1e6),
            median_ms=float(np.median(elapsed) / 1e6),
            p99_ms=float(np.quantile(elapsed, 0.99) / 1e6),
            maximum_ms=summary["maximum_ns"] / 1e6,
            over_frame_period=int(np.count_nonzero(elapsed > 1e9 / 750)),
            compute_to_source_time_ratio=summary["compute_ns"] / 1e9 / (1768 / 750),
        )
    result = dict(
        status="pass",
        scope="ARM_host_jobs_and_fits_against_independently_checked_replay",
        matched_measurements_per_platform=1768,
        accepted=1367,
        statistics=statistics,
        receiver_paced=False,
        RF_collection_and_DMA_cost_included=False,
        timing_excludes_input_loading_and_output_serialization=True,
