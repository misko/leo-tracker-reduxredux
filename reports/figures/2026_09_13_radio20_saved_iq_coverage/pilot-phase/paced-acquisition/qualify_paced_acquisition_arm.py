...
def main():
    ...
    files = dict(
        probe="paced-acquisition-arm-v2.bin",
        bank="coarse-bank.ci16",
        refs="direct-references.ci16",
        positive="paced-original-seed-input-v1/positive.ci16",
        control="paced-original-seed-input-v1/control.ci16",
    )
    ...
    for case in build["cases"]:
        assert hashes[case["label"]] == case["input_sha256"]
    ...
    evidence = dict(
        scope="paced_saved_IQ_acquisition_observer_ARM",
        new_rf_samples=0,
        native_jobs=0,
        status="started",
        serial=ENDPOINT[0],
    ...
    try:
        lease = authority.claim(
            ("radio_pluto_5d4d",),
            task_id="paced-acquisition-arm-v2",
    ...
    with lease, acquire_radio_lock(ENDPOINT[0]):
        ...
        try:
            ...
            for name, data in payload.items():
                ...
                check = run("sha256sum " + shlex.quote(remote + "/" + name))
                check.check_returncode()
                assert check.stdout.decode().split()[0] == hashes[name]
            ...
            for label, cut in [("positive", 0), ("positive", 3), ("control", 0)]:
                ...
                result = run(
                    ...
                    + " > "
                    + shlex.quote(directory + "/stdout.json")
                )
                ...
                archive = run("tar -C " + shlex.quote(remote) + " -cf - " + shlex.quote(name))
                archive.check_returncode()
                with tarfile.open(fileobj=io.BytesIO(archive.stdout)) as tar:
                    assert {m.name.rstrip("/") for m in tar.getmembers()} == {
                        name,
                        *(name + "/" + n for n in names),
                    }
                    tar.extractall(output, filter="data")
                row = json.loads((output / name / "stdout.json").read_text())
                evidence["cases"][-1]["summary"] = row
                evidence["cases"][-1]["sha256"] = {
                    n: hashlib.sha256((output / name / n).read_bytes()).hexdigest() for n in names
                }
                save()
            ...
            manifest = dict(
                cases=[
                    dict(label=c["label"], input_sha256=hashes[c["label"]], **c["summary"])
            ...
            try:
                ...
                if terminal:
                    paths = [remote + "/" + k for k in payload] + [
                        d + "/" + n for d in directories for n in names
                    ]
                    run(
                        "rm -f "
                        + shlex.join(paths)
                        + " && rmdir "
                        + shlex.join([*directories, remote])
                    ).check_returncode()
