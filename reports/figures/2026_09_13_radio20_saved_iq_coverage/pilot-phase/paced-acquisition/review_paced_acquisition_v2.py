...
refs = (
    np.fromfile(BASE / "direct-references.ci16", dtype="<i2").reshape(4, 3300, 4).astype(np.int64)
)
bases = []
for r in refs:
    z = r[:, 0] + 1j * r[:, 1]
    d = r[:, 2] + 1j * r[:, 3]
    bases.append(
        np.column_stack((z, -d, 1j * np.pi * 1000 / 2500000 * (2 * np.arange(3300) - 3299) * z))
    )
checks = []
for case in manifest["cases"]:
    ...
    np.testing.assert_allclose(
        [resolved[k] for k in ("best_shift", "cfo_hz", "coherence")],
        max(hypotheses, key=lambda h: h[2]),
        rtol=2e-12,
        atol=2e-8,
    )
    retained = np.fromfile(directory / "worker.iq.ci16", dtype="<i2").reshape(-1, 2)
    at = 0
    past = []
    for row in rows:
        ...
        np.testing.assert_array_equal(retained[at : at + count], iq[first : first + count])
        at += count
        view = row["copied"] if row["kind"] == 1 else row["source"]
        assert view["epoch"] == 1 and view["valid"] and not view["closed"]
        assert view["first"] <= first and first + count <= view["end"] <= view["source_now"] <= len(
            iq
        )
        assert view["observed_ns"] <= row["recorded_ns"]
        if row["kind"] == 1:
            continue
        z = rotated(iq[first : first + count], 0, row["phase_step"])
    ...
    for row in obsrows:
        if row["kind"] == "measurement":
            np.testing.assert_array_equal(
                obsraw[row["iq_offset"] : row["iq_offset"] + 3300],
                iq[row["first"] : row["first"] + 3300],
            )
    checks.append(
        dict(
            label=case["label"],
...
result = dict(
    ...
    manifest_sha256=digest(root / "result.json"),
)
with (root / "independent-review-v2.json").open("x") as f:
