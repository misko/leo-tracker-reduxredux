...
out = BASE / "paced-acquisition-host-v2-results"
out.mkdir(exist_ok=False)
header = FW / "tests/starlink_glrt/iio_probe_fixture/iio.h"
(out / "iio.h").write_text(
    header.read_text()
    ...
    subprocess.run(
        [
            ...
            *map(str, sources),
            ...
            "-lm",
            "-o",
            str(BASE / f"paced-acquisition-{arch}-v2.bin"),
        ],
        check=True,
    )
...
result = dict(
    new_rf_samples=0,
    sources={str(p): digest(p) for p in [*sources, FW / "tools/glrt_cpu_live_probe.c"]},
    arm_sha256=digest(BASE / "paced-acquisition-arm-v2.bin"),
    runner_sha256=digest(Path(__file__)),
    cases=[],
)
for label, cut in [("positive", 0), ("positive", 3), ("control", 0)]:
    directory = out / f"{label}-{cut}"
    directory.mkdir()
    iq = BASE / f"paced-original-seed-input-v1/{label}.ci16"
    run = subprocess.run(
        [
            str(BASE / "paced-acquisition-host-v2.bin"),
            str(iq),
    ...
    result["cases"].append(dict(label=label, input_sha256=digest(iq), **row))
