...
def main():
    if len(sys.argv) not in (3,6,7): raise SystemExit('usage: qualify_scan64_frequency_revisits.py NEW_OUTPUT DEPLOYMENT_RECEIPT [CH CH CH [CH]]')
    channels=tuple(map(int,sys.argv[3:])) if len(sys.argv)>3 else (1,2,3,4)
    count=len(channels)
    if count not in (3,4) or any(n not in (1,2,3,4) for n in channels) or any(a==b for a,b in zip(channels,channels[1:])):
        raise ValueError('three or four upper-edge visits with distinct adjacent channels are required')
    ...
    if matched is None: raise ValueError('deployment is not a frozen 30/60-MS/s tracking image')
    rate=int(matched[1])
    payload={'probe':(E/'glrt-cpu-visit-scan64-v1').read_bytes(),
    ...
    receipt={'scope':'bounded_ARM_frequency_revisits','visit_count':count,'child_profile':'1536-selected-observer3-scan64','candidate_budget':64,'ranking_fft':4096,'observer_spacing':3,'retention_mode':'selected_windows','status':'started','rate':rate,
        'serial':live.ENDPOINT[0],'lo_plan_hz':frequencies,
        'rf_sample_limit':count*25165824,'rf_duration_limit_s':count*25165824/2500000,
    ...
    names=['visits.txt']+[f'visit-{n}/{f}' for n in range(count) for f in (*('scan.iq.ci16' if a=='iq.ci16' else a for a in live.ARTIFACTS),'stdout.json','stderr.txt')]
    with lease,live.acquire_radio_lock(live.ENDPOINT[0]):
        ...
        try:
            ...
            try:
                ...
                receipt['configured']=live.configure_idle_rx(ctx,rate=rate,lo_hz=frequencies[0],evidence=receipt,
            ...
            result=run(shlex.join([remote+'/probe',str(rate),live.ENDPOINT[0],remote+'/bank',remote+'/references',remote,
                                  *map(str,receipt['lo_plan_hz']),'1536-selected-observer3-scan64']),timeout=70)
