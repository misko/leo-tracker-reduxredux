...
def test_valid_plan_is_bound_and_refused_before_radio_contact(
    monkeypatch, tmp_path, rate, channels
):
    ...
    class Authority:
        ...
        def claim(self, *args, **kwargs):
            raise Refused("test lease refusal")

    live = types.ModuleType("qualify_glrt_cpu_live20")
    live.EVIDENCE = BASE
    live.ENDPOINT = (SERIAL, "192.168.1.20")
    live.g = types.SimpleNamespace(
        deployment_identity=lambda *a, **k: (
            {"expected_firmware": f"glrt-iq-tracking-r{rate}-v1"},
            None,
        )
    )
    live.LocalCaptureAuthority = Authority
    live.RadioResource = lambda *a: a
    ...
    spec = importlib.util.spec_from_file_location(
        "operator_test", BASE / "qualify_scan64_frequency_revisits.py"
    )
    ...
    assert (
        r["rf_sample_limit"] == len(channels) * 25165824
        and r["retention_mode"] == "selected_windows"
    )
    assert len(calls) == 1
    assert r["child_profile"] == "1536-selected-observer3-scan64"
    assert (r["candidate_budget"], r["ranking_fft"], r["observer_spacing"]) == (64, 4096, 3)
...
def test_bad_plan_is_rejected_before_identity_or_evidence(monkeypatch, tmp_path, channels):
    ...
    spec.loader.exec_module(module)
    monkeypatch.setattr(
        sys, "argv", ["operator", str(tmp_path / "out"), "receipt", *map(str, channels)]
    )
