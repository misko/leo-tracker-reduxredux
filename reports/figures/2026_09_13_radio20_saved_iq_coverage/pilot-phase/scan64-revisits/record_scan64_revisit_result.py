...
root=BASE/'scan64-revisit60-v1'
op=json.loads((root/'operator.json').read_text())
children=[json.loads((root/f'visit-{n}/independent-visit-review.json').read_text()) for n in range(3)]
...
mutations=0
variants=[(text.replace('1536-selected-observer3-scan64','1536-selected'),children),
...
for field,value in [('probe_sha256','0'*64),('epoch_bindings',[[0,999,0,0]])]:
    changed=copy.deepcopy(children);changed[1][field]=value
    variants.append((text,changed))
for altered,reports in variants:
    try:review(root,text=altered,children=reports)
    except AssertionError:mutations+=1
    else:raise AssertionError('corrupted association accepted')
timings=[t for child in children for t in child['timings']]
result=dict(status='pass',serial=op['serial'],rate=op['rate'],lo_plan_hz=op['lo_plan_hz'],
    rf_seconds=sequence['rf_seconds'],mutations_rejected=mutations,
    ...
    native_tracking_qualified=False,physical_supported_loss_reacquisition_qualified=False,
    script_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest())
with (root/'result-summary.json').open('x') as f:json.dump(result,f,indent=2);f.write('\n')
