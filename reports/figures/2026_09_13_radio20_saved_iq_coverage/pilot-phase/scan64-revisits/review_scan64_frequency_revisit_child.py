...
def review(root):
    ...
    assert count in (3, 4) and root.name in [f"visit-{n}" for n in range(count)]
    artifacts = {
        name.split("/", 1)[1]: value
        for name, value in op["artifacts"].items()
    ...
    assert rf == initial
    assert (
        op["memory"]["required_kib"] == 360 * 1024 and op["memory"]["available_kib"] >= 360 * 1024
    )
    ...
    loss = None
    if status["status"] == 3:
        loss = review_clean_loss(
            rows,
            journals["native.journal"],
            status,
    ...
    return dict(
        status="pass",
        scope="selected_iq_acquisition_and_passive_observer_arithmetic_and_source_counters",
        passive_observer=observer,
        clean_loss=loss,
        child_disposition=status["status"],
        probe_sha256=op["payload_sha256"]["probe"],
        run_succeeded=status["status"] == 0,
        post_capture_rf_settings_checked="rf_after" in op,
        full_iq_retained=False,
