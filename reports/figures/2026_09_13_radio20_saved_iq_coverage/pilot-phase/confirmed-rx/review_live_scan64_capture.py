...
def review(root):
    ...
    assert op['payload_sha256']['probe']=='ede7d9bd6e60fb692c916d9f0707595d02d6f0a39066174c1aaf827616cfd456'
    assert op['passive_observer']=={'rate':2500000,'feedback_authority':False,
        'maximum_measurements_per_episode':200,'maximum_episodes':1,'frame_spacing':3}
    assert op['profile']=='1536-selected-observer3-scan64'
    ...
    assert rate in (30000000,60000000) and status['status'] == op['probe_exit_code']
    assert status['status'] in (0,1)
    assert status['reacquisitions']==0 and status['native_runs']<=1
    assert op['blocks'] == 1536 and op['rf_sample_limit'] == 25165824
    assert op['rf_duration_limit_s'] == 10.0663296
    ...
    assert op['memory_preflight']['required_kib'] == 176*1024
    assert op['filesystem_preflight']['required_kib'] == 136*1024
    assert op['memory_preflight']['available_kib'] >= 176*1024
    assert op['filesystem_preflight']['available_kib'] >= 136*1024
    ...
    journals={name:(root/name).read_bytes() for name,receipt in op['artifacts'].items()
    ...
    assert len(owned) <= 20000000 and 0 < len(scans) <= status['attempts'] <= 6
    ...
    for index,scan in enumerate(scans):
        ...
        assert scan['peaks']==[list(p) for p in sorted_peak_oracle(grids[index],64)]
...
if __name__=='__main__':
    ...
    with (root/'independent-observer-capture-review.json').open('x') as output:
