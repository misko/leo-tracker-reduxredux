...
def main():
    output = EVIDENCE / 'tracking-cadences-arm-v1-results'; output.mkdir(exist_ok=False)
    build = json.loads((EVIDENCE / 'cadence-arm-build-v1.json').read_text())
    files = dict(probe='cadence-bench-arm-v1', refs='direct-references.ci16',
                 training='adjacent-training-v1.txt', iq='paced-original-seed-input-v1/positive.ci16')
    observer_build = json.loads((EVIDENCE / 'observer-cadence-replay-build-v1.json').read_text())
    files['observer'] = 'observer-cadence-arm-v1'
    build['payloads']['observer'] = observer_build['arm_sha256']
    build['sources'].update(observer_build['sources'])
    ...
    evidence = dict(scope='saved_IQ_adjacent_feedback_ARM_compute', new_rf_samples=0, native_jobs=0,
                    tracking_qualified=False, payload_sha256=build['payloads'], status='started',
                    remote_directory=remote, serial=ENDPOINT[0], host=ENDPOINT[1])
    ...
    try:
        lease = authority.claim(('radio_pluto_5d4d',), task_id='tracking-cadences-arm-v1', task_kind=CaptureTaskKind.QUALIFICATION)
    ...
    with lease, acquire_radio_lock(ENDPOINT[0]):
        ...
        try:
            ...
            run('chmod 700 ' + shlex.join([remote+'/probe', remote+'/observer'])).check_returncode()
            save(); print('Running bounded saved-IQ ARM feedback replay', flush=True)
            evidence['cases'] = []
            for cadence in (2, 3):
                for offset in range(cadence):
                    completed = run(shlex.join([remote+'/probe', remote+'/iq', remote+'/refs', remote+'/training', str(cadence), str(offset)]))
                    name = f'c{cadence}-o{offset}.jsonl'
                    (output / name).write_bytes(completed.stdout)
                    evidence['cases'].append(dict(cadence=cadence, offset=offset, exit_code=completed.returncode,
                        journal=name, sha256=hashlib.sha256(completed.stdout).hexdigest(),
                        stderr=completed.stderr.decode()))
                    save()
                    completed.check_returncode()
            evidence['observer_cases'] = []
            for offset in range(3):
                completed = run(shlex.join([remote+'/observer', remote+'/iq', remote+'/refs', remote+'/training', str(offset)]))
                name = f'observer-o{offset}.jsonl'
                (output / name).write_bytes(completed.stdout)
                evidence['observer_cases'].append(dict(offset=offset, exit_code=completed.returncode,
                    journal=name, sha256=hashlib.sha256(completed.stdout).hexdigest(), stderr=completed.stderr.decode()))
                save()
                completed.check_returncode()
            terminal = True
            ...
            try:
                ...
                if terminal:
                    run('rm -f ' + shlex.join([remote+'/'+name for name in payload]) +
