...
def main():
    output = EVIDENCE / "tracking-cadences-arm-v1-results"
    ...
    try:
        lease = authority.claim(
            ("radio_pluto_5d4d",),
            task_id="tracking-cadences-arm-v1",
    ...
    with lease, acquire_radio_lock(ENDPOINT[0]):
        ...
        try:
            ...
            run(
                "chmod 700 " + shlex.join([remote + "/probe", remote + "/observer"])
            ).check_returncode()
            save()
            ...
            for offset in range(3):
                ...
                (output / name).write_bytes(completed.stdout)
                evidence["observer_cases"].append(
                    dict(
                        offset=offset,
                        exit_code=completed.returncode,
                        journal=name,
