...
SSH_ADVISORY = ('** WARNING: connection is not using a post-quantum key exchange algorithm.\r\n'
...
def main():
    ...
    assert host_review['status']=='pass' and host_review['independently_checked_dense_fits']==5545
    ...
    assert operator['payload_sha256']=={**build['payloads'],'observer':observer_build['arm_sha256']}
    assert [(r['cadence'],r['offset']) for r in operator['cases']]==[(2,0),(2,1),(3,0),(3,1),(3,2)]
    results=[]
    for case in operator['cases']:
        ...
        expected=next(r for r in build['cases'] if r['cadence']==case['cadence'] and r['offset']==case['offset'])
        ...
        summary=arm.pop();reference=host.pop();assert len(arm)==len(host)==summary['measurements']
        for key in ('cadence','offset','accepted','measurements','terminal_frame','prediction_result'):
        ...
        timing=np.array([r['elapsed_ns'] for r in arm]);assert np.all(timing>0)
        assert sum(timing)==summary['compute_ns'] and max(timing)==summary['maximum_ns']
        interval=case['cadence']/750
        results.append(dict(cadence=case['cadence'],offset=case['offset'],measurements=len(arm),
            accepted=summary['accepted'],mean_ms=float(timing.mean()/1e6),
            p99_ms=float(np.quantile(timing,.99)/1e6),maximum_ms=float(timing.max()/1e6),
    ...
    for case in operator['observer_cases']:
        ...
        arm=[json.loads(line) for line in path.read_text().splitlines()]
        host=[json.loads(line) for line in hostpath.read_text().splitlines()]
        summary=arm.pop();reference=host.pop()
        assert len(arm)==len(host)==200
        for key in ('offset','accepted','measurements','terminal_status'):assert summary[key]==reference[key]
        assert summary['terminal_status']==0 and summary['elapsed_ns']>0
        for row,truth in zip(arm,host):
            for key in ('frame','start','phase','step','rejection'):assert row[key]==truth[key]
            for key in ('coherence','delay_s','cfo_hz'):
                np.testing.assert_allclose(row[key],truth[key],rtol=2e-10,atol=1e-12)
        observer_results.append(dict(offset=case['offset'],accepted=summary['accepted'],measurements=200,
            total_seconds=summary['elapsed_ns']/1e9,
            compute_to_source_interval_ratio=summary['elapsed_ns']/1e9/.8))
    result=dict(status='pass',cases=results,observer_cases=observer_results,native_tracking_qualified=False,receiver_paced=False,
                capture_and_logging_cost_included=False,reviewer_sha256=digest(Path(__file__)),
