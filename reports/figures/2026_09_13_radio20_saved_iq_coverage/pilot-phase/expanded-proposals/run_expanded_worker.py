...
command = [
    ...
    "-I",
    str(PREFIX / "include"),
    str(BASE / "expanded_worker_bench.c"),
...
source = BASE / "expanded-proposal-ranking-v1.json"
ranking = json.loads(source.read_text())
...
result = dict(
    ...
    ranking_sha256=digest(source),
    binary_sha256=digest(out / "bench"),
    runner_sha256=digest(Path(__file__)),
    bench_source_sha256=digest(BASE / "expanded_worker_bench.c"),
    references_sha256=digest(BASE / "direct-references.ci16"),
    production_source_sha256={
        n: digest(FW / "tools" / n) for n in [*names, "glrt_cpu_tracking_bench.c"]
    },
    cases=[],
)
for case, original in zip(ranking["cases"], prior["cases"]):
    ...
    budgets = [
        dict(
            budget=b["budget"],
            **{
                k: dict(winner=b[k + "_winner"], **runs[b[k + "_winner"]])
                for k in ("single", "four")
            },
        )
    ...
    print(
        case["label"],
        case["number"],
        [(b["budget"], b["single"]["accepted"], b["four"]["accepted"]) for b in budgets],
        flush=True,
    )
