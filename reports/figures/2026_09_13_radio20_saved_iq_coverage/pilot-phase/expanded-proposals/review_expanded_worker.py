...
exec(compile(reviewer.read_text().split('\nmanifest=json.loads',1)[0],str(reviewer),'exec'),namespace)
    ...
    for index,run in case['runs'].items():
        ...
        if case['number']==0 and int(index)==min(map(int,case['runs'])):
            for field in ('coherence','cfo_hz'):
                ...
                try:namespace['check'](description,changed)
                except AssertionError:mutations+=1
                else:raise AssertionError('mutation accepted')
result=dict(status='pass',cases=checks,mutations_rejected=mutations,
    reviewer_sha256=namespace['digest'](Path(__file__)),oracle_reviewer_sha256=namespace['digest'](reviewer),
    result_sha256=namespace['digest'](root/'result.json'),native_tracking_qualified=False)
with (root/'independent-review.json').open('x') as f:json.dump(result,f,indent=2);f.write('\n')
print(json.dumps(dict(runs=len(checks),hypotheses=sum(r['resolver_hypotheses'] for r in checks),fits=sum(r['moment_dense_fits'] for r in checks),mutations=mutations)))
