...
def review(root):
    ...
    assert len(pairs)==terminal['paired_copied']==terminal['paired_queued']==min(64,len(native['heads']))
    ...
    for index,row in enumerate(pairs):
        head=native['heads'][index]; estimate=native['estimates'][index]
        ...
        checked.append(dict(sequence=index,frame=estimate['frame'],native_start=head.start,
    ...
    return dict(status='pass',scope='same_pilot_coarse_iq_native_moment_comparison',rate=rate,
        pairs=len(checked),native_total_results=len(native['heads']),
        ...
        sha256={name:hashlib.sha256((root/name).read_bytes()).hexdigest() for name in
                ('native.coarse.ci16','native.journal','worker.jsonl','operator.json','stdout.json')},
        reviewer_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest())
