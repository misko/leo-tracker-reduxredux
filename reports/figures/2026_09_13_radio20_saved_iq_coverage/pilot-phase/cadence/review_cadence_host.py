...
def main():
    ...
    for raw in refs:
        ...
        bases.append(np.column_stack((pilot, -raw[:, 2] - 1j * raw[:, 3],
    ...
    for case in build['cases']:
        ...
        for i, row in enumerate(rows):
            ...
            np.testing.assert_allclose(row['delay_s']*1e6, np.clip(correction[0],-.25,.25), rtol=2e-10, atol=2e-11)
            np.testing.assert_allclose(row['cfo_hz'],row['step']*2500000/2**32+np.clip(correction[1],-.25,.25)*1000,
                                       rtol=2e-12,atol=2e-8)
            assert row['rejection'] == ((32 if np.any(abs(correction)>=.25) else 0)|(64 if coherence<.05 else 0))
            count += 1
        cases.append(summary)
    result = dict(status='pass', independently_checked_dense_fits=count, cases=cases,
                  native_tracking_qualified=False, arm_cadence_timing_verified=False,
