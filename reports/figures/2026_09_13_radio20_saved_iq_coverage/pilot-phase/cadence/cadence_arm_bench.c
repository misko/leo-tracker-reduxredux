...
int main(int argc,char **argv) {
    unsigned cadence,offset,measured=0,terminal_frame=1800;int prediction_result=0;
    ...
    alarm(15);
    if(argc!=6 || !iq || !refs) return 2;
    cadence=(unsigned)strtoul(argv[4],NULL,10);offset=(unsigned)strtoul(argv[5],NULL,10);
    if(cadence<1 || cadence>9 || offset>=cadence) return 2;
    ...
    for(unsigned frame=32+offset;frame<1800;frame+=cadence) {
        ...
        uint64_t begin=now();
        prediction_result=glrt_tracking_trend_batch(&trend,frame,1,frame,0,&b,&slope);
        if(prediction_result) { terminal_frame=frame;break; }
        if(
        ...
        accepted+=e.rejection==0;measured++;
    ...
    printf("{\"summary\":true,\"cadence\":%u,\"offset\":%u,\"measurements\":%u,\"accepted\":%u,\"terminal_frame\":%u,\"prediction_result\":%d,\"compute_ns\":%" PRIu64 ",\"maximum_ns\":%" PRIu64 "}\n",cadence,offset,measured,accepted,terminal_frame,prediction_result,total,maximum);
