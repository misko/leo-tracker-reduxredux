...
from tests.starlink_glrt.test_cpu_coarse import integer_grid, bank, sorted_peak_oracle
...
def review(root):
    ...
    count=op['visit_count'];assert count in (3,4) and root.name in [f'visit-{n}' for n in range(count)]
    artifacts={name.split('/',1)[1]:value for name,value in op['artifacts'].items() if name.startswith(root.name+'/')}
    ...
    assert op['payload_sha256']['probe']=='5eff9cacccf945414c2f1ce2637142c85829b9484feecd12fd5b296063757a84'
    assert op['child_profile']=='1536-selected-observer3-scan64'
    assert (op['candidate_budget'],op['ranking_fft'],op['observer_spacing'])==(64,4096,3)
    observer=observer_review(root,expected_spacing=3)
    rate = status['rate']; ratio = rate//2500000
    assert rate in (30000000,60000000) and op['exit_code']==0
    assert status['status'] in (0,3)
    assert op['rf_sample_limit']==count*25165824
    assert op['rf_duration_limit_s']==count*25165824/2500000
    assert status['retention_mode'] == op['retention_mode'] == 'selected_windows'
    assert op['before'] == op['after'] and op['temporary_files_removed']
    rf=dict(op['rf_after']);initial=dict(op['configured']['rf_state'])
    assert abs(int(rf.pop('rx_lo'))-op['lo_plan_hz'][-1])<=16
    assert abs(int(initial.pop('rx_lo'))-op['lo_plan_hz'][0])<=16
    assert rf==initial
    assert op['memory']['required_kib']==360*1024 and op['memory']['available_kib']>=360*1024
    assert not (root/'iq.ci16').exists()
    ...
    for line in (root/'capture.txt').read_text().splitlines():
        ...
        if key == 'capture_snapshot':
            ...
            blocks.append((gen,w))
        elif key == 'capture_final_snapshot': _,final=capture(wire,rate)
        elif key == 'epoch_binding': bindings.append(list(map(int,wire.split())))
        elif key == 'tracking_snapshot': snapshots.append(TrackingSnapshot.from_sysfs(wire))
    assert 0<len(blocks)==status['completed_refills']==status['blocks']<=1536
    ...
    for state in snapshots[1:]: assert state.cdc_drops == state.pacer_drops == 0
    ...
    journals={name:(root/name).read_bytes() for name,receipt in artifacts.items()
    ...
    assert status['reacquisitions']==0
    loss=None
    if status['status']==3:
        loss=review_clean_loss(rows,journals['native.journal'],status,
            [json.loads(line) for line in (root/'observer.jsonl').read_text().splitlines()])
    scans=[r for r in rows if r['kind']=='scan']
    searched=np.fromfile(root/'scan.iq.ci16',dtype='<i2').reshape(-1,2)
    owned=np.fromfile(root/'worker.iq.ci16',dtype='<i2').reshape(-1,2)
    assert len(searched) == status['retained_scan_samples'] == len(scans)*14000 <= 84000
    assert len(owned)<=6*(13316+200*3300) and 0<len(scans)<=status['attempts']<=6
    grids=np.fromfile(root/'grids.u32',dtype='<u4').reshape(len(scans),11,3333)
    refs=np.fromfile(root.parent.parent/'direct-references.ci16',dtype='<i2').reshape(4,3300,4)
    bases=[]
    for raw in refs:
    ...
    cuts=[]; cursor=0; fits=0; hypotheses=0; ranking=0; timings=[]; accepted_total=0
    ...
    for index,scan in enumerate(scans):
        if index%50==0:
            print(json.dumps({'phase':'independent_scan_review','completed':index,'total':len(scans)}),file=sys.stderr,flush=True)
        binding=next(b for b in bindings if b[1]==scan['epoch'])
        assert scan['attempt'] == index+1 and scan['epoch'] == binding[1]
        assert scan['scan_iq_offset'] == index*14000 and scan['scan_iq_samples'] == 14000
        ...
        np.testing.assert_array_equal(scan['peaks'],sorted_peak_oracle(grids[index],64))
        assert order['candidate_budget']==64 and order['ranking_fft']==4096
        powers=[]; ref=bases[0][:,0]
        for epoch,frequency,score in scan['peaks']:
            assert int(grids[index,frequency,epoch]) == score
            x=cut[epoch+22:epoch+3322].astype(float); z=x[:,0]+1j*x[:,1]
            powers.append(float(max(abs(np.fft.fft(z*np.conj(ref),4096))**2)/
        ...
        assert order['selected_rank'] == int(np.argmax(powers)); ranking+=len(powers)
        ...
        for row in rr:
            ...
            x=owned[cursor:cursor+count]; assert len(x)==count; cursor+=count
            retain_view(row['first'],x,row['copied'] if row['kind']==1 else row['source'],row['recorded_ns'])
            if row['kind']==1:
                np.testing.assert_allclose(resolved['hypotheses'],oracle(refs[0,:,:2],x,seed['starts']),rtol=2e-12,atol=2e-8)
                hypotheses+=17
                continue
        ...
        startup_jobs+=startup['initial_jobs_checked'];startup_forecasts+=startup['causal_forecasts_checked']
        ...
        if terminal['status']==1:
            ...
            proposals=[r for r in rr if r['kind']==4]; assert 1<=len(proposals)<=2
            handoff=proposals[-1]
        ...
        timings.append(dict(attempt=scan['attempt'],worker_status=terminal['status'],
            accepted=len(accepted),past=len(past),
    ...
    overlap_samples=0; active=[]
    for start,iq in sorted(cuts,key=lambda cut:cut[0]):
        active=[(a,x) for a,x in active if a+len(x)>start]
        for a,x in active:
            end=min(start+len(iq),a+len(x)); count=end-start
            np.testing.assert_array_equal(iq[:count],x[start-a:start-a+count]); overlap_samples+=count
        active.append((start,iq))
    heads=epochs['native_results']; supported=sum(e['supported'] for e in epochs['episodes'])
    assert heads==status['native_results']
    return dict(status='pass',scope='selected_iq_acquisition_and_passive_observer_arithmetic_and_source_counters',
        passive_observer=observer,clean_loss=loss,child_disposition=status['status'],probe_sha256=op['payload_sha256']['probe'],
        run_succeeded=status['status']==0,post_capture_rf_settings_checked='rf_after' in op,
        full_iq_retained=False,live_tracking_qualified=False,native_numerical_review_required=bool(heads),
        rate=rate,exported_samples=pair(final,4),returned_samples=len(blocks)*16384,
        rf_seconds=pair(final,4)/2500000,coarse_origin=origin,epoch_bindings=bindings,epoch_review=epochs,
        cdc_drops=0,pacer_drops=0,attempts=status['attempts'],complete_searches=len(scans),
        grid_values_checked=int(grids.size),ordering_scores_checked=ranking,resolver_hypotheses_checked=hypotheses,
        moment_and_dense_fits_checked=fits,accepted_past_observations=accepted_total,
        retained_scan_samples=len(searched),retained_worker_samples=len(owned),overlapping_samples_checked=overlap_samples,
        native_results=heads,reported_native_support=supported,max_refill_gap_ns=status['max_refill_gap_ns'],timings=timings,
        rotation_scalar_oracle_checks=rotation_checks,
        startup_jobs_checked=startup_jobs,startup_forecasts_checked=startup_forecasts,
        startup_reviewer_sha256=hashlib.sha256(Path(review_glrt_cpu_startup.__file__).read_bytes()).hexdigest(),
        rotation_module_sha256=hashlib.sha256((Path(__file__).parent/'validate_native_admission.py').read_bytes()).hexdigest(),
...
if __name__=='__main__':
    root=Path(sys.argv[1]); result=review(root)
