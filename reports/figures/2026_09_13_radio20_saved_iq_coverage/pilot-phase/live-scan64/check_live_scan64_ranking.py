...
out = BASE / "live-scan64-ranking-v1"
...
for case in manifest["cases"]:
    ...
    iq = np.ascontiguousarray(raw[:14000])
    ...
    scores = np.full(64, 999.0, dtype=np.float64)
    selected = c.c_uint(999)
    handle = lib.live_new(
        refs.ctypes.data, bank.ctypes.data, os.fsencode(directory), c.byref(ports), 60000000
    )
    try:
        assert (
            lib.live_set_dwell(handle, b"1536-selected-observer3-scan64", (c.c_uint64 * 4)()) == 0
        )
        assert (
            lib.live_rank(
                handle,
        ...
        np.testing.assert_allclose(
            scores, [r["rank_power"] for r in rows[:-1]], rtol=2e-12, atol=2e-15
        )
...
result = dict(
    ...
    scores_checked=1664,
    new_rf_samples=0,
    source_sha256=digest(fixture.ROOT / "tools/glrt_cpu_live_probe.c")
    if hasattr(fixture, "ROOT")
    else digest(
        Path("/home/mouse9911/gits/plutosdr-fw-radio20-tracking/tools/glrt_cpu_live_probe.c")
    ),
