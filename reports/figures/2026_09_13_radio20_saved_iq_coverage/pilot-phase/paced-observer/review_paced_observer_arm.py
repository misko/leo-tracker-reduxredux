...
def main():
    ...
    assert digest(source) == build["payloads"]["iq"]
    iq = source.read_bytes()
    cases = []
    for case in op["cases"]:
        ...
        host = [
            json.loads(line)
            for line in (BASE / "paced-observer-host-v1-results" / case["journal"])
            .read_text()
            .splitlines()
        ]
        expected = host.pop()
        assert len(rows) == len(host) == 200
        for key in ("offset", "measurements", "accepted", "terminal_status"):
        ...
        assert retained.read_bytes() == b"".join(
    ...
    result = dict(
        status="pass",
        cases=cases,
        verified_measurements=600,
        verified_retained_iq_bytes=7920000,
        producer_paced=True,
        live_RF_DMA_tested=False,
