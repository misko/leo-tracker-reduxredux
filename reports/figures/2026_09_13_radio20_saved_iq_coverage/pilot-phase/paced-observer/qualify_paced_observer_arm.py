...
def main():
    output = EVIDENCE / 'paced-observer-arm-v1-results'; output.mkdir(exist_ok=False)
    build = json.loads((EVIDENCE / 'paced-observer-build-v1.json').read_text())
    files = dict(probe='paced-observer-arm-v1', refs='direct-references.ci16',
    ...
    try:
        lease = authority.claim(('radio_pluto_5d4d',), task_id='paced-observer-arm-v1', task_kind=CaptureTaskKind.QUALIFICATION)
    ...
    with lease, acquire_radio_lock(ENDPOINT[0]):
        ...
        try:
            ...
            for offset in range(3):
                completed = run(shlex.join([remote+'/probe', remote+'/iq', remote+'/refs', remote+'/training', str(offset), remote+'/retained']))
                name = f'o{offset}.jsonl'
                (output / name).write_bytes(completed.stdout)
                completed.check_returncode()
                retained = run('cat ' + shlex.quote(remote+'/retained'))
                retained.check_returncode()
                assert len(retained.stdout)==200*3300*4
                (output / f'o{offset}.ci16').write_bytes(retained.stdout)
                evidence['cases'].append(dict(offset=offset,exit_code=completed.returncode,journal=name,
                    sha256=hashlib.sha256(completed.stdout).hexdigest(),
                    retained_sha256=hashlib.sha256(retained.stdout).hexdigest(),stderr=completed.stderr.decode()))
                save()
            terminal = True
            ...
            try:
                ...
                if terminal:
                    run('rm -f ' + shlex.join([remote+'/'+name for name in payload]+[remote+'/retained']) +
