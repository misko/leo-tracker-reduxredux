...
def main():
    output = EVIDENCE / "paced-observer-arm-v1-results"
            ...
            for offset in range(3):
                ...
                evidence["cases"].append(
                    dict(
                        ...
                        retained_sha256=hashlib.sha256(retained.stdout).hexdigest(),
            ...
            try:
                ...
                if terminal:
                    run(
                        "rm -f "
                        + shlex.join(
                            [remote + "/" + name for name in payload] + [remote + "/retained"]
                        )
                        + " && rmdir "
                        + shlex.quote(remote)
                    ).check_returncode()
