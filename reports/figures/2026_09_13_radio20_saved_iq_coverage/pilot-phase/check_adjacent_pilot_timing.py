...
from check_adjacent_pilot_phase import (
    BASE,
    ROOT,
    RATE,
    TRAIN,
...
def main():
    ...
    result = dict(
        scope="adjacent_pilot_localization_and_training_only_timing_forecast",
        new_rf_samples=0,
        ...
        prior_sha256=digest(BASE / "adjacent-pilot-phase-v1.json"),
        cases=[],
    )
    for old in prior["cases"]:
        ...
        journal = [
            json.loads(line)
            for line in (ROOT / case["label"] / "stdout.jsonl").read_text().splitlines()
        ]
        ...
        for at, phase in old["jobs"]:
            ...
            for quarter in range(-8, 9):
                ...
                spectrum = zoom_fft(
                    z * ref.conj(), [grid[0], grid[-1]], len(grid), fs=RATE, endpoint=True
                )
                k = int(np.argmax(abs(spectrum)))
                power = float(abs(spectrum[k]) ** 2 / (np.vdot(ref, ref).real * np.vdot(z, z).real))
                candidate = dict(
                    start=start,
                    reference_phase=refphase,
                    offset_quarters=quarter,
                    cfo_hz=float(grid[k]),
                    power=power,
                )
        ...
        for (at, phase), time in zip(jobs, times):
            ...
            z = iq[at : at + 3300] * np.exp(
                -2j * np.pi * cycles(time + np.arange(3300) / RATE, carrier)
            )
        ...
        row = dict(
            ...
            localized=localized,
            predicted_jobs=jobs,
            trained_timing_samples_per_frame=float(timing[0]),
            trained_timing_offset=float(timing[1]),
            trained_drift_hz_per_second=float(carrier[0]),
            trained_cfo_hz=float(carrier[1]),
            forecast_matched_power=[float(abs(v) ** 2) for v in values],
            heldout_position_error_samples=(position[TRAIN:] - predicted[TRAIN:]).tolist(),
            complex_correlations=[[float(v.real), float(v.imag)] for v in values],
            **checked,
        )
        ...
        print(
            case["label"],
            "localized median",
            np.median([r["power"] for r in localized]),
            "held forecast median",
            np.median(row["forecast_matched_power"][TRAIN:]),
            "phase RMS",
