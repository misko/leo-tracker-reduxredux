...
def predict(time, correlation):
    ...
    return dict(
        train_residual_cfo_hz=float(fitted[0] / (2 * np.pi)),
        phase_error_rad=errors.tolist(),
        heldout_phase_rms_rad=float(np.sqrt(np.mean(errors[4:8] ** 2))),
...
def main():
    ...
    ideal = 0.2 * np.exp(1j * (0.6 + 2 * np.pi * 42 * time))
    check = predict(time, ideal)
    assert abs(check["train_residual_cfo_hz"] - 42) < 1e-10
    assert (
        check["heldout_phase_rms_rad"] < 1e-12 and abs(check["heldout_coherent_gain"] - 4) < 1e-12
    )
    opposite = ideal.copy()
    opposite[4:] *= np.array([1, -1, 1, -1])
    ...
    for case in manifest["cases"]:
        ...
        past = [r for r in rows if r["kind"] == 3][:8]
        assert len(past) == 8
        first = past[0]["first"]
        times = []
        correlation = []
        for r in past:
            at = r["first"]
            refrow = refs[r["reference_phase"]]
            ref = refrow[:, 0].astype(float) + 1j * refrow[:, 1]
            raw = iq[at : at + 3300].astype(float)
            z = raw[:, 0] + 1j * raw[:, 1]
            ...
            correlation.append(
                np.vdot(ref, z) / np.sqrt(np.vdot(ref, ref).real * np.vdot(z, z).real)
            )
        ...
        row = dict(
            ...
            frame_numbers=[r["frame"] for r in past],
            source_first=[r["first"] for r in past],
            complex_correlations=[[float(a.real), float(a.imag)] for a in correlation],
            **checked,
        )
        result["cases"].append(row)
        print(
            case["label"],
            checked["heldout_phase_rms_rad"],
            checked["heldout_coherent_gain"],
            flush=True,
        )
    with (BASE / "heldout-pilot-phase-v1.json").open("x") as f:
