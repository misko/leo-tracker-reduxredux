...
def phase_cycles(time, carrier):
    ...
    return intercept * time + 0.5 * slope * time**2


def main():
    times = np.arange(8) * 0.012
    expected = np.array([-4000.0, 460000.0])
    ...
    phase = 0.6 + 2 * np.pi * (phase_cycles(times, expected) + 12 * times)
    corrected = 0.2 * np.exp(1j * (phase - 2 * np.pi * phase_cycles(times, carrier)))
    ...
    result = dict(
        scope="four_training_local_cfo_linear_drift_then_heldout_phase",
        new_rf_samples=0,
    ...
    for case in manifest["cases"]:
        ...
        for row, start_time in zip(past, times):
            ...
            z = iq[row["first"] : row["first"] + 3300] * np.exp(
                -2j * np.pi * phase_cycles(t, carrier)
            )
            correlations.append(
