...
assert {(c['budget'],c['repeat']) for c in operator['cases']}=={(b,r) for b in (8,64) for r in range(3)}
for run in operator['cases']:
    assert run['exit_code']==0 and run['stderr'] in ('',advisory)
    path=root/run['journal'];assert digest(path)==run['sha256']
    rows=[json.loads(line) for line in path.read_text().splitlines()];budget=run['budget']
    ...
    timing=rows[-1];assert timing['budget']==budget
    assert timing['winner']==next(b for b in case['budgets'] if b['budget']==budget)['single_winner']
    assert timing['total_ns']==sum(timing[k] for k in ('grid_and_legacy_select_ns','bounded_select_ns','ranking_ns'))
    assert 0<timing['total_ns']<10_000_000_000
    timings.append(timing)
result=dict(status='pass',ranked_peaks_verified=216,operator_sha256=digest(root/'operator.json'),
    reviewer_sha256=digest(Path(__file__)),new_rf_samples=0,live_freshness_qualified=False,timings=timings)
