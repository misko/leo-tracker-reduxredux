...
assert op['new_rf_samples']==op['native_jobs']==0 and op['payload_sha256']==manifest['payloads']
...
assert {(c['fft'],c['repeat']) for c in op['cases']}=={(n,r) for n in (4096,8192,16384) for r in range(3)}
timings=[]
for run in op['cases']:
    ...
    expected=next(c for c in manifest['cases'] if c['label']=='positive' and c['number']==12 and c['fft']==run['fft'])
    ...
    for row,ref in zip(rows[:-1],prior[:-1]):
        assert (row['epoch'],row['frequency'],row['score'])==(ref['epoch'],ref['frequency'],ref['score'])
        np.testing.assert_allclose(row['rank_power'],ref['rank_power'],rtol=2e-12,atol=2e-14)
    timing=rows[-1];assert timing['budget']==64 and timing['winner']==expected['winner']
...
result=dict(status='pass',ranked_peaks_verified=576,timings=timings,new_rf_samples=0,
    live_freshness_qualified=False,operator_sha256=digest(root/'operator.json'),
    host_result_sha256=digest(BASE/'short-fft-scan-host-v1-results/result.json'),reviewer_sha256=digest(Path(__file__)))
