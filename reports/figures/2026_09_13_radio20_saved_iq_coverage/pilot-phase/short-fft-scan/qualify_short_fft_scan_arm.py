...
def main():
    ...
    build=json.loads((EVIDENCE/'short-fft-scan-host-v1-results/result.json').read_text())
    files=dict(probe='short-fft-scan-arm-v1',bank='coarse-bank.ci16',refs='direct-references.ci16',iq='combined-pilot-controls-v1/positive-12/iq.ci16')
    ...
    for path,sha in build['source_sha256'].items():assert hashlib.sha256(Path(path).read_bytes()).hexdigest()==sha
    ...
    remote='/tmp/short-fft-scan-'+uuid.uuid4().hex
    ...
    try:lease=authority.claim(('radio_pluto_5d4d',),task_id='short-fft-scan-arm-v1',task_kind=CaptureTaskKind.QUALIFICATION)
    ...
    with lease,acquire_radio_lock(ENDPOINT[0]):
        ...
        try:
            ...
            for repeat in range(3):
                for fft_size in (4096,8192,16384):
                    budget=64
                    completed=run(shlex.join([remote+'/probe',remote+'/bank',remote+'/iq',remote+'/refs',str(budget),str(fft_size)]))
                    name=f'fft{fft_size}-r{repeat}.jsonl';(output/name).write_bytes(completed.stdout)
                    evidence['cases'].append(dict(budget=budget,fft=fft_size,repeat=repeat,exit_code=completed.returncode,journal=name,
