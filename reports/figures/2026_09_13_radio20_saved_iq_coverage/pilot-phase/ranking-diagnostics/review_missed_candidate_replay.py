...
for r in refs.astype(float):
    ...
    bases.append(
        np.column_stack(
            (ref, -der, 1j * np.pi * 1000 / 2500000 * (2 * np.arange(3300) - 3299) * ref)
        )
    )
...
def check(case, rows):
    ...
    iq = np.fromfile(directory / "iq.ci16", dtype="<i2").reshape(-1, 2)
    ...
    np.testing.assert_allclose(
        [resolved[k] for k in ("best_shift", "cfo_hz", "coherence")], best, rtol=2e-12, atol=2e-8
    )
    past = [r for r in rows if r["kind"] == 3]
    for row in past:
        first = row["first"]
        assert 0 <= first and first + 3300 <= len(iq)
        z = rotated(iq[first : first + 3300], 0, row["phase_step"])
    ...
    return dict(
        label=case["label"],
        resolver_hypotheses=17,
        moment_dense_fits=len(past),
        accepted=sum(r["accepted"] for r in past),
        startup=startup,
        maximum_coherence=max(r["coherence"] for r in past),
        rejection_counts={
            str(k): sum(r["rejection"] == k for r in past)
            for k in set(r["rejection"] for r in past)
        },
    )
...
for case in manifest["cases"]:
    ...
    assert (
        digest(path) == case["source_sha256"]
        and digest(directory / "iq.ci16") == case["input_sha256"]
    )
    assert digest(directory / "stdout.jsonl") == case["journal_sha256"]
    source = np.memmap(path, mode="r", dtype="<i2").reshape(-1, 2)
    np.testing.assert_array_equal(
        np.fromfile(directory / "iq.ci16", dtype="<i2").reshape(-1, 2),
    ...
    if case["label"] in ("candidate-0", "positive", "control"):
        for field in ("coherence", "cfo_hz"):
            ...
            try:
                ...
                raise AssertionError("mutated estimate accepted")
result = dict(
    status="pass",
    scope="independent_frozen_candidate_refinement_and_moments",
    cases=results,
