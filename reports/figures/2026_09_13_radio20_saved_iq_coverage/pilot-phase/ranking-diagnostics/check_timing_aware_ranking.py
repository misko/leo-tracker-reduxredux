...
out = BASE / "timing-aware-ranking-v1"
...
result = dict(
    ...
    radii=[0, 1, 2, 4, 8],
    worker_binary_sha256=digest(benchmark),
...
for case, source in zip(ranking["cases"], inputs["cases"]):
    ...
    assert (case["label"], case["number"]) == (source["label"], source["number"]) and digest(
        path
    ) == source["input_sha256"]
    ...
    for peak in case["scores"]:
        ...
        for shift in range(-8, 9):
            ...
            powers.append(
                float(
                    max(abs(np.fft.fft(z * ref.conj(), 4096)) ** 2)
                    / max(float(energy * np.vdot(z, z).real), 1)
                )
            )
    ...
    np.testing.assert_allclose(
        scores[:64, 8], [r["rank_power"] for r in rows[:-1]], rtol=2e-12, atol=2e-14
    )
    for budget in (64, 256):
        for radius in result["radii"]:
            block = scores[:budget, 8 - radius : 9 + radius]
            index, column = np.unravel_index(np.argmax(block), block.shape)
            ...
            comparisons.append(
                dict(
                    budget=budget,
                    radius=radius,
                    ranking_shift=int(column) - radius,
                    power=float(block[index, column]),
                    **runs[int(index)],
                )
            )
    result["cases"].append(
    ...
    print(
        label,
        [(r["budget"], r["radius"], r["winner"], r["accepted"]) for r in comparisons],
        flush=True,
    )
