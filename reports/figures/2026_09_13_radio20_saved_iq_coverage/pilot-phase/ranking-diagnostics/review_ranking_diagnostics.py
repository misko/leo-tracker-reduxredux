    ...
    for case, proposal_case in zip(manifest["cases"], proposals, strict=True):
        ...
        if mode == "repeat-phase":
            ...
            np.testing.assert_array_equal(
                np.fromfile(path, dtype="<i2").reshape(-1, 2), raw[offset : offset + 447851]
            )
        ...
        for choice in choices:
            ...
            journal = root / (
                f"{label}-candidate{choice['winner']}.jsonl"
                if mode == "timing-aware"
                else f"{label}/worker.jsonl"
            )
...
result = dict(
    status="pass",
    ranking_winners_checked=rank_checks,
