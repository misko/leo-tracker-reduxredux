...
out = BASE / "repeat-phase-ranking-v1"
...
result = dict(
    scope="repeat_phase_ranking_frozen_worker_replay",
    new_rf_samples=0,
    acceptance_gates_changed=False,
    live_tracking_qualified=False,
    production_origin_contract_implemented=False,
    arm_cost_measured=False,
    script_sha256=digest(Path(__file__)),
    worker_sha256=digest(binary),
    cases=[],
)
for case, original in zip(manifest["cases"], prior["cases"]):
    ...
    offset = case["number"] * 447851
    assert (
        hashlib.sha256(raw[offset : offset + 447851].tobytes()).hexdigest()
        == original["input_sha256"]
    )
    iq = raw[offset : offset + 20000].astype(float)
    z = iq[:, 0] + 1j * iq[:, 1]
    powers = []
    bins = []
    for peak in case["scores"][:64]:
        ...
        for shift in (0, 3333, 6667):
            ...
            spectrum = abs(np.fft.fft(x * ref.conj(), 4096)) ** 2 / max(
                float(energy * np.vdot(x, x).real), 1
            )
            ...
            freq.append(index)
            direct = abs(
                np.vdot(ref, x * np.exp(-2j * np.pi * index * np.arange(3300) / 4096))
            ) ** 2 / max(float(energy * np.vdot(x, x).real), 1)
            np.testing.assert_allclose(power, direct, rtol=2e-10, atol=2e-14)
        powers.append(row)
        bins.append(freq)
    scores = np.array(powers)
    index, repeat = np.unravel_index(np.argmax(scores), scores.shape)
    peak = case["scores"][index]
    shift = (0, 3333, 6667)[repeat]
    selected = raw[offset + shift : offset + shift + 447851]
    assert len(selected) == 447851
    ...
    result["cases"].append(
        dict(
            ...
            winner=int(index) + 1,
            proposal=peak,
            power=float(scores[index, repeat]),
            powers=powers,
            bins=bins,
            terminal=rows[-1],
            accepted=sum(r["accepted"] for r in past),
            selected_iq_sha256=digest(directory / "iq.ci16"),
            journal_sha256=digest(directory / "worker.jsonl"),
        )
    )
    print(
        label,
