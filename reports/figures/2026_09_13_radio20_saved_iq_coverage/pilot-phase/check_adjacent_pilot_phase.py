...
def forecast(times, frequency):
    return np.polyfit(np.asarray(times[:TRAIN]) + 1649.5 / RATE, np.asarray(frequency[:TRAIN]), 1)


def cycles(time, carrier):
    return carrier[1] * time + 0.5 * carrier[0] * time**2
...
def phase_review(times, values):
    ...
    return dict(
        residual_frequency_hz=float(fitted[0] / (2 * np.pi)),
...
def main():
    ...
    known = np.array([-4000.0, 460000.0])
    ...
    signal = 0.2 * np.exp(1j * (0.6 + 2 * np.pi * (cycles(times, known) + 12 * times)))
    ...
    result = dict(
        scope="adjacent_64_pilots_first_32_training_next_32_heldout",
        sample_rate=RATE,
        new_rf_samples=0,
    ...
    for case in manifest["cases"]:
        ...
        for at, phase in jobs:
            ...
            powers.append(
                float(abs(spectrum[best]) ** 2 / (np.vdot(ref, ref).real * np.vdot(z, z).real))
            )
        ...
        row = dict(
            label=case["label"],
            jobs=jobs,
            local_peak_cfo_hz=frequencies,
            local_peak_power=powers,
            trained_drift_hz_per_second=float(model[0]),
            trained_cfo_hz=float(model[1]),
            forecast_matched_power=(abs(values) ** 2).tolist(),
            complex_correlations=[[float(a.real), float(a.imag)] for a in values],
            **checked,
        )
        ...
        print(
            ...
            round(model[0], 2),
            "phase RMS",
            round(checked["heldout_phase_rms_rad"], 4),
            "gain",
            round(checked["heldout_coherent_gain"], 4),
            "power median",
            round(float(np.median(powers)), 5),
            flush=True,
        )
