...
def main():
    ...
    def evaluate(iq, frame, at, phase, frequency):
        ...
        cut = np.ascontiguousarray(iq[at : at + 3300])
        ref = np.ascontiguousarray(refs[phase])
        data = Moments()
        out = Estimate()
        assert (
            collect(
                cut.ctypes.data_as(ptr), ref.ctypes.data_as(ptr), 3300, at, 0, step, c.byref(data)
            )
            == 0
        )
        ...
        np.testing.assert_allclose(
            [out.delay * 1e6, out.residual / 1000],
        ...
        np.testing.assert_allclose(
            out.cfo,
            step * RATE / 2**32 + np.clip(correction[1], -0.25, 0.25) * 1000,
            rtol=2e-12,
            atol=2e-8,
        )
        checks += 1
    ...
    result = dict(
        scope="offline_C_gates_on_localized_adjacent_pilots_then_training_only_forecasts",
        new_rf_samples=0,
        ...
        localization_sha256=digest(source),
        input_manifest_sha256=digest(manifest_path),
        c_source_sha256={str(p.relative_to(FW)): digest(p) for p in sources},
        cases=[],
    )
    for old in prior["cases"]:
        ...
        local = [
            evaluate(iq, frame, row["start"], row["reference_phase"], row["cfo_hz"])
        ...
        row = dict(
            label=case["label"],
            localized=local,
            accepted_training_frames=[r["frame"] for r in accepted],
        )
        if len(accepted) >= 4:
            ...
            positions = np.array(
                [r["start"] + r["reference_phase"] / 4 + r["delay_s"] * RATE for r in accepted]
            )
        ...
        row.update(
            heldout_forecast=future,
            localized_accepted=sum(r["rejection"] == 0 for r in local),
        ...
        print(
            case["label"],
            "localized accepted",
            row["localized_accepted"],
            "/64",
            "train",
            len(accepted),
            "/32",
            "forecast",
            row["forecast_accepted"],
            "/",
            len(future),
            flush=True,
        )
