...
def main():
    ...
    build=json.loads((EVIDENCE/'bounded-scan-build-v1.json').read_text())
    files=dict(probe='bounded-scan-arm-v1',bank='coarse-bank.ci16',refs='direct-references.ci16',iq='combined-pilot-controls-v1/positive-12/iq.ci16')
    ...
    remote='/tmp/bounded-scan-'+uuid.uuid4().hex
    ...
    try:lease=authority.claim(('radio_pluto_5d4d',),task_id='bounded-scan-arm-v1',task_kind=CaptureTaskKind.QUALIFICATION)
