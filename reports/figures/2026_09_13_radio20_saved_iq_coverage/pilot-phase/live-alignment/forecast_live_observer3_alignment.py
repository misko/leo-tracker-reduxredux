...
def main():
    source = BASE / "live-observer3-alignment-v1.json"
    ...
    result = dict(
        scope="first_four_alignment_fit_next_four_prediction",
        new_rf_samples=0,
        acceptance_gates_changed=False,
        production_acceptance_evaluated=False,
        diagnostic_rejected_estimates_used_for_training=True,
    ...
    for case in prior["cases"]:
        ...
        positions = np.array(
            [
                r["original_first"]
                - origin
                + (r["original_phase"] + r["local_search"]["offset_quarters"]) / 4
                - r["frame"] * RATE / 750
                for r in rows
            ]
        )
        ...
        changed[4:] += 100
        freq_changed = frequencies.copy()
        freq_changed[4:] += 10000
        alternate = fit(frames, changed, freq_changed)
        np.testing.assert_array_equal(alternate[0], timing)
        np.testing.assert_array_equal(alternate[1], carrier)
        measured = []
        for r in rows[4:]:
            frame = r["frame"]
            relative = (
                frame * RATE / 750 + np.polyval(timing, frame) - (r["original_first"] - origin)
            )
            whole, phase = divmod(round(relative * 4), 4)
            if not 0 <= 32 - whole or not 3268 - whole <= 3300:
                measured.append(dict(frame=frame, status="reference_extent_exceeded"))
                continue
            source_row = next(
                j
                for j in journal
                if j["kind"] == 3 and j["attempt"] == case["attempt"] and j["frame"] == frame
            )
            ...
            measured.append(
                dict(
                    frame=frame,
                    status="evaluated",
                    relative_start_quarters=whole * 4 + phase,
                    forecast_cfo_hz=frequency,
                    power=power,
                    diagnostic_local_maximum=r["local_search"]["power"],
                )
            )
        result["cases"].append(
            dict(
                attempt=case["attempt"],
                timing_samples_per_frame=float(timing[0]),
                timing_intercept_samples=float(timing[1]),
                cfo_hz_per_frame=float(carrier[0]),
                heldout=measured,
            )
        )
        print(
            case["attempt"],
            round(timing[0], 5),
            [(r["frame"], round(r.get("power", -1), 4)) for r in measured],
            flush=True,
        )
