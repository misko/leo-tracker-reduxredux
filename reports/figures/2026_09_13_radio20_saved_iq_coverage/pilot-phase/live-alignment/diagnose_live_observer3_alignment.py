...
def main():
    ...
    output = dict(
        scope="fixed_interior_alignment_search",
        sample_rate=RATE,
        common_samples=3236,
        timing_search_quarters=[-64, 64],
        frequency_half_width_hz=1000,
        frequency_grid_hz=5,
        new_rf_samples=0,
    ...
    for attempt in range(1, 7):
        ...
        for row in [r for r in local if r["kind"] == 3]:
            ...
            for quarter in range(-64, 65):
                ...
                candidate = dict(
                    ...
                    cfo_hz=float(grid[k]),
                    power=power,
                    frequency_boundary=k in (0, 400),
                    timing_boundary=abs(quarter) == 64,
                )
                if quarter == 0:
                    fixed = candidate
            ...
            measurements.append(
                dict(
                    ...
                    original_phase=row["reference_phase"],
                    original_c_coherence=row["coherence"],
                    fixed_timing_frequency_search=fixed,
                    local_search=best,
                )
            )
        ...
        print(
            ...
            [
                (
                    r["frame"],
                    round(r["local_search"]["power"], 4),
                    r["local_search"]["offset_quarters"],
                )
