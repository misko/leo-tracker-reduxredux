...
def review(root, rows=None, expected_spacing=9):
    assert expected_spacing in (3, 9)
    root = Path(root)
    if rows is None:
        rows = [json.loads(line) for line in (root / "observer.jsonl").read_text().splitlines()]
    ...
    refs = (
        np.fromfile(BASE / "direct-references.ci16", dtype="<i2")
        .reshape(4, 3300, 4)
        .astype(np.int64)
    )
    assert (
        hashlib.sha256(refs.astype("<i2").tobytes()).hexdigest()
    ...
    for r in refs:
        ...
        bases.append(
            np.column_stack(
                (ref, -der, 1j * np.pi * 1000 / RATE * (2 * np.arange(3300) - 3299) * ref)
            )
        )
    at = 0
    offset = 0
    ...
    while at < len(rows):
        ...
        assert start.get("frame_spacing", 9) == expected_spacing
        assert (
            start["epoch"] == seed["epoch"]
            and start["first_frame"] == seed["last_seen"] + expected_spacing
        )
        assert (
            start["maximum_measurements"] == 200
            and start["deadline_ns"] - start["started_ns"] == 3000000000
        )
        assert start["retained_total"] * 3300 == offset
        measurements = []
        while at < len(rows) and rows[at]["kind"] == "measurement":
            measurements.append(rows[at])
            at += 1
        terminal = rows[at]
        at += 1
        ...
        history = sorted(seed["history"][: seed["count"]])
        last_seen = seed["last_seen"]
        last_support = seed["last_supported"]
        for index, r in enumerate(measurements):
            assert all(r[k] == start[k] for k in ("attempt", "episode", "epoch"))
            assert (
                r["rate"] == RATE
                and r["sequence"] == index
                and r["frame"] == start["first_frame"] + expected_spacing * index
            )
            ...
            delay, frequency = np.array([r["frame"] - last_support, 1.0]) @ fit
            ...
            step = r["phase_step"]
            hz = (step if step < 2**31 else step - 2**32) * RATE / 2**32
            assert abs(hz - frequency) <= RATE / 2**32 + 1e-7
            if index == 0:
                assert start["source_limit"] == r["first"] + 7500000
            source = r["source"]
            assert (
                source["epoch"] == start["epoch"] and source["valid"] == 1 and source["closed"] == 0
            )
            assert (
                source["first"] <= r["first"]
                and r["first"] + 3300
                <= source["end"]
                <= source["source_now"]
                <= start["source_limit"]
            ...
            if np.any(cut):
                ...
                expected = [
                    np.clip(correction[0], -0.25, 0.25) * 1e-6,
                    np.clip(correction[1], -0.25, 0.25) * 1000,
                    coherence,
                    improved,
                ]
                actual = [
                    r["delay_correction_s"],
                    r["residual_cfo_hz"],
                    r["coherence"],
                    r["linearized_coherence"],
                ]
            ...
            if index < committed:
                ...
                if r["accepted"]:
                    ...
                    correction = (
                        ...
                        - nominal
                        + phase / 4
                        + r["delay_correction_s"] * RATE
                        - rem / 750
                    )
                    history.append([r["frame"], correction, r["cfo_hz"]])
                    history = history[-96:]
    ...
    return dict(
        status="pass",
        scope="independent_passive_observer_numerics_and_causality",
        native_tracking_qualified=False,
        episodes=episodes,
        moments_and_dense_fits_checked=checked,
        committed_supported=supported,
        iq_sha256=hashlib.sha256(raw).hexdigest(),
    )
...
if __name__ == "__main__":
    ...
    for p in sys.argv[1:]:
        ...
        for field in ("first", "phase_step", "coherence", "iq_offset"):
            ...
            changed[first][field] += 1
            try:
                review(p, changed)
            except (AssertionError, ValueError):
                mutations += 1
            else:
                raise AssertionError("undetected mutation " + field)
    result = dict(
        rotation_scalar_checks=rotation_checks, mutations_rejected=mutations, results=results
    )
