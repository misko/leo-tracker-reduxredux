...
for case in manifest["cases"]:
    ...
    path = root / "grids.u32"
    assert hashlib.sha256(path.read_bytes()).hexdigest() == op["artifacts"][path.name]["sha256"]
    ...
    nearby = [
        dict(rank=n + 1, epoch=e, frequency=f, score=score, delta=e - epoch)
        for n, (e, f, score) in enumerate(peaks)
        if distance(e) <= 8
    ]
    exact = grid[:, epoch]
    result = dict(
        ...
        exhaustive_epoch=epoch,
        exhaustive_power=case["group_maxima"][0]["power"],
        coarse_proposal_count=len(peaks),
        first_resolvable_candidate=nearby[0] if nearby else None,
...
output = dict(
    scope="exhaustive_delay_to_original_coarse_proposal_association",
    cases=results,
    new_rf_samples=0,
    acceptance_gates_changed=False,
    worker_handoffs_measured=False,
    script_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
)
with (BASE / "confirmed-rx-coarse-recall-v1.json").open("x") as f:
