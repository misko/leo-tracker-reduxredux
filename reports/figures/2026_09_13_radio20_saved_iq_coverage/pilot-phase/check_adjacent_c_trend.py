...
def main():
    ...
    names = ['glrt_tracking_iq.c', 'glrt_native_solver.c', 'glrt_native_trend.c',
             'glrt_native_schedule.c', 'glrt_tracking_schedule.c']
    binary = BASE / 'adjacent-c-trend-v1.so'
    assert not binary.exists()
    subprocess.run(['cc', '-std=c99', '-O2', '-Wall', '-Wextra', '-Werror', '-shared', '-fPIC',
                    *[str(FW / 'tools' / name) for name in names], '-lm', '-o', str(binary)], check=True)
    lib = c.CDLL(str(binary)); ptr = c.POINTER(c.c_int16)
    ...
    lib.glrt_tracking_iq_moments_2500000.argtypes = [ptr, ptr, c.c_size_t, c.c_uint64,
    ...
    result = dict(scope='production_C_trend_frozen_and_feedback_adjacent_replay', new_rf_samples=0,
                  acceptance_gates_changed=False, native_tracking_qualified=False,
                  training_localization_is_diagnostic=True, source_sha256=digest(Path(__file__)),
                  prior_sha256=digest(source), binary_sha256=digest(binary),
                  c_source_sha256={name: digest(FW / 'tools' / name) for name in names}, cases=[])
    for old in prior['cases']:
        ...
        for r in old['localized'][:32]:
            ...
            rc = lib.glrt_tracking_trend_observe(c.byref(training), 1, r['frame'], r['start'],
                                                r['reference_phase'], c.byref(estimate))
            assert rc == int(r['rejection'] == 0)
        for feedback in (False, True):
            trend = TrackingTrend.from_buffer_copy(bytes(training)); rows = []
            for frame in range(32, 64):
                batch = TrackingBatch(); slope = c.c_double(); job = Job()
                before = bytes(trend)
                rc = lib.glrt_tracking_trend_batch(c.byref(trend), frame, 1, frame, 0,
                                                  c.byref(batch), c.byref(slope))
                assert bytes(trend) == before
                if rc:
                    rows.append(dict(frame=frame, batch_result=rc)); break
                ...
                cut = np.ascontiguousarray(iq[job.start:job.start + 3300]); ref = np.ascontiguousarray(refs[job.reference_phase])
                data = Moments(); estimate = Estimate()
                assert lib.glrt_tracking_iq_moments_2500000(cut.ctypes.data_as(ptr), ref.ctypes.data_as(ptr),
                    3300, job.start, 0, job.phase_step, c.byref(data)) == 0
                assert lib.glrt_tracking_solve(RATE, job.reference_phase, c.byref(data), c.byref(estimate)) == 0
                z = rotated(cut, 0, job.phase_step)
                assert list(data.words) == list(moments(z, ref.astype(np.int64)).words)
                raw = ref.astype(float); pilot = raw[:, 0] + 1j * raw[:, 1]
                basis = np.column_stack((pilot, -raw[:, 2] - 1j * raw[:, 3],
                ...
                assert estimate.rejection == ((32 if np.any(abs(correction) >= .25) else 0) | (64 if coherence < .05 else 0))
                if feedback:
                    rc = lib.glrt_tracking_trend_observe(c.byref(trend), 1, frame, job.start,
                        job.reference_phase, c.byref(estimate))
                    assert rc == int(estimate.rejection == 0)
                rows.append(dict(frame=frame, start=job.start, phase=job.reference_phase,
                    step=job.phase_step, cfo_rate_hz_s=slope.value, coherence=estimate.coherence,
                    delay_s=estimate.delay, cfo_hz=estimate.cfo, rejection=estimate.rejection,
                    moments=list(data.words)))
            row = dict(label=case['label'], feedback=feedback, training_count=training.history.count,
                       terminal_history_count=trend.history.count, measurements=rows,
                       accepted=sum(r.get('rejection', -1)==0 for r in rows))
            result['cases'].append(row)
            print(case['label'], 'feedback', feedback, 'accepted', row['accepted'], 'records', len(rows), flush=True)
    with (BASE / 'adjacent-c-trend-v1.json').open('x') as stream:
