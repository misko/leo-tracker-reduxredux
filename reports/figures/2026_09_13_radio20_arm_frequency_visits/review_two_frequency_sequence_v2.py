...
def review(root,text=None):
    ...
    snapshot=None;visits=[];last_native=0;last_gen=0
    for row in rows[1:-1]:
        ...
        if kind=='snapshot':
            ...
            assert snapshot.faults==snapshot.configured==snapshot.cdc_drops==snapshot.pacer_drops==0
    ...
    return dict(status='pass',scope='ARM_local_two_frequency_transition_association',visits=visits,
                nominal_lo_hz=op['lo_plan_hz'],rf_seconds=20.1326592,native_tracking_qualified=False)
