...
def review(root):
    ...
    # Both builds contain the identical finite child. v1's separate parent
    # snapshot association is defective and must fail sequence review; this
    # child-only arithmetic check cannot qualify the parent's transitions.
    assert op['payload_sha256']['probe'] in (
    ...
    return dict(status='pass',scope='full_iq_two_frequency_visit_acquisition_arithmetic_and_source_counters',
        ...
        full_iq_retained=True,live_tracking_qualified=False,native_numerical_review_required=bool(heads),
