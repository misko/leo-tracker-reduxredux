...
OUT=Path('/home/mouse9911/gits/leo-radio20-reboot-publish/reports/figures/2026_09_13_radio20_arm_frequency_visits')
...
def main():
    result={'scope':'corrected_ARM_two_frequency_transition_qualification',
        'native_tracking_qualified':False,'parent_transitions_qualified':True,
    ...
    with (OUT/'evidence-corrected.json').open('x') as f: json.dump(result,f,indent=2);f.write('\n')
    print(json.dumps({'output':str(OUT/'evidence-corrected.json'),'sha256':digest(OUT/'evidence-corrected.json'),
