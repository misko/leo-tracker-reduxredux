...
OUT = Path(
    "/home/mouse9911/gits/leo-radio20-reboot-publish/reports/figures/2026_09_13_radio20_30ms_visit_readiness"
)
...
def main():
    ...
    assert (
        ...
        and deployed["returned_serial"] == "1040005e0b100007100010000bf33a5d4d"
    )
    ...
    assert all(
        value is None for name, value in failed["artifacts"].items() if name.startswith("visit-")
    )
    junit = BASE / "two-frequency-boot-epoch-tests-v2.xml"
    suite = ET.parse(junit).getroot().find("testsuite")
    result = {
        "scope": "30MS_deployment_and_boot_epoch_admission_fix",
        "native_tracking_qualified": False,
        "corrected_30MS_visits_executed": False,
        "new_RF_captures": 0,
        "fw_commit": subprocess.check_output(
            ["git", "rev-parse", "HEAD"], cwd=FW, text=True
        ).strip(),
        "deployment": deployed,
        "initial_probe_refusal": failed,
        "initial_probe_status": read(rejected / "stdout.json"),
        ...
        "binary_sha256": digest(BASE / "glrt-cpu-visit-probe-v3"),
        "sources": {},
    }
    for name in (
        "qualify_two_frequency_visits_multirate.py",
        "qualify_two_frequency_visits_boot_epoch.py",
        "review_two_frequency_boot_visit.py",
        "review_two_frequency_boot_sequence.py",
        "record_30ms_visit_readiness.py",
    ):
        shutil.copyfile(BASE / name, OUT / name)
        result["sources"][name] = digest(OUT / name)
