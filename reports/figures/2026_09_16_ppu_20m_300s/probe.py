    ...
    try:
        with radio.begin_metadata_capture(
            ...
            for index in range(target):
                ...
                if (index + 1) % 400 == 0:
                    print(json.dumps({"frames": index + 1, "target": target,
                        "elapsed_seconds": time.monotonic() - reading,
                        "source_seconds": (frames[-1]["end"] - frames[0]["first"]) / rate}), flush=True)
...
def main():
    ...
    parser.add_argument("--rates", default="20")
    ...
    if args.seconds != 300 or args.repeats != 1 or rates != [20_000_000] or args.ram_slots != 0:
        parser.error("authorized experiment is one 300-second 20 MS/s cell, no RAM extension")
