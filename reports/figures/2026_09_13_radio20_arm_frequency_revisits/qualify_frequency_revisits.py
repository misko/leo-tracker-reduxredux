...
def main():
    if len(sys.argv) not in (3,6,7): raise SystemExit('usage: qualify_frequency_revisits.py NEW_OUTPUT DEPLOYMENT_RECEIPT [CH CH CH [CH]]')
    ...
    rate=int(matched[1])
    payload={'probe':(E/'glrt-cpu-visit-probe-v6').read_bytes(),
    ...
    receipt={'scope':'bounded_ARM_frequency_revisits','visit_count':count,'retention_mode':'selected_windows','status':'started','rate':rate,
