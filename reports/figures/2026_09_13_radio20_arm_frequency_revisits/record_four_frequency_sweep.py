...
root = BASE / "frequency-sweep60-v2"
out = Path(
...
assert (
    op["serial"] == "1040005e0b100007100010000bf33a5d4d"
    and op["rate"] == 60000000
    and op["visit_count"] == 4
)
...
result = dict(
    status="pass_bounded_four_visit_sweep",
    operator=op,
    sequence=sequence,
    children=children,
...
with (out / "sweep60-evidence.json").open("x") as f:
    json.dump(result, f, indent=2)
    f.write("\n")
print(
    "evidence_sha256",
    digest(out / "sweep60-evidence.json"),
    "bytes",
    result["retained_artifact_bytes"],
)
for c in children:
