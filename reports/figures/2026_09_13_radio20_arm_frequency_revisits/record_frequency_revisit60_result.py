...
BASE=Path(__file__).parent;root=BASE/'frequency-revisits60-v1'
...
assert deployment['outcome']=='success' and deployment['returned_firmware']=='glrt-iq-tracking-r60000000-v1'
...
assert op['rate']==60000000 and op['before']==op['after'] and op['temporary_files_removed'] and op['exit_code']==0
...
result=dict(status='pass_bounded_three_visit_plan',deployment_result=deployment,
    deployment_receipt_sha256=hashlib.sha256(privileged(Path(deployment['receipt_path']))).hexdigest(),
    operator=op,sequence=sequence,children=children,four_visit_attempt=refusal,
...
with (out/'physical60-evidence.json').open('x') as f:json.dump(result,f,indent=2);f.write('\n')
