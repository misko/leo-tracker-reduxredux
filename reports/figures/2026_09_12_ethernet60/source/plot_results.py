...
axs[0].set(
    xlabel="RFIC and capture sample rate (MS/s)",
    ylabel="IQ payload (MB/s, decimal)",
    title="Measured Ethernet IQ delivery",
    xlim=(0, 63),
    ylim=(0, 253),
)
...
axs[1].annotate(
    f"60 MS/s: {mean:.2f} MB/s\n{100 * mean / 240:.1f}% delivery equivalent",
    xy=(60, 100 * mean / 240),
    xytext=(24, 50),
    arrowprops={"arrowstyle": "->", "color": "#6c4799"},
    color="#6c4799",
)
axs[1].axhline(100, color="gray", ls=":", lw=1)
axs[1].set(
    xlabel="RFIC and capture sample rate (MS/s)",
    ylabel="Delivered payload / continuous input (%)",
    title="Transport capacity; not a continuity guarantee",
    xlim=(0, 63),
    ylim=(0, 108),
)
for ax in axs:
...
for index, c in enumerate(direct):
    rate = c["rate_hz"]
    first = c["frames"][0]["first"]
    span = c["source_span_seconds"]
    y = 3 - index
    ax.broken_barh([(0, span)], (y - 0.24, 0.48), facecolors="#eee4df")
    intervals = [((r["first"] - first) / rate, r["samples"] / rate) for r in c["runs"]]
    ax.broken_barh(intervals, (y - 0.24, 0.48), facecolors="#17818d")
    labels.append(f"{rate / 1e6:g} MS/s · longest {c['longest_contiguous_seconds']:.2f} s")
    ax.text(
        span + 0.4,
        y,
        f"{100 * c['source_coverage_fraction']:.1f}% of source interval",
        va="center",
        fontsize=10,
    )
...
ax.set(
    xlim=(0, 61),
    ylim=(-0.7, 3.8),
    xlabel="Seconds since first delivered sample in each independent session",
    title="Counter-proven contiguous intervals · 50 × 1M-sample DMA buffers = 200 MB",
)
ax.text(
    0.01,
    -0.25,
    "Teal = received contiguous IQ; pale = counter-proven missing IQ. Each row contains 30 seconds of delivered samples.\n"
...
bars = ax.bar(np.arange(1, len(lengths) + 1), lengths, color="#17818d", width=0.72)
bars[-1].set(facecolor="#cbdde0", hatch="//", edgecolor="#17818d")
ax.axhline(
    np.median(lengths), color="#a15022", ls="--", label=f"Median: {np.median(lengths):.2f} s"
)
ax.scatter(
    np.flatnonzero(lengths < 0.04) + 1,
    lengths[lengths < 0.04],
    color="#b34843",
    zorder=3,
    label="Six isolated buffers: 33.33 ms each",
)
ax.set(
    xticks=np.arange(1, len(lengths) + 1),
    xlabel="Continuous segment, in source-time order",
    ylabel="Continuous IQ duration (seconds)",
    ylim=(0, 4.05),
    title="30 MS/s · 18 counter-proven segments · minimum 33.33 ms · median 2.35 s",
)
ax.annotate(
    "Final segment reaches test end;\nnatural end is unknown",
    xy=(18, lengths[-1]),
    xytext=(10, 3.6),
    arrowprops={"arrowstyle": "->", "color": "#385a65"},
    fontsize=10,
)
