...
def review(root,text=None,children=None):
    ...
    if children is None:
        children=[json.loads((root/f'visit-{n}/independent-visit-review.json').read_text()) for n in range(2)]
    assert len(children)==2
    rows=text.splitlines();plan=rows[0].split()
    assert plan==['plan',str(op['rate']),op['serial'],*map(str,op['lo_plan_hz']),'1536','2','60000000000']
    ...
    return dict(status='pass',scope='ARM_local_two_frequency_transition_association',visits=visits,
