...
def check_estimate(head, estimate, basis, gram, rate):
    ...
    direction = np.r_[1, np.clip(correction, -.25, .25)]
    ...
    reported = [estimate[k] for k in ('delay_s','residual_hz','cfo_hz','coherence','linearized_coherence')]
...
def main(root):
    ...
    for ep in epochs['episodes']:
        ...
        fitted = [check_estimate(h,e,basis,gram,rate)
                  for h,e in zip(checked['heads'], checked['estimates'], strict=True)]
        if not mutation_checks:
            for field, delta in (('cfo_hz',1), ('coherence',.01)):
        ...
        last_supported = max([proposals[-1]['last_supported']] + [e['frame'] for e in fitted if e['rejection'] == 0])
        terminal = next(x for x in rows if x['kind'] == 'native_terminal' and x['native_episode'] == ep['episode'])
        if terminal['result'] == -4:
            assert fitted[-1]['frame'] == last_supported+32
        episodes.append(dict(episode=ep['episode'], epoch=ep['epoch'], results=len(fitted),
            supported=checked['supported'], controller_result=terminal['result'],
            first_frame=fitted[0]['frame'], final_frame=fitted[-1]['frame'],
            last_supported_frame=last_supported, horizon_exhausted=terminal['result'] == -4,
            estimates=fitted, journal_sha256=hashlib.sha256(journals[name]).hexdigest()))
    assert sum(ep['results'] for ep in episodes) == status['native_results']
    result = dict(status='pass', scope='all_native_epoch_retained_moment_estimates_and_drain',
        rate=rate, native_moments_independently_recomputed=False, live_tracking_qualified=False,
        native_results=status['native_results'], supported=sum(ep['supported'] for ep in episodes),
        corruption_checks=mutation_checks, episodes=episodes,
    ...
    with (root/'native-epoch-estimate-review.json').open('x') as output:
        ...
        output.write('\n')
    print(json.dumps({**result, 'episodes':[{k:v for k,v in ep.items() if k != 'estimates'} for ep in episodes]}))
