...
def review(root):
    ...
    probe_hash=op['payload_sha256']['probe']
    assert probe_hash in ('360d81ee27bb30b0f350e2d6e56f587895ab34c84e61e58d5fc0c15f4110007e',
    ...
    assert rate in (30000000,60000000) and status['status'] in (0,3) and op['exit_code']==0
    if status['status']==3:
        assert probe_hash=='dca73d4861f67327f352f89f1cc2d74e6c8c73788dc471cf340502aeab84e53b'
    ...
    if not status['worker_complete']:
        assert len(blocks)==1536 and pair(final,4)==25165824
    else: assert status['stage']=='worker_complete'
    ...
    assert len(full)==len(blocks)*16384
    searched=np.concatenate([full[r['window_start']-origin:r['window_start']-origin+14000] for r in scans])
    ...
    return dict(status='pass',scope='full_iq_two_frequency_visit_acquisition_arithmetic_and_source_counters',
        passive_observer=observer,clean_loss=loss,child_disposition=status['status'],probe_sha256=probe_hash,
        run_succeeded=status['status']==0,post_capture_rf_settings_checked='rf_after' in op,
        full_iq_retained=len(full)==pair(final,4),full_returned_iq_retained=True,
        unretained_exported_tail_samples=pair(final,4)-len(full),
        live_tracking_qualified=False,native_numerical_review_required=bool(heads),
