...
def review(root):
    ...
    assert root.name in ('visit-0','visit-1')
    op={**op,'artifacts':{name.split('/',1)[1]:value for name,value in op['artifacts'].items()
    ...
    if not pairs:
        ...
        assert op['artifacts']['native.coarse.ci16']==dict(bytes=0,sha256=hashlib.sha256(b'').hexdigest())
        return dict(status='no_pairs',scope='same_pilot_coarse_iq_native_moment_comparison',
            pairs=0,native_total_results=0,live_tracking_qualified=False,
    ...
    for ep in epochs['episodes']:
        if not ep['results']: continue
        name='native.journal' if not ep['episode'] else f"native-{ep['episode']}.journal"
        native[ep['episode']]=native_review(journals[name],epoch=ep['epoch'],rate=rate)
    assert len(pairs)==min(64,epochs['native_results'])
    ...
    bank=Path('/home/mouse9911/gits/plutosdr-fw-radio20-tracking/hdl/library/starlink_glrt/native_cubic_60000000_upper.mem')
    ...
    native_basis=np.column_stack((ref,-derivative,1j*np.pi*1000/rate*(2*np.arange(n)-(n-1))*ref))
    gram=native_basis.conj().T@native_basis
    checked=[]
    for index,row in enumerate(pairs):
        episode=row['native_episode']; sequence=row['native_sequence']; epoch=row['source']['epoch']
        head=native[episode]['heads'][sequence]; estimate=native[episode]['estimates'][sequence]
        head.require_complete()
        assert row['native_sequence']==head.sequence and row['iq_offset']==index*3333
        ...
        assert (row['native_start'],row['native_phase_step'],row['native_phase_seed'],
                row['native_reference_phase'],row['native_count'],row['native_fault'])==(
                head.start,head.phase_step,head.phase_seed,head.reference_phase,head.count,head.fault)
        view=row['source']
        assert view['epoch']==epoch and view['valid'] and not view['closed']
        assert view['first']<=row['first'] and row['first']+3333<=view['end']<=view['source_now']
        ...
        rotated=np.asarray([rotate(int(i),int(q),(phase0+k*step)%2**32)
                            for k,(i,q) in enumerate(cut)],dtype=np.int64)
        fraction=head.start%ratio/ratio
        quarters=round(4*fraction);carry,phase=divmod(quarters,4)
        ...
        rejected=(32 if np.any(abs(correction)>=.25) else 0)|(64 if coherence<.05 else 0)
        scores=[]
        for shift in range(-8,9):
            x=rotated[16+shift:16+shift+3300].astype(float);z=x[:,0]+1j*x[:,1]
        ...
        native_correction,_,_=dense_fit(native_basis,np.column_stack((projected.real,projected.imag)))
        native_coherence=float(abs(p[0])**2/(gram[0,0].real*head.observed_energy))
        direction=np.r_[1,np.clip(native_correction,-.25,.25)]
        model=native_basis@direction
        native_improved=float(abs(direction@p)**2/(np.vdot(model,model).real*head.observed_energy))
        predicted=(head.phase_step if head.phase_step<2**31 else head.phase_step-2**32)*rate/2**32
        expected=[direction[1]*1e-6,direction[2]*1000,predicted+direction[2]*1000,native_coherence,native_improved]
        np.testing.assert_allclose([estimate[k] for k in ('delay_s','residual_hz','cfo_hz','coherence','linearized_coherence')],
                                   expected,rtol=2e-10,atol=2e-12)
        native_rejection=(32 if np.any(abs(native_correction)>=.25) else 0)|(64 if native_coherence<.05 else 0)
        assert native_rejection==estimate['rejection']
        checked.append(dict(native_episode=episode,epoch=epoch,sequence=sequence,frame=estimate['frame'],native_start=head.start,
            native_coherence=native_coherence,native_energy_per_sample=head.observed_energy/n,
            native_matched_energy_per_sample=native_coherence*head.observed_energy/n,
            native_rejection=native_rejection,native_delay_ns=estimate['delay_s']*1e9,
            native_residual_hz=estimate['residual_hz'],
            coarse_fraction=fraction,coarse_reference_phase=phase,coarse_start_carry=carry,
            coarse_coherence=float(coherence),coarse_linearized_coherence=float(improved),
            coarse_energy_per_sample=energy,coarse_matched_energy_per_sample=float(coherence)*energy,
            coarse_rejection=rejected,coarse_delay_ns=float(np.clip(correction[0],-.25,.25))*1000,
            coarse_residual_hz=float(np.clip(correction[1],-.25,.25))*1000,
            timing_search_best=best,timing_search_scores=scores))
    return dict(status='pass',scope='same_pilot_coarse_iq_native_moment_comparison',rate=rate,
        pairs=len(checked),native_total_results=epochs['native_results'],
        native_moments_independently_recomputed=False,live_tracking_qualified=False,
        coarse_supported=sum(x['coarse_rejection']==0 for x in checked),
        native_supported=sum(x['native_rejection']==0 for x in checked),comparisons=checked,
        sha256={name:hashlib.sha256((root/name).read_bytes()).hexdigest() for name in
                ('native.coarse.ci16','worker.jsonl','operator.json','stdout.json',*journals)},
        reviewer_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest())
