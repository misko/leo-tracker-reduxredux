...
def filter_signal(values, origin, rate):
    ...
    for fs,decimation in stages(rate):
        values = lfilter(fir(fs)/2**17, [1.], values)
        stride *= decimation
...
def geometry_checks(rate):
    ...
    for fs,_ in stages(rate):
        h = fir(fs)/2**17
        expected *= np.sum(h*np.exp(-2j*np.pi*f*(np.arange(len(h))-(len(h)-1)/2)/fs))
...
def main():
    assert hashlib.sha256((BASE/'direct-references.ci16').read_bytes()).hexdigest() == '78b50e1aea5c350889b0798fc691491299925932e496a918cd5fbd3b9bc4faf2'
    ...
    result = {'scope':'native_projected_component_through_frozen_ddc',
        'native_raw_iq_available':False, 'unknown_orthogonal_component_included':False,
        'arithmetic':'floating linear filter; excludes fixed-point rounding and clipping',
        'new_rf_samples':0, 'runtime_change':False, 'threshold_calibrated':False,
        'input_sha256':{}, 'rates':{}}
    for rate in (30000000,60000000):
        ...
        for name in ('paired-coherence-review.json','native-epoch-estimate-review.json'):
            result['input_sha256'][str(root/name)] = hashlib.sha256((root/name).read_bytes()).hexdigest()
        native = {}
        ...
        basis = np.column_stack((ref,-derivative,1j*np.pi*1000/rate*(2*np.arange(n)-(n-1))*ref))
        ...
        for row in pairs['comparisons']:
            ...
            p = np.array([complex(*head.reference_sum),complex(*head.delay_sum),-1j*np.pi*1000/rate*complex(*center)])
            ...
            phase = 2*np.pi*((head.phase_seed+np.arange(n,dtype=np.int64)*step)%2**32)/2**32
            ...
            phase = 2*np.pi*((head.phase_seed+(center_indexes-head.start)*step)%2**32)/2**32
            ...
            checked.append({'episode':row['native_episode'],'sequence':row['sequence'],
                'frame':row['frame'],'carrier_hz':step*rate/2**32,
                'observed_coarse_native_matched_ratio':observed/row['native_matched_energy_per_sample'],
                'predicted_coarse_native_matched_ratio':predicted/row['native_matched_energy_per_sample'],
        ...
        result['rates'][str(rate)] = {'geometry_checks':geometry_checks(rate),'pairs':len(checked),
            'observed_matched_ratio_median':statistics.median(x['observed_coarse_native_matched_ratio'] for x in checked),
            'predicted_matched_ratio_median':statistics.median(x['predicted_coarse_native_matched_ratio'] for x in checked),
            'observed_to_predicted_median':statistics.median(x['observed_to_predicted_matched_ratio'] for x in checked),
            'native_heads':len(heads),'native_coherence_supported':sum(h['coherence']>=.05 for h in heads),
            'native_linearized_coherence_supported':sum(h['linearized_coherence']>=.05 for h in heads),
            'rejection_counts':dict(Counter(h['rejection'] for h in heads)),
            'native_residual_hz_min_median_max':np.quantile([h['residual_hz'] for h in heads],[0,.5,1]).tolist(),
            'native_delay_ns_min_median_max':np.quantile([h['delay_s']*1e9 for h in heads],[0,.5,1]).tolist(),
            'coarse_fixed_coherence_median':statistics.median(x['coarse_fixed_coherence'] for x in checked),
            'coarse_best_timing_coherence_median':statistics.median(x['coarse_best_timing_coherence'] for x in checked),
            'comparisons':checked}
    result['source_sha256'] = hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
    result['native_bank_sha256'] = hashlib.sha256(BANK.read_bytes()).hexdigest()
    from tests.starlink_glrt import ddc
    for path in [Path(ddc.__file__), BASE/'direct-references.ci16',
    ...
    with (BASE/'native-filter-transfer-diagnostic-v2.json').open('x') as out:
        ...
        out.write('\n')
    print(json.dumps({**result,'rates':{k:{n:v for n,v in d.items() if n!='comparisons'} for k,d in result['rates'].items()}},indent=2))
