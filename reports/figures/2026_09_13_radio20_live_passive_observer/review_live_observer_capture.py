...
def review(root):
    ...
    assert op['payload_sha256']['probe']=='f21bafa7c919d24e217b7fa36b9800b69aae45522ec613427a0eb56480be4576'
    assert op['passive_observer']=={'rate':2500000,'feedback_authority':False,
        'maximum_measurements_per_episode':200,'maximum_episodes':4}
    ...
    assert op['blocks'] == 45000 and op['rf_sample_limit'] == 737280000
    assert op['rf_duration_limit_s'] == 294.912
    ...
    assert op['memory_preflight']['required_kib'] == 336*1024
    assert op['filesystem_preflight']['required_kib'] == 296*1024
    assert op['memory_preflight']['available_kib'] >= 336*1024
    assert op['filesystem_preflight']['available_kib'] >= 296*1024
    ...
    assert len(blocks) == status['completed_refills'] == status['blocks'] <= 45000
    assert len(blocks)*16384 <= pair(final,4) == pair(final,6) <= 737280000
    ...
    assert len(searched) == status['retained_scan_samples'] == len(scans)*14000 <= 2800000
    assert len(owned) <= 20000000 and 0 < len(scans) <= status['attempts'] <= 200
