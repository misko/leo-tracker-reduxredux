...
def main():
    ...
    assert (
        ...
        == hashlib.sha256((BASE / "paced-live-observer-arm-v1").read_bytes()).hexdigest()
    )
    ...
    result = dict(
        status="pass",
        scope="paced_saved_iq_causal_startup_and_live_observer_review",
        execution_platform="host" if host else "ARM",
        rotation_oracle_checks=check_rotation(),
        new_rf_samples=0,
        native_jobs=0,
    ...
    for name in ("positive", "control"):
        ...
        iq = np.fromfile(
            BASE / "paced-original-seed-input-v1" / f"{name}.ci16", dtype="<i2"
        ).reshape(-1, 2)
        ...
        scan = rows[0]
        assert scan["kind"] == "paced_scan" and scan["window_start"] == origin
        ...
        for epoch, frequency, score, power in scan["peaks"]:
            x = iq[epoch + 22 : epoch + 3322].astype(float)
            z = x[:, 0] + 1j * x[:, 1]
            ref = bases[0][:, 0]
            expected = max(abs(np.fft.fft(z * np.conj(ref), 16384)) ** 2) / (
                np.vdot(z, z).real * np.vdot(ref, ref).real
            )
        ...
        for row in rows:
            ...
            past.append(row)
            if row["accepted"]:
                accepted.append((row, np.clip(correction[0], -0.25, 0.25) * 1e-6, cfo))
        ...
        if name == "positive":
            ...
            for row, delay, cfo in accepted[-96:]:
                offset = (
                    row["first"]
                    - handoff["anchor"]
                    - (row["frame"] - handoff["first_frame"]) * 10000 / 3
                )
            ...
            for frame, offset, cfo in handoff["history"][: handoff["count"]]:
                np.testing.assert_allclose(
                    [offset, cfo], expected_history[frame], rtol=2e-12, atol=2e-8
                )
            recent = [
                r
                for r in handoff["history"][: handoff["count"]]
                if handoff["last_supported"] - r[0] < 96
            ]
            assert len(recent) >= 8
        ...
        for row in map(json.loads, (root / "observer.jsonl").read_text().splitlines()):
            if row["kind"] != "measurement":
                continue
            first = row["first"] - origin
            offset = row["iq_offset"]
            np.testing.assert_array_equal(
                observer_iq[offset : offset + 3300], iq[first : first + 3300]
            )
        if name == "positive":
            ...
            assert not observer["episodes"] and status["observer_measurements"] == 0
        result["cases"][name] = dict(
            status="pass",
            worker_status=status["worker_status"],
            passive_observer=observer,
            acquisition_elapsed_ms=terminal["elapsed_ms"],
            startup_prediction_review=startup,
            grid_values_checked=int(grid.size),
            ranking_scores_checked=len(scores),
            resolver_hypotheses_checked=17,
            moment_heads_checked=len(past),
            dense_svd_fits_checked=len(past),
            accepted_observations=len(accepted),
            supported_history=terminal["supported_history"],
            worker_iq_samples_checked=checked,
            elapsed_ms=status["elapsed_ms"],
            scan_ms=scan["scan_ms"],
            rank_ms=scan["rank_ms"],
            seed_age_ms=(seed["copied"]["source_now"] - origin) / 2500,
            handoff_lead_samples=lead,
    ...
    with (ROOT / "independent-review.json").open("x") as out:
