...
def main():
    ...
    evidence = dict(scope='paced_saved_iq_arm_live_observer_no_rx_no_native_io', new_rf_samples=0,
                    live_tracking_qualified=False, status='started', cases={})
    ...
    payload = {'probe': (EVIDENCE/'paced-live-observer-arm-v1').read_bytes(),
    ...
    evidence['payload_sha256'] = hashes; evidence['inputs'] = manifest
    ...
    try:
        lease = authority.claim(('radio_pluto_5d4d',), task_id='paced20-live-observer-arm-v1', task_kind=CaptureTaskKind.QUALIFICATION)
    ...
    with lease, acquire_radio_lock(ENDPOINT[0]):
        ...
        try:
            ...
            available = next(int(line.split()[1]) for line in memory.stdout.decode().splitlines()
            ...
            for name in ('positive', 'control'):
                ...
                assert len(iq) == 25000000 and hashlib.sha256(iq).hexdigest() == manifest[name]['sha256']
                run('cat > '+shlex.quote(remote+'/iq'), iq).check_returncode()
                checked = run('sha256sum '+shlex.quote(remote+'/iq')); checked.check_returncode()
                ...
                save(); print(json.dumps({'phase':'paced_replay', 'case':name}), flush=True)
                result = run(shlex.join([remote+'/probe', remote+'/refs', remote+'/bank', remote+'/iq',
                                        str(manifest[name]['source_window']), directory]))
                (root/'stdout.json').write_bytes(result.stdout); (root/'stderr.txt').write_bytes(result.stderr)
                case['exit_code'] = result.returncode
                case['artifacts'] = {}
                for artifact in ('worker.jsonl', 'worker.iq.ci16', 'grids.u32', 'observer.jsonl', 'observer.iq.ci16'):
                    data = run('cat '+shlex.quote(directory+'/'+artifact)); data.check_returncode()
                    (root/artifact).write_bytes(data.stdout)
                    case['artifacts'][artifact] = {'bytes':len(data.stdout), 'sha256':hashlib.sha256(data.stdout).hexdigest()}
            ...
            try:
                ...
                if certain:
                    ...
                    files += [remote+'/'+case+'/'+n for case in ('positive','control')
                              for n in ('worker.jsonl','worker.iq.ci16','grids.u32','observer.jsonl','observer.iq.ci16')]
                    run('rm -f '+shlex.join(files)+' && rmdir '+
                        shlex.join([remote+'/positive',remote+'/control',remote])).check_returncode()
