...
def main():
    ...
    result = {
        "scope": "live_observer_host_integration",
        "new_rf_samples": 0,
        ...
        "binary": {
            "path": str(BASE / "glrt-cpu-live-probe-v12"),
            "sha256": digest(BASE / "glrt-cpu-live-probe-v12"),
        },
        "compiler_flags": [
            "-std=c99",
            "-O3",
        ...
        "sources": {},
        "cases": {},
        "rotation_scalar_checks": check_rotation(),
    }
    for path in sorted((FW / "tools").glob("glrt_*")):
        if path.suffix in (".c", ".h", ".inc"):
            result["sources"][str(path.relative_to(FW))] = digest(path)
    ...
    selected = {
        "native30": "test_advancing_capture_worker_0",
        "native60": "test_advancing_capture_worker_1",
        "reacquire30": "test_clean_native_loss_reacqui0",
        "reacquire60": "test_clean_native_loss_reacqui1",
    }
    ...
    result["live_attempt"] = json.loads(
        (BASE / "cpu-live60-passive-observer-v1/operator.json").read_text()
    )
    assert (
        result["live_attempt"]["status"] == "admission_refused"
        and result["live_attempt"]["rf_samples_collected"] == 0
    )
        ...
        json.dumps(
            {
                ...
                "moments_and_dense_fits_checked": sum(
                    c["moments_and_dense_fits_checked"] for c in result["cases"].values()
                ),
            }
        )
    )
