#!/usr/bin/env python3
"""Reproduce the frozen sixteen-hour RX0 multi-rate scanner evidence summary."""
...
END = datetime(2026, 9, 18, 1, 0, tzinfo=UTC)
...
def main(output: Path) -> None:
    ...
    for path in capture_root.glob("*/manifest.json"):
        ...
        row = {
            ...
            "online_detections": detections if decisions else None,
            "online_detection_fraction": detections / len(decisions) if decisions else None,
            ...
            "queue_high_water_visits": (item.get("queue_telemetry") or {}).get("high_water_visits"),
            "queue_enqueue_failures": (item.get("queue_telemetry") or {}).get("enqueue_failure_count"),
        }
        captures.append(row)
    ...
    for row in captures:
        if (
            row["sample_rate_hz"] not in {10_000_000, 15_000_000, 20_000_000}
    ...
    for row in captures:
        ...
        adaptive = sum(p.name.endswith(".png") for p in (analysis_root / sid).glob("**/*.png"))
    ...
    for (case, profile), rows in sorted(grouped.items()):
        ...
        def pooled(name: str, rows: list[dict] = rows) -> float | None:
            usable = [x for x in rows if x.get(name) is not None and x["common"]]
            weight = sum(x["common"] for x in usable)
            return (
                math.sqrt(sum(x["common"] * x[name] ** 2 for x in usable) / weight)
                if weight else None
            )
    ...
    for rate in (10_000_000, 15_000_000, 20_000_000):
        rate_captures = [x for x in captures if x["sample_rate_hz"] == rate]
        rate_ids = {x["session_id"] for x in rate_captures}
        rate_tracking = [x for x in tracking_sessions if x["session_id"] in rate_ids]
        rate_tracklets = [x for x in tracklets if x["session_id"] in rate_ids]
        rate_candidates = [x for x in tle_candidates if x["session_id"] in rate_ids]
        rate_duty = [x["valid_duty_percent"] for x in rate_captures]
        by_rate[str(rate)] = {
            "sessions": len(rate_captures),
            "duty_percent_median": float(np.median(rate_duty)) if rate_duty else None,
            "complete_visits": sum(x["complete_visits"] for x in rate_captures),
            "online_detections": None,
            "online_detection_fraction": (
                None
            ),
            "tracking_sessions": len(rate_tracking),
            "tracklets": len(rate_tracklets),
            "observations": sum(x["observation_count"] for x in rate_tracklets),
            "tracklets_per_session_median": float(np.median([
                x["tracklet_count"] for x in rate_tracking
            ])) if rate_tracking else None,
            "observations_per_session_median": float(np.median([
                x["observation_count"] for x in rate_tracking
            ])) if rate_tracking else None,
            "session_pooled_linear_fit_residual_rms_hz_median": float(np.median([
                x["pooled_linear_fit_residual_rms_hz"] for x in rate_tracking
                if x["pooled_linear_fit_residual_rms_hz"] is not None
            ])) if any(x["pooled_linear_fit_residual_rms_hz"] is not None for x in rate_tracking) else None,
            "tle_states": dict(Counter(x["tle_state"] for x in rate_tracking)),
            "trajectory_states": dict(Counter(x["trajectory_state"] for x in rate_tracking)),
            "tle_candidates": len(rate_candidates),
            "identity_claims": sum(x["identity_claimed"] for x in rate_candidates),
        }

    summary = {
        "schema": "org.leo.research.sixteen-hour-multirate-scanner-summary/v1",
        "window": {
        ...
        "acquisition": {
            ...
            "scheduled_slots": 96,
            "sealed_sessions_missing_from_schedule": 96 - len(captures),
            "radio_serial": RADIO_SERIAL,
            "sample_rates_hz": [10_000_000, 15_000_000, 20_000_000],
            "receiver_ids": [0],
        ...
        "tle": {
            ...
            if any(x["tle_snapshot_age_minutes"] is not None for x in tracking_sessions)
            else None,
        },
        "pss": {
        ...
        "presentation": {
            "sessions_with_three_overview_pngs": sum(x["adaptive_pngs"] >= 3 for x in png_rows),
            "sessions_with_trajectory_png": sum(x["tracking_pngs"] >= 1 for x in png_rows),
            "sessions_with_two_refinement_pngs": sum(x["refinement_pngs"] >= 2 for x in png_rows),
        },
    }
    ...
    ax1.set_ylim(0, 100)
    ax1.grid(alpha=0.25)
    ax1.set_title("Frozen sixteen-hour RX0 multi-rate scanner cohort")
    fig.autofmt_xdate()
    fig.tight_layout()
    ...
    labels = ["10", "15", "20"]
    rate_values = [by_rate[str(x * 1_000_000)] for x in (10, 15, 20)]
    fig, axes = plt.subplots(1, 3, figsize=(13, 4.8))
    axes[0].bar(labels, [x["duty_percent_median"] for x in rate_values], color="#1676a3")
    axes[0].set_ylabel("Median valid duty (%)")
    axes[0].set_xlabel("Native sample rate (MS/s)")
    axes[0].set_ylim(0, 100)
    axes[1].bar(labels, [x["observations_per_session_median"] or 0 for x in rate_values], color="#cf4d61")
    axes[1].set_ylabel("Median associated GLRT observations/session")
    axes[1].set_xlabel("Native sample rate (MS/s)")
    axes[2].bar(labels, [x["session_pooled_linear_fit_residual_rms_hz_median"] or 0 for x in rate_values], color="#6b8e23")
    axes[2].set_ylabel("Median session-pooled local-fit RMS (Hz)")
    axes[2].set_xlabel("Native sample rate (MS/s)")
    fig.suptitle("Observed production outcomes by native capture rate")
    fig.tight_layout()
    fig.savefig(output / "sample-rate-comparison.png", dpi=170)
    plt.close(fig)
    ...
    fig.suptitle("Controlled refinement checks; pooled over common recovered probes")
    fig.tight_layout()
    ...
    ax.set_title("Historical five-dwell comparator (unmatched to the sixteen-hour scanner cohort)")
