...
class PssTimingSearchConfig:
    ...
    fractional_delay_phases: int = 32
    fractional_interpolation_half_width: int = 8
    ...
    def __post_init__(self) -> None:
        ...
        for name, value, minimum in (
            ("fractional delay phases", self.fractional_delay_phases, 2),
            ("fractional interpolation half width", self.fractional_interpolation_half_width, 2),
        ):
            if isinstance(value, bool) or not isinstance(value, int) or value < minimum:
                raise ValueError(f"PSS {name} must be an integer at least {minimum}")
        ...
        if qualified:
            windows.extend(
                _measure_candidate_windows(
                    ...
                    fractional_delay_phases=policy.fractional_delay_phases,
                    fractional_interpolation_half_width=policy.fractional_interpolation_half_width,
                )
            )
...
def _measure_candidate_windows(
    ...
    *,
    ...
    for frame_index, predicted in enumerate(starts):
        ...
        if (
            search_start - fractional_interpolation_half_width + 1 < 0
            or search_stop + template.size + fractional_interpolation_half_width - 1 > values.size
        ):
            ...
            continue
        scores, _correlations = _local_match_scores(
            values,
            conditioned,
            search_start=search_start,
            search_stop=search_stop,
        )
        fractional_local, match_power, correlation = _fractional_match_peak(
            values,
            conditioned,
            search_start=search_start,
            search_stop=search_stop,
            phase_count=fractional_delay_phases,
            half_width=fractional_interpolation_half_width,
        )
        measured = round(fractional_local)
        ...
        absolute_phase = correlation * np.exp(
            -2j * np.pi * candidate.frequency_offset_hz * fractional_global / sample_rate_hz
        )
        output.append(
            PssFrameWindow(
                ...
                normalized_match_power=match_power,
                peak_to_local_median=float(
                    match_power / max(float(np.median(scores)), np.finfo(float).tiny)
                ),
...
def _fractional_kernels(phase_count: int, half_width: int) -> npt.NDArray[np.float64]:
    offsets = np.arange(-half_width + 1, half_width + 1)
    distance = np.arange(phase_count)[:, None] / phase_count - offsets[None, :]
...
def _fractional_match_peak(
    ...
    *,
    ...
    """Compare all aperture peaks at fractional delays before selecting a winner.

    Polyphase convolutions evaluate the same windowed-sinc IQ interpolation as
    fractional_take without allocating a delay-by-template-by-tap tensor. The
    aperture ends at search_stop - 1; interpolation never adds candidate delays
    outside it. Full interpolation support must lie in the supplied continuous IQ.
    """
    if (
        search_stop <= search_start
        or search_start - half_width + 1 < 0
        or search_stop + template.size + half_width - 1 > values.size
    ):
        raise ValueError("fractional PSS aperture lacks interpolation support")
    padded = values[search_start - half_width + 1 : search_stop + template.size + half_width - 1]
    count = search_stop - search_start
    grid = np.zeros((count, phase_count), dtype=np.float64)
    reverse_template = np.conj(template[::-1])
    for phase, kernel in enumerate(_fractional_kernels(phase_count, half_width)):
        interpolated = (
            np.asarray(values[search_start : search_stop + template.size - 1], dtype=np.complex128)
            if phase == 0
            else np.convolve(padded, kernel[::-1], mode="valid")
        )
        correlations = np.convolve(interpolated, reverse_template, mode="valid")
        cumulative = np.concatenate(([0.0], np.cumsum(abs(interpolated) ** 2)))
        energy = cumulative[template.size :] - cumulative[: -template.size]
        np.divide(abs(correlations) ** 2, energy, out=grid[:, phase], where=energy > 0)
    surface = grid.ravel()[: (count - 1) * phase_count + 1]
    best = int(np.argmax(surface))
    fine = _parabolic_log_peak(surface, best) / phase_count
    delay = search_start + best / phase_count + fine
    # fractional_take promotes its source to complex128: pass only this bounded
    # aperture, never copy the complete high-rate recording once per frame.
    local_origin = search_start - half_width + 1
    aligned = fractional_take(
        padded, delay - local_origin + np.arange(template.size), half_width=half_width
    )
    correlation = complex(np.vdot(template, aligned))
    energy = float(np.vdot(aligned, aligned).real)
    power = abs(correlation) ** 2 / energy if energy > 0 else 0.0
    return float(delay), float(power), correlation
