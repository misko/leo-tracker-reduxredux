...
@pytest.mark.parametrize("fraction", (0.35, 0.45, 0.65))
def test_fractional_peak_competition_recovers_peak_missed_by_integer_argmax(fraction):
    # Independent Fourier-shift synthesis: the correct pulse lies between samples.
    # A distorted competing pulse lies exactly on the integer grid and wins there.
    rng = np.random.default_rng(129)
    template = rng.normal(size=33) + 1j * rng.normal(size=33)
    template /= np.linalg.norm(template)
    source = np.zeros(512, dtype=complex)
    source[120:153] = template
    shifted = np.fft.ifft(
        np.fft.fft(source) * np.exp(-2j * np.pi * np.fft.fftfreq(len(source)) * fraction)
    )
    distortion = rng.normal(size=33) + 1j * rng.normal(size=33)
    distortion -= template * np.vdot(template, distortion)
    distortion /= np.linalg.norm(distortion)
    shifted[200:233] += template + 0.35 * distortion
    values = shifted.astype(np.complex64)
    template = template.astype(np.complex64)
    sampled, _ = _local_match_scores(values, template, search_start=110, search_stop=211)
    assert 110 + np.argmax(sampled) == 200
    delay, power, correlation = _fractional_match_peak(
        values, template, search_start=110, search_stop=211
    )
    assert delay == pytest.approx(120 + fraction, abs=0.06)
    assert power > float(sampled.max())
    assert abs(correlation) > 0


def test_fractional_peak_requires_full_support_and_stays_inside_aperture():
    values = np.zeros(128, dtype=np.complex64)
    template = np.ones(5, dtype=np.complex64) / np.sqrt(5)
    for start, stop in ((0, 20), (100, 124), (30, 30)):
        with pytest.raises(ValueError, match="interpolation support"):
            _fractional_match_peak(values, template, search_start=start, search_stop=stop)
    # Zero energy is deterministic and finite, including a one-position aperture.
    assert _fractional_match_peak(values, template, search_start=30, search_stop=31) == (
        30.0,
        0.0,
        0j,
    )
    values[31:36] = template
    delay, _, _ = _fractional_match_peak(values, template, search_start=30, search_stop=31)
    assert delay == 30.0  # The stronger peak outside the aperture cannot be used.


@pytest.mark.parametrize("value", (True, 1, 2.5))
def test_fractional_policy_rejects_invalid_sampling_geometry(value):
    with pytest.raises(ValueError, match="fractional delay phases"):
        PssTimingSearchConfig(fractional_delay_phases=value)
    with pytest.raises(ValueError, match="fractional interpolation half width"):
        PssTimingSearchConfig(fractional_interpolation_half_width=value)


def test_fractional_alignment_work_is_bounded_by_aperture_not_recording(monkeypatch):
    from leo.analysis.starlink import pss_timing

    original = pss_timing.fractional_take
    source_lengths = []

    def checked(samples, positions, **kwargs):
        source_lengths.append(len(samples))
        return original(samples, positions, **kwargs)

    monkeypatch.setattr(pss_timing, "fractional_take", checked)
    values = np.zeros(1_000_000, dtype=np.complex64)
    template = np.ones(11, dtype=np.complex64) / np.sqrt(11)
    _fractional_match_peak(values, template, search_start=500_000, search_stop=500_101)
    assert source_lengths and max(source_lengths) < 256
