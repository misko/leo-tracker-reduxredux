...
ORDER = [
    ...
    "recording-2",
    "recording-3",
    "recording-4-initial-clean-loss",
    "recording-4",
    "recording-5-initial-source-loss",
    "recording-5",
]
PROBES = {
...
def review_run(name: str) -> dict:
    ...
    assert (
        op["rf_after"]["tx_powerdown"] == "1" and op["rf_after"]["sampling_frequency"] == "30000000"
    )
    ...
    for snap in snapshots:
        snap.require_drained()
        assert snap.rate == 30_000_000
    ...
    if status["stage"] == "finite_source_complete" and not status["worker_complete"]:
        assert status["attempts"] - len(terminals) in (0, 1) and len(scans) - len(terminals) in (
            0,
            1,
        )
    else:
    ...
    for i in sorted(sample_indexes):
        ...
        expected = integer_grid(cut, bank())
        np.testing.assert_array_equal(grids[i], expected)
        peaks = sorted_peak_oracle(expected, 64)
        assert scans[i]["peaks"] == [list(p) for p in peaks]
        powers = []
        zall = cut[:, 0].astype(float) + 1j * cut[:, 1]
        for epoch, frequency, score in peaks:
            z = zall[epoch + 22 : epoch + 3322]
            powers.append(
                float(
                    max(abs(np.fft.fft(z * np.conj(ref), 4096)) ** 2)
                    / max(float(np.vdot(z, z).real) * ref_energy, 1)
                )
            )
        np.testing.assert_allclose(ranks[i]["single_pilot_power"], powers, rtol=2e-12, atol=2e-15)
        assert ranks[i]["selected_rank"] == int(np.argmax(powers))
        numerical += grids[i].size
        ranking_fft += len(powers)
    journals = {
    ...
    basis, gram, reference_sha = native_basis(30_000_000)
    native_rows = []
    incomplete = []
    episode_summaries = []
    for ep in epoch_review["episodes"]:
        ...
        terminal = next(
            r
            for r in rows
            if r.get("kind") == "native_terminal" and r["native_episode"] == ep["episode"]
        )
        complete, bad = review_native_episode(checked, terminal, basis, gram, 30_000_000)
        native_rows.extend(dict(x, episode=ep["episode"]) for x in complete)
        incomplete.extend(dict(x, episode=ep["episode"]) for x in bad)
        episode_summaries.append(
            dict(ep, complete_results=len(complete), incomplete_results=len(bad))
        )
    ...
    worker_ms = [
        (terminals[i]["completed_ns"] - ranks[i]["completed_ns"]) / 1e6 for i in range(len(ranks))
    ]
    ...
    return dict(
        ...
        stage=status["stage"],
        completed_refills=status["completed_refills"],
        rf_seconds=status["completed_refills"] * 16384 / 2_500_000,
        attempts=status["attempts"],
        handoffs=status["handoffs"],
        reacquisitions=status["reacquisitions"],
        native_results=status["native_results"],
        native_supported=sum(x["rejection"] == 0 for x in native_rows),
        native_completed_runs=status["native_completed_runs"],
        native_incomplete_results=len(incomplete),
        native_episodes=episode_summaries,
        native_complete=native_rows,
        native_incomplete=incomplete,
        observer_measurements=len(observations),
        observer_supported=sum(r.get("accepted", 0) for r in observations),
        accepted_startup_measurements=accepted,
        max_refill_gap_ms=status["max_refill_gap_ns"] / 1e6,
        scan_rank_ms=dict(
            median=statistics.median(scan_ms), p95=percentile(scan_ms, 95), maximum=max(scan_ms)
        ),
        worker_ms=dict(
            median=statistics.median(worker_ms),
            p95=percentile(worker_ms, 95),
            maximum=max(worker_ms),
        ),
        max_candidate_power=dict(
            median=statistics.median(max_power),
            p95=percentile(max_power, 95),
            maximum=max(max_power),
        ),
        sampled_grid_values_checked=numerical,
        sampled_ranking_ffts_checked=ranking_fft,
        native_reference_sha256=reference_sha,
        cdc_drops=0,
        pacer_drops=0,
        artifact_sha256={
            p.name: sha(p) for p in root.iterdir() if p.is_file() and p.name in op["artifacts"]
        },
...
def main():
    runs = [review_run(name) for name in ORDER]
    primary = [r for r in runs if r["primary"]]
    result = dict(
        status="pass",
        scope="five_bounded_30_msps_fpga_tracking_recordings",
        nominal_recording_seconds=300,
        exact_sample_ceiling_seconds=294.912,
        serial="1040005e0b100007100010000bf33a5d4d",
        source_rate=30_000_000,
        acquisition_rate=2_500_000,
        primary_recordings=PRIMARY,
        diagnostic_partial_recordings=[n for n in ORDER if n not in PRIMARY],
        primary_rf_seconds=sum(r["rf_seconds"] for r in primary),
        all_rf_seconds=sum(r["rf_seconds"] for r in runs),
        primary_handoffs=sum(r["handoffs"] for r in primary),
        primary_reacquisitions=sum(r["reacquisitions"] for r in primary),
        primary_native_results=sum(r["native_results"] for r in primary),
        primary_native_supported=sum(r["native_supported"] for r in primary),
        ...
        native_incomplete_heads_checked=sum(r["native_incomplete_results"] for r in runs),
        runs=[],
    )
    for r in runs:
    ...
    x = np.arange(1, 6)
    axes[0].bar(x, [r["rf_seconds"] for r in primary], color=colors)
    axes[0].axhline(294.912, color="black", ls="--", lw=1, label="exact 737,280,000-sample ceiling")
    axes[0].set(ylabel="RF seconds", title="Five nominal 300-second recordings at 30 MS/s")
    axes[0].set_xticks(x)
    axes[0].legend()
    axes[0].grid(axis="y", alpha=0.2)
    width = 0.26
    axes[1].bar(
        x - width, [r["handoffs"] for r in primary], width, label="handoffs", color="#009E73"
    )
    axes[1].bar(
        x, [r["reacquisitions"] for r in primary], width, label="reacquisitions", color="#E69F00"
    )
    axes[1].bar(
        x + width,
        [r["native_completed_runs"] for r in primary],
        width,
        label="1,500-result completions",
        color="#CC79A7",
    )
    axes[1].set(xlabel="Recording", ylabel="Count")
    axes[1].set_xticks(x)
    axes[1].legend()
    axes[1].grid(axis="y", alpha=0.2)
    fig.savefig(HERE / "campaign-overview.png", dpi=180)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(11, 5), layout="constrained")
    offset = 0
    for r in runs:
        power = r["plot"]["max_power"]
        step = r["rf_seconds"] / max(len(power), 1)
        xx = offset + (np.arange(len(power)) + 0.5) * step
        ax.plot(xx, power, lw=0.8, label=r["name"])
        offset += r["rf_seconds"]
        ax.axvline(offset, color="0.75", lw=0.6)
    ax.set(
        xlabel="Cumulative RF seconds (including preserved partials)",
        ylabel="Maximum single-pilot ranking power",
        title="Acquisition opportunity varies sharply across the campaign",
    )
    ax.grid(alpha=0.2)
    ax.legend(fontsize=7, ncol=2)
    fig.savefig(HERE / "acquisition-power.png", dpi=180)
    plt.close(fig)
    ...
    if native:
        axes[0].scatter(
            [x for x, _, _ in native], [y for _, y, _ in native], s=6, alpha=0.55, color="#0072B2"
        )
    if observer:
        axes[1].scatter(
            [x for x, _, _ in observer],
            [y for _, y, _ in observer],
            s=10,
            alpha=0.7,
            color="#009E73",
        )
    for ax, title in zip(
        axes, ("Native FPGA retained estimates", "Simultaneous 2.5-MS/s ARM observer"), strict=True
    ):
        ax.axhline(0.05, color="black", ls="--", lw=1, label="unchanged 0.05 coherence gate")
        ax.set_ylabel("Coherence")
        ax.set_title(title)
        ax.grid(alpha=0.2)
        ax.legend()
    axes[1].set_xlabel("Campaign-relative frame time (separate epochs concatenated)")
    fig.savefig(HERE / "tracking-support.png", dpi=180)
    plt.close(fig)
