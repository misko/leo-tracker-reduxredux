...
from leo.analysis.starlink.adaptive_dual_rx_phase_extract import extract_dual_receiver_phase_with_offset_authority, extract_dual_receiver_phase_with_offset_authority_shared_residual
...
def main():
    ...
    for n in sorted(by_rx[0]):
        ...
        for name,lift in [('pilot_principal',False),('pilot_branch_lifted',True),('pilot_common_authority',True),('pilot_shared_residual',True),('pilot_refined_authority',True)]:
            try:
                if name in ('pilot_common_authority','pilot_shared_residual','pilot_refined_authority'):
                    fn=extract_dual_receiver_phase_with_offset_authority if name=='pilot_common_authority' else extract_dual_receiver_phase_with_offset_authority_shared_residual
                    authority=m.relative_cfo_hz+m.relative_cfo_rate_hz_s*(local+25000-m.reference_sample)/FS
                    obs=fn(chunk,FS,StarlinkEdge.UPPER,candidates[0]['local_epoch_sample'],seeds,authority,common_reference_sample=0,frame_radius=16).observation
                    if name=='pilot_refined_authority':
                        _,obs=refine_shared_pilot(chunk,FS,StarlinkEdge.UPPER,candidates[0]['local_epoch_sample'],seeds,authority,frame_radius=16)
                else:
    ...
    for name,color in [('pilot_principal','tab:red'),('pilot_branch_lifted','tab:purple'),('pilot_common_authority','tab:blue'),('pilot_shared_residual','tab:green'),('pilot_refined_authority','tab:cyan')]:
