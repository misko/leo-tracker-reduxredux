"""One authorized dual-RX transport measurement, bounded by FPGA source time."""
import dataclasses
import json
...
def main():
    ...
    report = dict(
        kind="authorized_dual_rx_10msps_30s_transport_duty",
        sample_rate_hz=RATE, requested_source_seconds=30,
        samples_per_frame=SAMPLES, kernel_buffers=8, receiver_ids=[0, 1],
    ...
    while lease is None:
        try:
            lease = owner.claim((RADIO,), task_id="user-dual10m-30s-duty", task_kind=CaptureTaskKind.QUALIFICATION)
            ...
            try:
                ...
                with radio.begin_metadata_capture(
                    ...
                    while True:
                        ...
                        assert block.samples.shape == (2, SAMPLES)
                    ...
                    report.update(
                        ...
                        received_payload_bytes=len(report["frames"]) * SAMPLES * 8,
                    )
