...
def shape_preserving_order(
    observations: tuple[Observation, ...], *, seed: int, packet_size: int = 3,
    temporal_bins: int = 8,
) -> tuple[str, ...]:
...
"""
    ...
    for track, rows in tracks.items():
        ...
        bin_index = min(temporal_bins-1, (midpoint-start)*temporal_bins//max(1,end-start))
        ...
        while remaining:
            chosen = max(range(len(remaining)), key=lambda i: (
    ...
    track_order = [strata[b][i][1] for i in range(max(map(len, strata.values())))
                   for b in bin_order if i < len(strata[b])]
    output = []
