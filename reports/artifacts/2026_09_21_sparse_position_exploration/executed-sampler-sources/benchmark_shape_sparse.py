...
from benchmark_position_subsets import atomic_write_result, file_hash, read_json, _subset_metrics
    ...
    for packet in (3,5):
        for seed in seeds:
            ...
            for fraction in (1/32,1/16,1/8):
                ...
                subset = PositionSubset("density",fraction,seed,ids,evaluation,unsupported,
                    tuple(sorted(all_tracks-tracks)),f"shape-packet-{packet}-spanweighted-v1")
                for label,settings in (("formal",{}),("gaussian-fixed250",{
                    "gaussian_noise":True,"infer_measurement_sigma":False})):
                    if formal_only and label != "formal":
                        continue
                    job = deepcopy(original)
                    job.pop("job_id")
                    job.update(model=f"shape{packet}-{label}",model_config=settings,
                        subset=asdict(subset),subset_id=subset.subset_id,subset_aliases=[],
                        ...
                        sampling_source_hash=file_hash(root/'src/leo/analysis/research/position_shape_subsets.py'))
                    job["job_id"]=canonical_hash(job)
                    jobs.append(job)
    result={"schema":"position-subset-plan-v1","jobs":jobs,
            "manifest_hash":manifest["manifest_hash"],
            "config_hash":canonical_hash({"packets":[3,5],"seeds":list(seeds),
