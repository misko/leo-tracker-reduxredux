...
def main() -> None:
    ...
    for number in range(1, 14):
        ...
        parent = (
            json.loads(raw_parent.read_text())
            if raw_parent.exists() and raw_parent.stat().st_size
            else operator.get("parent")
        )
        los = operator["los"]
        children = []
        ...
        if retained.exists():
            for child in sorted(retained.glob("visit-*"), key=lambda p: int(p.name[6:])):
                ...
                powers = [
                    value
                    for row in rows
                    if row.get("kind") == "candidate_order"
                ...
                for terminal in (row for row in rows if row.get("kind") == "native_terminal"):
                    ...
                    journal = child / (
                        "native.journal" if episode == 0 else f"native-{episode}.journal"
                    )
                    ...
                    episodes.append(
                        {
                            ...
                            "source_span_s": (
                                (estimates[-1]["frame"] - estimates[0]["frame"]) / 750
                                if len(estimates) > 1
                                else 0
                            ),
    ...
    complete = [
        cycle for cycle in result["cycles"] if cycle["parent"] and cycle["parent"]["track_complete"]
    ]
    episodes = [
        episode
    ...
    result["acceptance"] = {
        ...
        "followup_cycles": sum(
            bool(cycle["parent"] and cycle["parent"]["followup_started"])
            for cycle in result["cycles"]
        ),
        "longest_episode_measurements": max(
            (episode["measurements"] for episode in episodes), default=0
        ),
        "longest_episode_source_span_s": max(
            (episode["source_span_s"] for episode in episodes), default=0
        ),
    ...
    colors = [
        "#16a34a"
        if cycle["parent"]["track_complete"]
        else "#f59e0b"
        if cycle["parent"]["followup_started"]
        else "#94a3b8"
        for cycle in cycles
    ]
    maxima = [
        max(
            (
                child["maximum_single_pilot_power"] or 0
                for child in cycle["children"]
                if child["role"] == "scout"
            ),
            default=0,
    ...
    for index, ((_, episode), span, count) in enumerate(
        zip(episode_rows, spans, supported, strict=True)
    ):
        long = span > 9
        axes[1].text(
            span - 0.08 if long else span + 0.08,
            index,
            f"{episode['measurements']} results; {count} supported",
            ha="right" if long else "left",
            va="center",
            fontsize=8,
