...
def main() -> None:
    ...
    for number in (3, 4, 5, 6):
        ...
        for terminal in terminals:
            ...
            checked = review(
                (root / journal_name).read_bytes(), epoch=terminal["epoch"], rate=30_000_000
            )
    ...
    for run in result["runs"]:
        ...
        for episode in run["episodes"]:
            ...
            annotations.append(
                f"{episode['fpga_results']} results; {episode['native_supported']} native-supported"
            )
    ...
    for index, run in enumerate(result["runs"]):
        ...
        axes[1].text(
            index, durations[index] + 3, f"{run['attempts']} searches", ha="center", fontsize=8
        )
