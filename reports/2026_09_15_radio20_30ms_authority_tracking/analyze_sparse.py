...
def main() -> None:
    ...
    for number in range(2, 10):
        ...
        for terminal in terminals:
            ...
            try:
                ...
                cadence_fields = next(row.payload.decode().split() for row in retained
            ...
            episodes.append({
                ...
                "detection_elapsed_s": ((next(row["window_start"] for row in scans
                                                if row["attempt"] == terminal["attempt"])-initial)
                                         / 2_500_000),
            })
    ...
    result["acceptance"] = {
        ...
        "completed_episodes": sum(episode["measurements"] == 751 and
                                  episode["journal_review"] == "pass" for episode in all_episodes),
        "longest_source_span_s": max((episode["source_span_s"] for episode in all_episodes), default=0),
        "passed": False,
    }
    ...
    for run in result["runs"]:
        if not run["episodes"]:
            labels.append(f"{run['run']} (no handoff)"); spans.append(0); supported.append(0)
            review_passed.append(True)
        for episode in run["episodes"]:
            labels.append(f"{run['run']} ep {episode['episode']}")
            spans.append(episode["source_span_s"]); supported.append(episode["supported"])
            review_passed.append(episode["journal_review"] == "pass")
    y = range(len(labels))
    axes[0].barh(list(y), spans, color=["#2563eb" if passed else "#f59e0b"
                                       for passed in review_passed])
    axes[0].axvline(10, color="#b91c1c", linestyle="--", label="10 s target")
    axes[0].set_yticks(list(y), labels); axes[0].invert_yaxis(); axes[0].set_xlim(0, 10.7)
    ...
    for index, (span, count) in enumerate(zip(spans, supported, strict=True)):
        axes[0].text(span+0.08, index, f"{span:.3f} s; {count} supported", va="center", fontsize=8)
    ...
    for index, run in enumerate(result["runs"]):
        ...
        axes[1].text(index, duration[index]+2, f"{run['attempts']} searches", ha="center", fontsize=8)
