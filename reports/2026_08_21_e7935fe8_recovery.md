...
- aliases -1, +1, and +2 are not retained because they degrade the independently searched
  baseline well beyond the calibrated equivalence interval, have weak absolute evidence,
  and each contains a harmful block;
...
| Exact V2 final candidate / automatic | 1 / 0 | alias 0 display-only; no correction |
...
V2 reduces correlated probes to one-second block medians and records geometry, replay
coverage, relative equivalence, absolute evidence, and harmful-tail behavior separately.
...
The frozen null-control p95 absolute delta is `0.00019`; its explicit `2×` safety factor
gives an equivalence interval of ±`0.00038`. Alias 0 is equivalent to the independently
optimized baseline and has zero harmful blocks. It is above the explicit display-only
absolute floor of `0.0025`, but far below the unchanged automatic-correction floor of
`0.05`. The correct label is therefore `geometry_only`, not `replay_stable` and not
correction-safe.
...
|---|---|---|
| Automatic correction | credible geometry; adequate probes/blocks/coverage; corrected margin ≥0.05; relative replay improved or equivalent; harmful-tail guard passes | retain every qualifying observed lift and permit correction |
| Display-only fallback | branch has no automatic lift; `geometry_only`; adequate probes/blocks/coverage; zero harmful blocks and run; median delta ≥ -equivalence tolerance; corrected margin ≥0.0025 | retain at most one alias, explicitly nonautomatic |
...
`ReplayGateConfigV2` field. It is not derived from the `0.05` correction floor and is not
...
Machine-readable funnel facts and the exact current V2 contracts are archived beside
the figures:
...
Standard and Research both use the shared receiver runner, so both now produce replay,
final-bank, and final-table V2 products. Research retains its independent probe geometry;
...
- positive improved and stable automatic tracks;
- optimizer-equivalent geometry-only recovery;
- exactly one non-degrading fallback from aliases -1, 0, +1, +2;
...
- V2 codec, application projection, final PNG, and UI labels/flags;
...
| Check | Result |
|---|---|
| CFO de-alias component tests | 37 passed |
| Focused runner/config/presentation tests | 58 passed |
| Downstream Standard/Research/topology/fan-in slice | 15 passed |
...
| Exact lower-edge same-IQ replay | passed, 11 lifts; about 12 minutes |

The same-capture wrong-edge replay was not repeated because each exact 60-second,
11-lift replay costs about 12 minutes. Wrong-edge behavior remains covered by the frozen
named control and component red test; the actual alternate aliases on this exact capture
also fail the independent relative, absolute, and harmful-tail controls. This report does
not claim a measured wrong-edge false-positive rate for this one capture.
