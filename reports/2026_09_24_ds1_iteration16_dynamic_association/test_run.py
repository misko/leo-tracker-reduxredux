...
def module():
    ...
    spec = importlib.util.spec_from_file_location("iteration16_run_test", path)
...
def test_lattice_is_symmetric_and_edge_rule_is_reference_free() -> None:
    ...
    assert len(rows) == 9
    assert {row["east_km_from_parent"] for row in rows} == {1.5, 2.0, 2.5}
    assert {row["north_km_from_parent"] for row in rows} == {-3.5, -3.0, -2.5}
...
def test_evaluator_distance_is_zero_at_reference() -> None:
    ...
    spec = importlib.util.spec_from_file_location("iteration16_evaluate_test", path)
    assert spec and spec.loader
