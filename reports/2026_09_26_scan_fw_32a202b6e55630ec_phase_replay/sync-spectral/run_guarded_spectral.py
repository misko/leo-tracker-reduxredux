...
def dense_seed(visit: int, directory: Path):
    path = directory / f"visit-{visit:06d}.corrected-dense.json"
    if not path.exists():
        return None, "pending_dense_acquisition"
    probes = json.loads(path.read_text())["product"]["probes"]
    best = {}
    for probe in probes:
        passing = [
            candidate
        ...
        if passing:
            best[probe["probe_index"], probe["receiver_id"]] = min(
                passing,
    ...
    for index in range(3):
        if (index, 0) in best and (index, 1) in best:
            return tuple(
                float(best[index, rx]["fractional_tracking_cfo_hz"]) for rx in (0, 1)
            ), f"dense_probe_{index}_within_training_prefix"
    return None, "no_training_half_passing_dual_rx_probe"


def choose_alias(raw, starts, training, centers):
    offsets = np.arange(-4, 5) / 4.4e-6
    scores = []
    for offset in offsets:
        _, coherence, _ = HB.time_phasors(
            raw, RATE, centers[1] - centers[0] + offset, 0.0, starts[training]
        )
        scores.append(float(np.median(coherence)))
    selected = int(np.argmax(scores))
    return (centers[0], centers[1] + float(offsets[selected])), {
        "selected_symbol_alias": selected - 4,
        "training_median_coherence_by_alias": scores,
    }


def run(output: Path, dense: Path | None = None) -> None:
    ...
    with output.open("a") as stream:
        for item in json.loads(selection.read_text())["visits"]:
            ...
            row["schema"] = "scan-guarded-spectral/v5"
            began = time.monotonic()
            centers = (
                (cfo[visit, 0], cfo[visit, 1]) if (visit, 0) in cfo and (visit, 1) in cfo else None
            )
            seed_authority = "sparse_first_probe"
            if dense is not None:
                centers, seed_authority = dense_seed(visit, dense)
            if seed_authority == "pending_dense_acquisition":
                continue
            row["seed_authority"] = seed_authority
            if centers is None:
                row.update(
                    status="unsupported", reason="no_training_half_passing_dual_rx_acquisition"
                )
            else:
                ...
                for name, (train, held) in partitions.items():
                    physical_centers, aliases = choose_alias(raw, starts, train, centers)
                    recentered = raw * np.exp(
                        -2j * np.pi * times[:, None] * np.array(physical_centers)[None, :]
                    )
                    row["alias_selection"][name] = aliases
                    row["native"].update(
                        evaluate(recentered, RATE, starts, {name: (train, held)}, physical_centers)
                    )
                    row["derived_2p5"].update(
                        evaluate(
                            HB.decimate_four(recentered),
                            RATE // 4,
                            starts // 4,
                            {name: (train, held)},
                            physical_centers,
