#!/usr/bin/env python3
"""Aggregate the frozen blind tau-zero search over the full 00Z TRAIN block."""
...
def load_fast_loader(path):
    spec = importlib.util.spec_from_file_location("long_training_fast_loader", path)
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)
...
def run(args):
    ...
    training = manifest["source_group"]["session_ids"]
    if (
        len(training) != 72
        or manifest["source_group"]["utc_8h_start"] != "2026-09-21T00:00:00+00:00"
    ):
        raise ValueError("expected frozen full Sep21 00Z TRAIN group")
    fast_loader = load_fast_loader(args.fast_loader)
    prepared_started = time.monotonic()
    sessions = [
        fast_loader.load_session(single, args.cache_root / session, session) for session in training
    ]
    preparation_s = time.monotonic() - prepared_started
    started = time.monotonic()

    def evaluate(latitude, longitude, held=False):
        return combined_score(single, sessions, latitude, longitude, held)

    searches = [single.search_prior(name, prior, evaluate) for name, prior in single.PRIORS.items()]
    ...
    for search in results["searches"]:
        ...
        for scan, source in zip(scans, sessions, strict=True):
            for track, evidence in zip(scan["tracks"], source["prepared"], strict=True):
                ...
                total_weight += weight
        point["scans"] = scans
        point["reserved_capped800_rmse_hz"] = float(np.sqrt(capped_loss / total_weight))
        point["reserved_uncapped_rmse_hz"] = float(np.sqrt(uncapped_loss / total_weight))
