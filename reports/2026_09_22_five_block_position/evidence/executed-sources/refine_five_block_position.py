...
def seal_refinement(
    output: Path, acquisition_run: Path, result: dict, acquisition_receipt: dict
) -> None:
    path = output / "result.json"
    checksum_path = output / "result.sha256"
    fits = result.get("fits", [])
    if (
        result.get("complete") is not True
        or result.get("position_truth_used") is not False
        or not fits
        or not all(fit.get("converged") is True for fit in fits)
    ):
        raise ValueError("only complete converged truth-free refinements may be sealed")
    seal = {
        "schema": "regional-five-block-refinement-seal/v1",
        "acquisition_seal_digest": digest(acquisition_run / "acquisition-seal.json"),
        "adapter_digest": result["five_block_adapter_digest"],
        "all_fits_converged": True,
        "base_refiner_digest": result["base_refiner_digest"],
        "fits": len(fits),
        "numerical_core_digest": result["numerical_source_digest"],
        "observation_count": result["observation_count"],
        "partition_receipt_digest": acquisition_receipt["content_digest"],
        "position_truth_used": False,
        "result_checksum_digest": digest(checksum_path),
        "result_digest": digest(path),
        "sessions": result["sessions"],
        "wrapper_digest": result["five_block_wrapper_digest"],
    }
    (output / "refinement-seal.json").write_text(
...
def run(args) -> None:
    ...
    checksum_path = args.output / "result.sha256"
    checksum_path.write_text(hashlib.sha256(payload.encode()).hexdigest() + "\n")
    seal_refinement(args.output, args.run, result, acquisition_receipt)
