...
import leo.analysis.research.regional_doppler as regional_doppler
...
def write_execution_receipt(args, replay_path: Path, receipt: dict[str, object]) -> None:
    """Bind a completed acquisition to its command, sources, and partition."""
    ...
    execution = {
        "schema": "regional-five-block-execution-receipt/v1",
        "base_git_commit": commit,
        "command": shlex.join([Path(sys.executable).name, *sys.argv]),
        "configuration_digest": "sha256:"
        + hashlib.sha256((args.output / "configuration.json").read_bytes()).hexdigest(),
        "frozen_adapter_digest": "sha256:"
        + hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        "frozen_base_replay_digest": "sha256:"
        + hashlib.sha256(replay_path.read_bytes()).hexdigest(),
        "numerical_core_digest": "sha256:"
        + hashlib.sha256(Path(regional_doppler.__file__).read_bytes()).hexdigest(),
        "partition_receipt_digest": receipt["content_digest"],
        "position_truth_used": False,
        "result_digest": "sha256:"
        + hashlib.sha256((args.output / "result.json").read_bytes()).hexdigest(),
    ...
    _write(args.output / "execution-receipt.json", execution)
...
def run(args) -> None:
    ...
    write_execution_receipt(args, replay_path, receipt)
