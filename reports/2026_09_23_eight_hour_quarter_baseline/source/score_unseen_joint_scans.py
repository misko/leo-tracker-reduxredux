...
def orbit_phase(row, fitted_rate, mode):
    """Ablate only orbit corrections at the same frozen receiver position."""
    if mode == "original":
        return 0.0
    if mode == "causal_mean":
        return row["nominal_phase_s"]
    if mode == "fitted":
        return row["nominal_phase_s"] + fitted_rate * row["causal_age_h"]
    raise ValueError("unknown orbit ablation mode")
...
def run(args):
    ...
    mode = getattr(args, "orbit_mode", "fitted")
    orbit_phase({"nominal_phase_s": 0.0, "causal_age_h": 0.0}, 0.0, mode)
    ...
    result = {
        ...
        "unseen_norad_policy": (
            "original orbit" if mode == "original" else "causal mean with zero residual rate"
        ),
        "orbit_mode": mode,
        "comparison_scope": "orbit ablation at identical frozen fitted position",
    }
    for scan in scans:
        ...
        for episode_id, arc in loader(document, max_per_partition=0):
            ...
            for j, (index, norad) in enumerate(zip(indices, norads, strict=True)):
                row = rows[int(norad)]
                phase = orbit_phase(row, rates.get(int(norad), 0.0), mode)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--orbit-mode", choices=["fitted", "causal_mean", "original"], default="fitted"
    )
