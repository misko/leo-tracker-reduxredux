...
runtime = Path(
    "/opt/leo-tracker/releases/d9b1e6dd486918cf8bd8876870cfabc0ceab11f6/runtime/scanner-glrt"
)
authority = LocalCaptureAuthority(
    Path("/srv/bulk/leo/control"), (RadioResource(radio_id, serial, "ip:192.168.1.20"),)
)
with authority.claim((radio_id,), task_id=session_id, task_kind=CaptureTaskKind.SCANNER_SWEEP):
    ...
    try:
        ...
        target = before.model_copy(
            update={"channels": (0, 1), "gain_mode": GainMode.MANUAL, "gain_db": 40.0}
        )
        after = radio.apply_settings(target)
        assert after.gain_mode is GainMode.MANUAL and after.gain_db == 40.0
        (root / "manual-gain-baseline.json").write_text(
            json.dumps(
                {
    ...
    lifecycle = UserspaceIiodDeployment(
        ...
        binary_path=runtime / "iiod",
        bundle_manifest_path=runtime / "bundle.json",
    ...
    def cleanup():
        ...
        command = [
            "sshpass",
            "-f",
            str(credentials / "scanner-iiod-ssh-password"),
            "ssh",
            "-o",
    ...
    try:
        result = run_persistent_hop_canary(
