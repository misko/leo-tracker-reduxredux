#!/usr/bin/env python3
"""Plot paired iteration-30 preflight evidence without reference coordinates."""
...
def main() -> None:
    ...
    for column, dataset in enumerate(("DS1", "DS3")):
        ...
        axes[0, column].set_title(f"{dataset} bounded RF surface")
        ...
        axes[1, column].set_title(
            f"{dataset} candidate retention · T={summary['temperature_hz']:.0f} Hz"
        )
        axes[1, column].set_ylabel("Tracks")
        axes[1, column].grid(axis="y", alpha=0.25)
    fig.suptitle(
        "Iteration 30 paired preflight · both arms unqualified · no full search",
        fontsize=15,
    )
