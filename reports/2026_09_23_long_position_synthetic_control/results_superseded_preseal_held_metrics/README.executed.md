...
The exact noiseless recovery rules out a basic coordinate transform, Doppler
sign, direct-SGP4 state, CFO elimination, or local optimizer failure on this
support. It also shows that the two distant starting centres reach the same
solution. The 300 Hz draw landed at 370 m, just outside 300 m, and the added
0.3 s satellite epoch uncertainty increased error to 1.48 km. These outcomes
are controls for sensitivity, not substitutes for real measurements. They
support treating the much larger real-data error as data/model mismatch while
showing that sub-300 m is not robust to the tested noise realization.
