#!/usr/bin/env python3
"""Reference-free exact replay and objective-consistency gate for iteration 13."""
...
def coordinate(row: dict[str, Any]) -> tuple[float, float]:
    return (
        round(float(row["east_km_from_iteration12"]), 10),
        round(float(row["north_km_from_iteration12"]), 10),
    )


def objective_agreement(steps: list[dict[str, Any]]) -> list[dict[str, Any]]:
    output = []
    for step in steps:
        regularized = step["winner"]
        raw = min(
        ...
        output.append(
            {
                ...
                "regularized_winner": coordinate(regularized),
                "raw_cap800_winner": coordinate(raw),
                "agrees": coordinate(regularized) == coordinate(raw),
                "raw_cap800_margin": sorted(
                    row["balanced_exact_capped_loss"]
                    for row in step["ranked_coordinates"]
                )[1]
                - raw["balanced_exact_capped_loss"],
            }
        )
    return output
...
def main() -> None:
    ...
    driver = load_module(DRIVER, "i13_qualify_driver")
    orbit = load_module(ORBIT, "i13_qualify_orbit")
    ...
    agreement = objective_agreement(inference["steps"])
    qualified = bool(
    ...
    output = {
        "schema": "ds1-iteration13-basin-closure-qualification/v1",
        "reference_used": False,
        "inference": {"path": str(args.inference), "sha256": digest(args.inference)},
        "objective_agreement": agreement,
        "all_steps_raw_and_regularized_agree": all(row["agrees"] for row in agreement),
        "exact_replay_gates": gates,
        "qualified": qualified,
        "qualification_rule": (
            "the planned basin closes on an interior final cell, both group fits converge, "
            "and both direct exact-SGP4 replay gates pass at 0.2 Hz"
        ),
    }
