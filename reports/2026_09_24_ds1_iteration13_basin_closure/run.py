#!/usr/bin/env python3
"""Close the reference-free exact-rate basin beyond DS1 iteration 12."""
...
def combine(
    ...
    for point in points:
        ...
        fits = [row["rate_only"] for row in sorted(grouped, key=lambda row: row["group_id"])]
        rows.append(
            {
                **point,
                ...
                * sum(float(fit["exact_full_observation_capped_loss"]) for fit in fits),
                "all_converged": all(bool(fit["converged"]) for fit in fits),
                "rate_boundary_count": sum(int(fit["rate_boundary_count"]) for fit in fits),
            }
        )
...
def main() -> None:
    ...
    output = {
        "schema": "ds1-iteration13-basin-closure/v1",
        "complete": complete,
