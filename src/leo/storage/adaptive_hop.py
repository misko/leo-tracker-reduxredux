...
class Feature104AdaptiveHopIqChunkV10(AdaptiveHopIqChunkV1):
...
class Feature104DualRxAdaptiveHopIqManifestV10(GeometryBoundAdaptiveHopIqManifestV6):
    """Multirate dual-RX IQ including the 15 MS/s production rate."""

    schema_version: Literal[10] = 10  # type: ignore[assignment]
    _visits_per_chunk: ClassVar[int] = 4
    _timing_model: ClassVar[type[PersistentHopUtcTimingAuthorityV1]] = Feature104DualRxTimingV4
    receipt: AdaptiveHopReceiptV5  # type: ignore[assignment]
    timing: Feature104DualRxTimingV4 | None  # type: ignore[assignment]
    chunks: Annotated[tuple[Feature104AdaptiveHopIqChunkV10, ...], Field(max_length=625)]


class VariableDualRxAdaptiveHopIqChunkV11(AdaptiveHopIqChunkV1):
    schema_version: Literal[11] = 11  # type: ignore[assignment]
    chunk_index: Annotated[int, Field(strict=True, ge=0, le=624)]
    visit_count: Annotated[int, Field(strict=True, ge=1, le=4)]
    sample_count: Annotated[int, Field(strict=True, gt=0, le=14_400_000)]


class VariableDualRxAdaptiveHopIqManifestV11(GeometryBoundAdaptiveHopIqManifestV6):
    """Protocol-three dual-RX IQ with exact variable visit lengths."""
...
class UnboundFourRateVariableDualRxAdaptiveHopIqManifestV15(AdaptiveHopIqManifestV1):
    """Four-rate protocol-three IQ without established fixture geometry."""

    schema_version: Literal[15] = 15  # type: ignore[assignment]
    _visits_per_chunk: ClassVar[int] = 1
    _timing_model: ClassVar[type[PersistentHopUtcTimingAuthorityV1]] = FourRateDualRxTimingV5
    receipt: AdaptiveHopReceiptV7  # type: ignore[assignment]
    timing: FourRateDualRxTimingV5 | None  # type: ignore[assignment]
    chunks: Annotated[tuple[VariableDualRxAdaptiveHopIqChunkV12, ...], Field(max_length=2500)]
...
class AdaptiveHopIqReader:
    ...
    def read_visit_ci16(self, visit_index: int) -> tuple[AdaptiveHopVisitV1, npt.NDArray[np.int16]]:
        ...
        start = sum(visit.valid_sample_count for visit in visits[:local])
        return visits[local], values[start : start + count]
...
class AdaptiveHopSessionWriter:
    def __init__(
        ...
        *,
        ...
        self._closed = False
        self._failed = False
        self._visits: list[AdaptiveHopVisitV1 | AdaptiveHopVisitV2 | AdaptiveHopVisitV3] = []
    ...
    def append(self, block: AdaptiveHopVisitBlock | HostAdaptiveHopVisitBlock) -> None:
        ...
        try:
            ...
            e = visit.event
            sparse_source = isinstance(self._plan, (HostAdaptiveHopPlanV2, AdaptiveHopPlanV5))
            if (
                (
    ...
    def _finish_chunk(self) -> None:
        ...
        chunk_model = (
            ...
            else Feature103AdaptiveHopIqChunkV9
            if self._feature103
        ...
        self._chunks.append(
            chunk_model(
                ...
                uncompressed_bytes=chunk_sample_count * self._bytes_per_sample,
