"""Immutable blind regional evidence in its own local namespace."""

import fcntl
import os
import stat
from contextlib import contextmanager, suppress
from pathlib import Path

from pydantic import TypeAdapter

from leo.contracts.blind_regional import (
    ArtifactName,
    BlindArtifactV1,
    BlindRegionalDocumentV1,
    BlindRegionalManifestV1,
    BlindRegionalStatusV1,
    SessionId,
)
from leo.contracts.digests import canonical_json_bytes, sha256_digest
from leo.storage.adaptive_hop_analysis import _publish, _read, _seal, _unseal
from leo.storage.pinned import PinnedLocalRoot

NAMES: tuple[ArtifactName, ...] = (
    "blind-association",
    "blind-position",
    "blind-position-modes",
)
_LIMIT = 16 * 1024 * 1024


class BlindRegionalStore:
    def __init__(self, root: Path, *, read_only: bool = True):
        resolved = root.resolve()
        if resolved == Path("/mnt/qnap01") or Path("/mnt/qnap01") in resolved.parents:
            raise ValueError("blind regional evidence cannot use QNAP")
        self.root, self.read_only = root, read_only

    @contextmanager
    def _directory(self, session_id: str, *, create: bool = False):
        TypeAdapter(SessionId).validate_python(session_id)
        if create and self.read_only:
            raise PermissionError("blind regional store is read-only")
        root = PinnedLocalRoot(self.root)
        directory = None
        try:
            directory = root.child("scanner-blind-regional-v1", session_id, create=create)
            yield directory
        finally:
            if directory is not None:
                directory.close()
            root.close()

    def status(self, session_id: str) -> BlindRegionalStatusV1:
        try:
            with self._directory(session_id) as directory:
                manifest = _unseal(
                    _read(directory, "manifest.json", _LIMIT), BlindRegionalManifestV1
                )
                raw = _read(directory, "document.json", _LIMIT)
            if (
                raw != canonical_json_bytes(manifest.document.model_dump(mode="json"))
                or sha256_digest(raw) != manifest.document_sha256
            ):
                raise ValueError("blind regional document digest differs")
            return BlindRegionalStatusV1(session_id=session_id, state="complete", manifest=manifest)
        except FileNotFoundError:
            return BlindRegionalStatusV1(session_id=session_id)
        except ValueError as error:
            if isinstance(error.__cause__, FileNotFoundError):
                return BlindRegionalStatusV1(session_id=session_id)
            raise

    @contextmanager
    def writer(self, session_id: str):
        with self._directory(session_id, create=True) as directory:
            descriptor = os.open(
                ".writer.lock",
                os.O_RDWR | os.O_CREAT | os.O_NOFOLLOW,
                0o640,
                dir_fd=directory.fileno(),
            )
            try:
                info = os.fstat(descriptor)
                if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1 or info.st_size:
                    raise ValueError("blind regional writer lock is invalid")
                fcntl.flock(descriptor, fcntl.LOCK_EX)
                yield self
            finally:
                fcntl.flock(descriptor, fcntl.LOCK_UN)
                os.close(descriptor)

    def publish(self, document: BlindRegionalDocumentV1, images: dict[str, bytes]):
        if self.read_only:
            raise PermissionError("blind regional store is read-only")
        if tuple(images) != NAMES:
            raise ValueError("blind regional image inventory differs")
        raw = canonical_json_bytes(document.model_dump(mode="json"))
        refs = []
        for name in NAMES:
            payload = images[name]
            if not payload.startswith(b"\x89PNG\r\n\x1a\n"):
                raise ValueError("blind regional artifact is not PNG")
            refs.append(
                BlindArtifactV1(name=name, sha256=sha256_digest(payload), byte_count=len(payload))
            )
        manifest = BlindRegionalManifestV1(
            document=document, document_sha256=sha256_digest(raw), artifacts=tuple(refs)
        )
        with self._directory(document.session_id, create=True) as directory:
            try:
                existing = _read(directory, "manifest.json", _LIMIT)
            except FileNotFoundError:
                existing = None
            sealed = _seal(manifest)
            if existing is not None:
                if existing != sealed:
                    raise ValueError("immutable blind regional publication conflict")
                return manifest
            members = [("document.json", raw), *((name + ".png", images[name]) for name in NAMES)]
            for member_name, payload in members:
                temporary = f".{member_name}.{os.getpid()}.partial"
                try:
                    _publish(directory, temporary, payload, _LIMIT)
                    os.replace(
                        temporary,
                        member_name,
                        src_dir_fd=directory.fileno(),
                        dst_dir_fd=directory.fileno(),
                    )
                finally:
                    with suppress(FileNotFoundError):
                        os.unlink(temporary, dir_fd=directory.fileno())
            _publish(directory, "manifest.json", sealed, _LIMIT)
            os.fsync(directory.fileno())
            return manifest

    def artifact(self, session_id: str, name: ArtifactName) -> bytes | None:
        status = self.status(session_id)
        if status.manifest is None:
            return None
        ref = next(x for x in status.manifest.artifacts if x.name == name)
        with self._directory(session_id) as directory:
            payload = _read(directory, name + ".png", _LIMIT)
        if len(payload) != ref.byte_count or sha256_digest(payload) != ref.sha256:
            raise ValueError("blind regional PNG digest differs")
        return payload
