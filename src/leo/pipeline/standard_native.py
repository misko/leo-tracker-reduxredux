...
STANDARD_NATIVE_SAMPLE_RATES_HZ = (2_500_000, 3_000_000, 5_000_000)
...
def compile_standard_native_scope_inventory(
    ...
    """Build V3 logical-span scopes without reinterpreting the V1/V2 helper."""

    if not isinstance(manifest, RecordingManifestV3):
        raise ValueError("Standard-native scope inventory requires a V3 recording")
    ordered = tuple(
...
def _require_reviewed_native_geometry(
    ...
    if not isinstance(manifest, RecordingManifestV3):
        raise ValueError("Standard-native expanded runs require a V3 device-axis recording")
