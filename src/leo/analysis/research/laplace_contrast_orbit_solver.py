"""Gaussian contrast solver with a bounded GN-Laplace phase-rate integral.
...
class LaplaceContrastOrbitConfig:
    ...
    max_nfev: int = 500
    nuisance_max_iterations: int = 120
    nuisance_tolerance: float = 1e-3
    nuisance_min_step: float = 2**-12
    ...
    def __post_init__(self):
        ...
        if self.nuisance_max_iterations < 1 or not (0 < self.nuisance_tolerance < 1):
            raise ValueError("invalid nuisance convergence setting")
        if not (0 < self.nuisance_min_step <= 1):
            raise ValueError("invalid nuisance line-search setting")
        if not self.gaussian_noise:
            raise ValueError("offset marginalization is implemented only for Gaussian noise")


@dataclass(frozen=True)
class LaplaceContrastOrbitData:
    y_hz: np.ndarray
    training: np.ndarray
    segment: np.ndarray
    track: np.ndarray
...
@dataclass(frozen=True)
class LaplaceContrastOrbitResult:
    ...
    approximation: str = (
        "quadratic phase states; Gaussian offsets analytically marginalized; "
        "phase rates integrated by bounded Gauss-Newton Laplace approximation"
    )
    optimizer_restarts: int = 0
    nuisance_iterations: int = 0
    nuisance_backtracks: int = 0
    nuisance_scaled_step: float = float("inf")
    nuisance_termination: str = "not-run"
    nuisance_trace: list[dict[str, float | int]] | None = None
...
def bounded_laplace_adjustment(rate, curvature, bound, prior_sigma):
    """Negative log integral of the local quadratic around a MAP rate."""
    rate = np.asarray(rate, float)
    curvature = np.asarray(curvature, float)
    root = np.sqrt(curvature)
    mass = ndtr((bound - rate) * root) - ndtr((-bound - rate) * root)
...
def fit_laplace_contrast_orbit(
    data: LaplaceContrastOrbitData,
    region: Region,
    initial_x_km,
    config: LaplaceContrastOrbitConfig | None = None,
    fitting_mask=None,
):
    config = config or LaplaceContrastOrbitConfig()
    n = len(data.y_hz)
    ...
    def profiled(value, details=False):
        ...
        offset_unscaled = np.asarray(whitening @ np.ones(len(rows)))
        offset_a = offset_unscaled / sigma
        ...
        nuisance_iterations = 0
        nuisance_backtracks = 0
        nuisance_scaled_step = float("inf")
        nuisance_termination = "iteration-limit"
        nuisance_trace = []
        ...
        for iteration in range(1, config.nuisance_max_iterations + 1):
            nuisance_iterations = iteration
            ...
            while step >= config.nuisance_min_step:
                new = beta + step * (proposal - beta)
                new_loss = nuisance_loss(new)
                if np.isfinite(new_loss) and new_loss <= current_loss + 1e-10:
                    break
                step *= 0.5
                nuisance_backtracks += 1
        ...
        projected = np.bincount(src, phase_d**2, minlength=len(sources)) - np.bincount(
            segment_source, cross**2 / design_norm, minlength=len(sources)
        )
        ...
        nlp = (
            ...
            + 0.5 * logdet
            + 0.5 * np.sum(np.log(design_norm2))
            + 0.5 * np.sum(r * r)
    ...
    return LaplaceContrastOrbitResult(
        ...
        nuisance_iterations=nuisance_iterations,
        nuisance_backtracks=nuisance_backtracks,
        nuisance_scaled_step=nuisance_scaled_step,
        nuisance_termination=nuisance_termination,
