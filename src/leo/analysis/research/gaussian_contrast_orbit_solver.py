"""Gaussian sparse-orbit solver with marginalized offsets and MAP phase rates.
...
class GaussianContrastOrbitConfig:
...
@dataclass(frozen=True)
class GaussianContrastOrbitData:
    y_hz: np.ndarray
    training: np.ndarray
    segment: np.ndarray
    track: np.ndarray
...
@dataclass(frozen=True)
class GaussianContrastOrbitResult:
    ...
    approximation: str = (
        ...
        "phase rates iteratively relinearized and profiled by MAP"
    )
...
def fit_gaussian_contrast_orbit(
    data: GaussianContrastOrbitData,
    region: Region,
    initial_x_km,
    config: GaussianContrastOrbitConfig | None = None,
    fitting_mask=None,
):
    config = config or GaussianContrastOrbitConfig()
    n = len(data.y_hz)
