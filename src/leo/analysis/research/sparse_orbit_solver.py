"""Stabilized sparse-data variant of the formal fixed-identity orbit solver.
...
def fit_sparse_orbit(
    data: SparseOrbitData,
    region: Region,
    initial_x_km,
    config: SparseOrbitConfig | None = None,
    fitting_mask=None,
):
    config = config or SparseOrbitConfig()
    n = len(data.y_hz)
    ...
    def profiled(value, details=False):
        ...
        def nuisance_loss(candidate):
            ...
            likelihood = (
                0.5 * np.sum(candidate_r**2)
                if config.gaussian_noise
                else 0.5 * (config.robust_df + 1)
                * np.sum(np.log1p(candidate_r**2 / config.robust_df))
            )
            return float(
                likelihood
