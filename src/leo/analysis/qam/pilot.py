...
class PilotFrameCfoEvenEvidence:
    ...
    The caller supplies one guarded full-frame IQ span, so odd-time samples can
    be present in memory.  This deliberately has no odd-fold fields and only
    demodulates or scientifically evaluates/scores the zero-based even Qin
    symbols.  It is the narrow entry point for cohort construction and other
    protocol work that must not compute a held-out response before the
    evaluation mask is frozen.
    """
...
def estimate_edge_pilot_frame_cfo_even_evidence(
    ...
    *,
    ...
    The guarded input remains a full-frame array.  Upstream acquisition, source,
    alias, and epoch conditioning are outside this frame-local API and may have
    been formed from all-Qin evidence.
    """
...
class _KnownPilotDemodulator:
    ...
    def even_frame(self, frame_start: int) -> np.ndarray:
        """Demodulate only zero-based even Qin positions."""

        result = np.empty((150, 8), dtype=np.complex128)
        for positions, relative, solves in _known_pilot_layout(float(self._rate), self._edge):
            selected = positions % 2 == 0
            selected_positions = positions[selected]
            absolute = frame_start + relative[selected]
            values = np.asarray(self._samples[absolute], dtype=np.complex128)
            values *= np.exp(-2j * np.pi * self._cfo * absolute / self._rate)
