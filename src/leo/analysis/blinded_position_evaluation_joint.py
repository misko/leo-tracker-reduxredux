"""Reveal-only evaluation for partial joint-correction positioning."""
...
from leo.contracts.satellite_pnt_joint_challenge import JointCorrectionNavigationLane
...
class JointCorrectionBlindedPositionEvaluation:
    ...
    lane: JointCorrectionNavigationLane
    ...
    evaluation_algorithm: str = field(
        default="blinded-position-joint-correction-reveal-evaluation-v1", init=False
    )
    ...
    def __post_init__(self) -> None:
        if not self.modes or self.reported_mode != self.modes[0]:
            raise BlindedPositionEvaluationError(
                "joint evaluation requires its rank-one reported mode"
            )
        if self.target_likelihood_compared_to_unresolved:
            raise BlindedPositionEvaluationError(
                "partial joint evaluation cannot claim a null likelihood comparison"
            )
        if not math.isclose(
            ...
            raise BlindedPositionEvaluationError(
                "joint evaluation posterior accounting is not closed"
            )
...
def evaluate_blinded_position_joint_correction_reveal(
    receipt: BlindedPositionJointCorrectionRevealReceiptV3,
) -> JointCorrectionBlindedPositionEvaluation:
    """Recompute errors while retaining all unresolved prior mass."""

    receipt = BlindedPositionJointCorrectionRevealReceiptV3.model_validate(
    ...
    if returned_mass <= 0.0:
        raise BlindedPositionEvaluationError("joint returned posterior mass is zero")
    conditional_mean = (
    ...
    return JointCorrectionBlindedPositionEvaluation(
