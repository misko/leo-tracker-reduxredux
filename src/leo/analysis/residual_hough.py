"""Bounded residual-Hough proposal grouping for persisted CFO evidence.
...
returned line.  The implementation is infrastructure-blind so Standard and
Research pipeline adapters use the same deterministic segmentation.
"""
...
def hough_config_from_contract(config: AlternateCfoLineFinderConfigV1) -> HoughConfig:
    """Project the persisted V1 proposal policy into the infrastructure-free detector."""

    from leo.analysis.cfo_lines import LineDetectionConfig
...
def select_residual_hough_partition(
    *,
    ...
    for raw_partition in _partitions(tuple(range(len(considered)))):
        ...
        # Exact synthetic lines legitimately have zero floating-point residual.
        # A scale floor keeps MDL finite; the explicit split cost then selects
        # the simpler exact model rather than making the production stage fail.
        robust_sad = max(robust_sad, np.finfo(float).tiny * observation_count)
        gaussian_sse = max(gaussian_sse, np.finfo(float).tiny * observation_count)
        robust_mdl = float(
...
def detect_all_residual_hough_lines(
    *,
    points: tuple[CfoPoint, ...],
    hough_config: HoughConfig,
    selection_config: ResidualHoughSelectionConfig = _DEFAULT_SELECTION_CONFIG,
    ...
    tuple[LineSegment, ...],
    tuple[tuple[LineSegment, ResidualHoughSelection], ...],
]:
    """Peel strongest initial parents and refine each bounded parent in order."""

    parents = weighted_hough_lines(points, hough_config)
    refined = tuple(
        (
            parent,
        ...
        if parent.support <= selection_config.maximum_parent_support
    )
    return parents, refined
