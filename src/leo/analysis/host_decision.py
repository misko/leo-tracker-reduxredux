...
class _Result(ct.Structure):
    _fields_ = [
        *[(name, ct.c_uint32) for name in (
            "version", "outcome", "screen_mask", "confirmation_mask", "supported_start",
            "supported_end", "candidate_supported", "reserved",
        )],
        ("epoch", ct.c_int32), ("fractional_complete", ct.c_int32),
        *[(name, ct.c_double) for name in (
            "fractional_offset", "cfo_hz", "exact_score", "margin", "filter_cpu_ms",
            "cpu_ms", "wall_ms",
        )],
...
class NativeHostDecision:
    ...
    def run(self, iq: npt.NDArray[np.int16], *, edge: Literal["lower", "upper"]
        ...
        return HostDecisionEvidence(
            ...
            screen_mask=result.screen_mask, confirmation_mask=result.confirmation_mask,
            supported_start=result.supported_start, supported_end=result.supported_end,
            candidate_supported=bool(result.candidate_supported), epoch=result.epoch,
            fractional_complete=bool(result.fractional_complete),
            fractional_offset=result.fractional_offset, cfo_hz=result.cfo_hz,
            exact_score=result.exact_score, margin=result.margin,
            filter_cpu_ms=result.filter_cpu_ms, cpu_ms=result.cpu_ms, wall_ms=result.wall_ms,
