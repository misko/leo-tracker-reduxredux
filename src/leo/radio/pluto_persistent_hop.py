...
class _PlutoPersistentHopSession:
    ...
    def start_clock_bracket(self) -> PersistentHopStartClockBracketV1 | None:
        upstream = getattr(self._upstream, "start_clock_bracket", None)
        if upstream is None:
            return None
        return PersistentHopStartClockBracketV1(
            before_realtime_ns=upstream.before_realtime_ns,
            before_monotonic_ns=upstream.before_monotonic_ns,
            after_realtime_ns=upstream.after_realtime_ns,
            after_monotonic_ns=upstream.after_monotonic_ns,
        )
