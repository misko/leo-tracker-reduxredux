...
def create_app(
    ...
    *,
    ...
    @router.api_route(
        ...
        methods=["GET", "HEAD"],
        response_model=AdaptiveHopHistoryPageV1,
    )
    def adaptive_hop_session_history(
        cursor: Annotated[int, Query(ge=0)] = 0,
        limit: Annotated[int, Query(ge=1, le=20)] = 20,
    ) -> AdaptiveHopHistoryPageV1:
        if adaptive_hop_sessions is None:
            raise HTTPException(status_code=404, detail="adaptive history is not available")
        try:
            return adaptive_hop_sessions.page(cursor=cursor, limit=limit)
    ...
    @router.api_route(
        ...
        methods=["GET", "HEAD"],
        response_model=AdaptiveHopSessionDetailV1,
    )
    def adaptive_hop_session_detail(
        session_id: Annotated[str, ApiPath(pattern=GLRT_SESSION_PATTERN)],
    ) -> AdaptiveHopSessionDetailV1:
        if adaptive_hop_sessions is None:
            raise HTTPException(status_code=404, detail="adaptive history is not available")
        try:
            detail = adaptive_hop_sessions.detail(session_id)
        except Exception as error:
            raise HTTPException(status_code=409, detail="adaptive detail is unavailable") from error
        if detail is None:
    ...
    def adaptive_history_v2(
        ...
        if adaptive_hop_sessions_v2 is None:
            raise HTTPException(status_code=404, detail="adaptive history is not available")
        try:
            return adaptive_hop_sessions_v2.page_v2(cursor=cursor, limit=limit)
        except Exception as error:
            raise HTTPException(
                status_code=409, detail="adaptive history is unavailable"
            ) from error
    ...
    def adaptive_detail_v2(
        ...
        if adaptive_hop_sessions_v2 is None:
            raise HTTPException(status_code=404, detail="adaptive history is not available")
        try:
            detail = adaptive_hop_sessions_v2.detail_v2(session_id)
            if detail is not None and detail.capture.session_id != session_id:
                raise ValueError("adaptive detail identity differs")
        except Exception as error:
            raise HTTPException(status_code=409, detail="adaptive detail is unavailable") from error
        if detail is None:
            raise HTTPException(status_code=404, detail="adaptive session not found")
    ...
    def adaptive_hop_glrt_detail(
        session_id: Annotated[str, ApiPath(pattern=GLRT_SESSION_PATTERN)],
    ) -> ScannerGlrtPublicationV1:
        if adaptive_scanner_glrt is None:
            raise HTTPException(status_code=404, detail="adaptive GLRT results are unavailable")
        try:
            publication = adaptive_scanner_glrt.detail(session_id)
        except Exception as error:
            raise HTTPException(
                status_code=409, detail="adaptive GLRT evidence is unavailable"
            ) from error
    ...
    def standard_subject_png_inventory(
        ...
        if inventory is None:
            raise HTTPException(status_code=404, detail="Native PNG inventory is not published")
        try:
            if isinstance(inventory, StandardNativePngArtifactInventoryV14):
                return StandardNativePngArtifactInventoryV14.model_validate(inventory.model_dump())
            ...
            if isinstance(inventory, StandardNativePngArtifactInventoryV7):
                return StandardNativePngArtifactInventoryV7.model_validate(inventory.model_dump())
            if isinstance(inventory, StandardNativePngArtifactInventoryV6):
                return StandardNativePngArtifactInventoryV6.model_validate(inventory.model_dump())
