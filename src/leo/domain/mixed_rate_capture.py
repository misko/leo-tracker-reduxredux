...
def compile_production_capture_plan_v5(
    *,
    intent: ProductionDwellIntentV3,
    profile_revisions_by_radio: Mapping[str, CaptureProfileRevisionV2],
    source_type: SourceType = SourceType.LIVE,
) -> CapturePlanV5:
    """Bind a V3 direct-async intent to exact reviewed profile revisions."""

    radios = intent.radio_ids
    if set(profile_revisions_by_radio) != set(radios):
        raise ValueError("direct-async production profiles must cover scheduled radios")
    legs: list[ProductionRadioPlanV2] = []
    duration_seconds: Decimal | None = None
    scheduled = {item.radio_id: item for item in intent.radio_legs}
    for radio_id in radios:
        ...
        if (
            ...
            raise ValueError("direct-async profile revision disagrees with scheduled authority")
        if profile.duration_seconds is None:
            raise ValueError("direct-async production profiles require duration_seconds")
        if duration_seconds is None:
            duration_seconds = profile.duration_seconds
        elif duration_seconds != profile.duration_seconds:
            raise ValueError("direct-async production profiles require one common duration")
        sample_count = int(
            (profile.duration_seconds * profile.sample_rate_hz).to_integral_value(
                rounding=ROUND_HALF_UP
            )
    ...
    candidate = CapturePlanV5.model_construct(plan_digest="sha256:" + "0" * 64, **values)
    document = candidate.model_dump(mode="json", exclude={"plan_digest"})
    return CapturePlanV5.model_validate({**document, "plan_digest": canonical_digest(document)})
