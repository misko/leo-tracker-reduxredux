...
class SealedLegacyReferenceDecisionPort:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            receipt.config.config_digest != detector_binding.legacy_configuration_digest
...
def campaign_config_from_accepted_capture(
    *,
    ...
    return MatchedPilotAcceptanceCampaignConfigV1.create(
        campaign_id=campaign_id,
        capture_campaign_receipt_digest=canonical_digest(capture_receipt.model_dump(mode="json")),
        detector_binding=detector_binding,
        strata=strata,
        capture_inventory=tuple(inventory),
    )
