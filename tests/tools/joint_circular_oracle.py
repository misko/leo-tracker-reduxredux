...
def dense_group_score(
    train, test, null_train, null_test, offsets, groups, sigma=3000.0, outlier=0.05, size=512
):
    ...
    for tr, te, nt, ne, means in zip(train, test, null_train, null_test, offsets, strict=True):
        ...
        joint_rows.append(
            np.logaddexp(logsumexp(signal + np.asarray(te)[:, None], axis=0), nt + ne)
        )
...
def synthetic_scene(position):
    ...
    for track in range(8):
        ...
        group = track // 4
        intercept = [-38000.0, -31000.0][group] + (track - 4) * PERIOD_HZ
        ...
        null_train.append(
            -3 * np.mean(null_residual[mask] ** 2) / 30000**2 - 6 * np.log(30000) + np.log(0.5)
        )
