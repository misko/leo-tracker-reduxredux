...
@pytest.mark.parametrize("spacing,count", [(12.5, 64), (25.0, 16), (6.25, 256)])
def test_local_reference_stays_within_selected_coarse_cell(spacing, count):
    points = runner.exact_descendants(-150.0, 50.0, spacing)
    assert len(points) == count
    assert len({tuple(point) for point in points}) == count
    assert np.all(np.abs(points[:, 0] + 150.0) < 50.0)
    assert np.all(np.abs(points[:, 1] - 50.0) < 50.0)


def test_uncapped_diagnostic_does_not_hide_cap_or_unmatched_track():
    row = point_evaluation(
        0.0,
        0.0,
        [
            TrackResidual("large", 1000.0, 1.0),
            TrackResidual("small", 0.0, 3.0),
            TrackResidual("missing", None, 2.0),
        ],
    )
    diagnostic = runner.residual_diagnostics(row)
    assert diagnostic["uncapped_matched_track_weighted_rms_hz"] == 500.0
    assert diagnostic["unmatched_track_count"] == 1
    assert diagnostic["matched_track_count"] == 2
    assert row.weighted_mse_hz2 == 320000.0
