...
def subject():
    path = Path(__file__).parents[2] / "reports/2026_09_23_long_global_epoch_position/fit.py"
    spec = importlib.util.spec_from_file_location("long_global_epoch", path)
...
def test_training_score_and_cfo_ignore_complementary_rows():
    ...
    before = module.score(single, [track(0.0)], (0.0, 0.0), {"global": 0.0}, 1.0)
    after = module.score(single, [track(1e7)], (0.0, 0.0), {"global": 0.0}, 1.0)
