...
def accelerated_score(position, arm):
    ...
    shapes = [
        FITTER.TrackShape(
            tr, tr + te, off, null_tr, null_tr + null_te, np.ones(len(tr), dtype=bool)
        )
...
def test_synthetic_position_and_shared_offsets_are_identifiable():
    ...
    result = minimize(loss, [1.2, 0.5], method="Powell", options={"xtol": 1e-7, "ftol": 1e-10})
...
def test_candidate_with_tiny_training_weight_can_dominate_heldout():
    ...
    _, predictive = ORACLE.dense_group_score(train, test, [-1000.0], [0.0], [[0.0, 0.0]], [0])
...
def test_all_null_tracks_leave_group_offsets_unidentified():
    ...
    np.testing.assert_allclose([training, predictive], [-9.0, -13.0], atol=1e-12)


@pytest.mark.parametrize("sigma", [3000.0, 10000.0, 30000.0])
...
def test_accelerated_fitter_matches_independent_dense_oracle(sigma, outlier):
    ...
    for position in ([0.4, -0.7], [1.2, 0.5], [-2.0, 2.0]):
        ...
        error = np.max(
            np.abs(np.array([actual["training_score"], actual["heldout_score"]]) - expected)
        )
        assert error <= actual["pruning_log_error_bound"] + 1e-10


def test_accelerated_fitter_recovers_injected_position():
    arm = FITTER.CircularArm(3000.0, 0.05, grid_size=512)
    result = minimize(
        lambda p: -accelerated_score(p, arm)["training_score"],
        [1.2, 0.5],
        method="Powell",
