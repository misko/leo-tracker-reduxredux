...
def test_residual_port_preserves_uncapped_results_and_returns_copy():
    sites = Region(38., -121., 1000., 1000.).points(np.asarray([25.]), np.asarray([-25.]))
    key = (float(sites.latitude_deg[0]), float(sites.longitude_deg[0]), 0.)
    instance = CoverageEvaluator.__new__(CoverageEvaluator)
    instance.evaluate_points = lambda _: []
    instance._details = {key: [{
        "tracklet_id": "track", "partition_seed": "frozen", "training_count": 4,
        "heldout_count": 2, "best_candidate": {"norad": 123, "heldout_rms_hz": 900.},
    }]}
    row = instance.evaluate_residual_points(sites)[0]
    assert row["east_km"] == 25.
    assert row["tracks"][0]["heldout_rms_hz"] == 900.
...
def test_residual_port_recomputes_missing_coverage_only_details():
    sites = Region(38., -121., 1000., 1000.).points(np.asarray([0.]), np.asarray([0.]))
    instance = CoverageEvaluator.__new__(CoverageEvaluator)
    instance.evaluate_points = lambda _: []
    instance._details = {}
    calls = []
    ...
    def finalists(_):
        calls.append(True)
        return [{"tracks": [{"tracklet_id": "missing", "partition_seed": "frozen",
                              "training_count": 4, "heldout_count": 2, "best_candidate": None}]}]
