...
def test_dimensionless_circular_factor_has_unit_uniform_mean():
    grid = np.linspace(-MODULE.PILOT_ALIAS_HZ / 2, MODULE.PILOT_ALIAS_HZ / 2, 2048, endpoint=False)
...
def test_wrapped_normal_matches_independent_image_sum_at_broad_sigma():
    ...
    expected = (
        ...
        * sum(np.exp(-0.5 * ((residual + image * period) / sigma) ** 2) for image in range(-20, 21))
    )
    np.testing.assert_allclose(actual, expected, rtol=2e-15, atol=2e-15)

    very_broad = MODULE.circular_factor(np.array([offset]), grid, 10 * period, 0.0)[0]
...
def test_circular_factor_is_invariant_to_integer_period_offsets():
    ...
    shifted = MODULE.circular_factor(np.array([31_234.0 + 10 * period]), grid, 30_000.0, 0.05)
...
def test_synthetic_shared_circular_offset_predicts_heldout_with_outlier():
    grid = np.linspace(-MODULE.PILOT_ALIAS_HZ / 2, MODULE.PILOT_ALIAS_HZ / 2, 1024, endpoint=False)
...
def test_null_dominated_track_is_offset_insensitive():
    grid = np.linspace(-MODULE.PILOT_ALIAS_HZ / 2, MODULE.PILOT_ALIAS_HZ / 2, 512, endpoint=False)
