...
def test_pruning_certifies_tiny_tail_without_total_minus_retained_cancellation():
    weights = np.r_[1 - 1e-13, np.full(1000, 1e-16)]
    budget = 1e-11
    retained, omitted, bound = TOOL._retained_indices(
        weights,
        sigma_hz=10_000.0,
        outlier_probability=0.05,
        log_error_budget=budget,
    )
    expected = np.sum(weights[np.setdiff1d(np.arange(len(weights)), retained)])
    assert omitted == pytest.approx(expected, rel=1e-15)
    assert bound <= budget
