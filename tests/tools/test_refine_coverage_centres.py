...
def scorer(seen):
    def evaluate(points):
        ...
        for e, n in points:
            ...
            seen.add(key)
            result.append(
                CellScore(*key, (ThresholdCoverage(200.0, 10, 1.0, 1, float(e * e + n * n), 1.0),))
            )
        return result
...
def test_seeded_refinement_uses_declared_lattice_without_duplicate_evaluations():
    ...
    rows, trace = refine_from_centres(
        scorer(seen),
        np.asarray([[-25.0, -25.0], [25.0, 25.0]]),
    ...
    axis = set(((np.arange(16) + 0.5) / 16 - 0.5) * 200.0)
    assert all(row.east_km in axis and row.north_km in axis for row in rows)
    assert all(np.hypot(row.east_km, row.north_km) <= 100.0 for row in rows)
...
def test_seed_on_final_lattice_is_retained_as_possible_winner():
    rows, _ = refine_from_centres(
        scorer(set()),
        np.asarray([[0.0, 0.0]]),
        initial_spacing_km=200.0,
        radius_km=400.0,
        region_size_km=1000.0,
        levels_km=(40.0,),
        basins=8,
    )
    assert (rows[0].east_km, rows[0].north_km) == (0.0, 0.0)
