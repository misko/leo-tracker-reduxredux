...
class FakeEvaluator:
    ...
    def metrics(self):
        return {
            "cache_hits": 0,
            "fully_scored_cell_count": self.count,
    ...
    def evaluate_top_k(self, sites, *, top_k, threshold_hz):
        ...
        rows = [
            CellScore(
                float(e),
                float(n),
                (ThresholdCoverage(threshold_hz, int(abs(e)) % 4, 1.0, 1, float(abs(n)), 1.0),),
            )
...
def test_fork_shard_merge_matches_global_ranking_and_preserves_parent(workers, top_k):
    sites = Region(38.0, -121.0, 100.0, 100.0).points(np.arange(-16.0, 16.0), np.arange(32.0) % 3)
    expected, _ = FakeEvaluator().evaluate_top_k(sites, top_k=top_k, threshold_hz=200.0)
    parent = FakeEvaluator()
    actual, bounds, metrics = evaluate_top_k_parallel(
        parent, sites, top_k=top_k, threshold_hz=200.0, workers=workers
    )
...
def test_invalid_parallel_worker_count_rejected():
    with pytest.raises(ValueError, match="positive"):
        evaluate_top_k_parallel(None, None, top_k=1, threshold_hz=200.0, workers=0)
