...
def test_worker_component_selector_is_a_trusted_bounded_alias(tmp_path: Path) -> None:
    deployment, _metadata, revision = _published_fixture(tmp_path)
    worker_selector = deployment / "current-worker"
    worker_selector.symlink_to(deployment / "releases" / revision)

    evidence = load_trusted_current_release(
        pipeline_release="worker-release",
        current_link=worker_selector,
        deployment_root=deployment,
        validator=lambda _release, _revision: None,
    )
