...
def test_adaptive_workers_provision_position_tracking_root() -> None:
    for relative in (
        ...
        adaptive = next(
            line for line in commands if line.endswith("/scanner-adaptive-tle-position-v1")
        )
        assert adaptive.startswith("ExecStartPre=+/usr/bin/install -d ")
        assert "-o leo -g leo" in adaptive
        assert "0750" in adaptive
        adaptive_v2 = next(
            line for line in commands if line.endswith("/scanner-adaptive-tle-position-v2")
        )
        assert adaptive_v2.startswith("ExecStartPre=+/usr/bin/install -d ")
        assert "-o leo -g leo" in adaptive_v2
        assert "0750" in adaptive_v2
...
def test_persistent_hop_analysis_is_restartable_bounded_and_capture_subordinate() -> None:
    ...
    assert analysis_text.count("ExecStartPre=+/usr/bin/install -d -o leo -g leo -m 0750") == 9
    ...
    assert "/srv/bulk/leo/scanner-shared-tracking-v2" in analysis_text
    for directory in (
...
def test_adaptive_tracking_worker_provisions_its_versioned_output_roots() -> None:
    ...
    assert (
        "ExecStartPre=+/usr/bin/install -d -o leo -g leo -m 0750 "
        "/srv/bulk/leo/scanner-shared-tracking-v5"
    ) in worker_text
    assert (
        "ExecStartPre=+/usr/bin/install -d -o leo -g leo -m 0750 "
        "/srv/bulk/leo/scanner-shared-tracking-v6"
    ) in worker_text
    assert (
        "ExecStartPre=+/usr/bin/install -d -o leo -g leo -m 0750 "
...
def test_production_deployment_is_staged_guarded_and_data_safe() -> None:
    ...
    assert "streaming=16`, `cpu=8`,\n`memory=4`, and `heavy=16" in deployment_text
