...
def spec(kind, **updates):
    return (
        dict(rate_hz=2500000, edge="lower", seed=99001, kind=kind, start_ms=40.0, duration_ms=20.0)
        | updates
    )
...
def test_decision_needs_complete_fractional_work_and_both_thresholds():
    ...
    candidates = [
        good,
        good | {"fractional_complete": 0},
        good | {"exact_score": 0.174},
        good | {"margin": 0.024},
    ]
...
def test_association_requires_time_frequency_and_signal_support():
    ...
    limits = {"maximum_circular_epoch_difference_us": 2, "maximum_cfo_difference_hz": 8000}
    epoch = truth["epoch_samples"]
    candidate = {
        "epoch": int(epoch),
        "fractional_offset_samples": epoch % 1,
    ...
    rows = [
        {
            ...
            "decisions": {
                name: {"flagged": True, "associated": False} for name in protocol["policies"]
            },
        },
        {
...
def test_timing_ambiguous_control_audit_never_turns_original_failure_into_pass(rate):
    ...
    candidate = {
        "epoch": int(shifted),
        "fractional_offset_samples": shifted % 1,
    ...
    row = {
        "truth": dict(
            spec("wrong_pilot", rate_hz=rate), starlink_model_present=False, epoch_samples=epoch
        ),
        "selected_window": 5,
