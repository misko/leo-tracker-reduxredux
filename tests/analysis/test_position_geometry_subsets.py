...
def _inputs():
    ...
    p = np.column_stack((7000*np.cos(angle), 7000*np.sin(angle), np.full(n, 800.)))
    v = np.column_stack((-7*np.sin(angle), 7*np.cos(angle), np.full(n, .2)))
...
def test_geometry_order_is_complete_deterministic_and_truth_blind():
    args = _inputs()
    grid = np.asarray([[6371., 0, 0], [6000., 1500., 1000.]])
