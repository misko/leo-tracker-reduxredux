...
def test_frozen_standard_registry_identity_does_not_move() -> None:
    ...
    assert registry.get("path-standard").spec.algorithm_version == "standard-v2-production-10"
...
def test_native_evidence_registry_declares_only_executable_products() -> None:
    ...
    assert alternate.algorithm_version == "standard-native-path-projection-v9"
    assert alternate.configuration_schema == "path-alternate-tracks-native.projection.v9"
    ...
    assert registry.get("radio-scientific-report-native").spec.algorithm_version == (
        "standard-native-radio-report-presentation-v11"
    )
    assert tuple(
