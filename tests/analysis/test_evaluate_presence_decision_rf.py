...
def test_rf_selection_is_bounded_and_all_edges_included():
    ...
    for change in (
        {"rf_sweeps": [30, 30]}, {"rf_sweeps": [30, 300]},
...
def test_unmatched_rf_is_never_counted_as_a_false_alarm():
    protocol = json.loads(PROTOCOL.read_text())
    record = {"rate_hz": 2500000, "reference_positive": [False] * 6,
    ...
    for policy in result.values():
        assert policy["2500000"] == {
            "dwells": 2, "reference_positive_dwells": 1,
            "flags_in_reference_positive": 1, "associated_reference_positive": 0,
            "unresolved_rf_dwells": 1, "flags_in_unresolved_rf": 1,
        }
...
def test_reused_reference_validates_iq_geometry_and_exact_large_counter():
    counter = 2**53 + 19
    row = {"iq_sha256": "a", "rate_hz": 2500000, "edge": "lower", "rx": 1,
    ...
    for field, value in (("iq_sha256", "b"), ("rate_hz", 5000000), ("edge", "upper"),
                         ("rx", 0), ("source_counter", str(counter + 1)),
...
def diagnostic_case():
    ...
    candidate = {"epoch": 3333, "fractional_offset_samples": 0.4,
                 "tracking_cfo_hz": 70000, "exact_score": 0.3, "margin": 0.2,
                 "fractional_complete": 1}
    reference = {"epoch_sample": 0, "fractional_offset_samples": 0.1,
                 "tracking_cfo_hz": 70001, "exact_score": 0.3, "margin": 0.2}
    row = {"rate_hz": 2500000, "selected_window": 0, "candidates": [candidate],
           "reference_candidates": [[], [reference], [], [], [], []],
           "reference_positive": [False, True, False, False, False, False]}
    result = {"confirmation_count": 6, "confirmation_window_mask": 63,
              "rank": {"order": list(range(6))},
              "confirmations": [{"candidates": [copy.deepcopy(candidate)], "candidate_count": 1}
                                for _ in range(6)]}
...
def test_diagnostic_requires_complete_inventory_and_reproduced_single_confirmation():
    protocol, row, result = diagnostic_case()
    for change in ({"confirmation_count": 1}, {"confirmation_window_mask": 62},
