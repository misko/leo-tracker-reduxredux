...
def test_v2_store_uses_separate_namespace_and_preserves_v1(tmp_path):
    AdaptiveTlePositionStore(tmp_path, read_only=False).publish(document(), PNG)
    writer = AdaptiveTlePositionStoreV2(tmp_path, read_only=False)
    manifest = writer.publish(document_v2(), PNG)
    assert AdaptiveTlePositionStore(tmp_path).status("scan-1").manifest.document.schema_version == 1
    assert AdaptiveTlePositionStoreV2(tmp_path).status("scan-1").manifest == manifest
