...
test("scanner view presents an empty archive without a server failure", async ({ page }) => {
  ...
  await page.route("**/api/v3/scanner/analyses?*", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
        schema_version: 3, cursor: 0, limit: 20, total: 0, next_cursor: null, items: [],
      }),
    });
  });
...
test("scanner view reports a corrupt selected page as a bounded conflict, not a 5xx", async ({ page }) => {
  ...
  await page.route("**/api/v3/scanner/analyses?cursor=20&limit=20", async (route) => {
    await route.fulfill({
      status: 409,
      contentType: "application/json",
