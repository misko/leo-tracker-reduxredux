...
def _strict_t1_lines(summary_path: Path) -> list[str]:
    ...
    lines.extend(
        [
            ...
            "",
            "At 7.5–7.9 s, the persisted 8-basin/coarse search places 12/16 "
            "best candidates within 500 Hz of the local line and has a 227.51 kHz "
            "worst miss. Finer grids with 8 basins recover 15/16 (2.66 kHz worst); "
            "32 basins on the coarse grid recover 16/16 (0.31 kHz worst); combining "
            "32 basins with fine grids also recovers 16/16 (0.24 kHz worst). Basin "
            "retention is therefore the dominant local fix, with grid refinement "
            "secondary.",
            "",
