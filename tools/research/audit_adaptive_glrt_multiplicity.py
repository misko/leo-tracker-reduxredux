...
def _selected_matches(visit: object) -> list[dict]:
    ...
    for probe_index in sorted(
                    ...
                    edges.append(
                        {
                            ...
                            "quality": min(
                                left_candidate.fractional_margin, right_candidate.fractional_margin
                            ),
        ...
        matched = select_consistent_receiver_pairs(
            [
                (e["left_index"], e["right_index"], e["receiver_offset_hz"], e["quality"])
                for e in edges
            ],
    ...
    try:
        ...
        with analyses.job(binding) as job:
            ...
            for visit in job.published_visits():
                ...
                groups = {
                    receiver: _frequency_groups(items) for receiver, items in candidates.items()
                }
                for counter in (summary, bucket):
                    ...
                    counter["both_passed_ge_1"] += (
                        len(candidates[0]) >= 1 and len(candidates[1]) >= 1
                    )
                    counter["both_passed_ge_2"] += (
                        len(candidates[0]) >= 2 and len(candidates[1]) >= 2
                    )
                ...
                for candidate in matches:
                    ...
                    for old in retained:
                        distance0 = abs(
                            circular_frequency_delta(
                                candidate["rx0"].fractional_tracking_cfo_hz,
                                old["rx0"].fractional_tracking_cfo_hz,
                            )
                        )
                        distance1 = abs(
                            circular_frequency_delta(
                                candidate["rx1"].fractional_tracking_cfo_hz,
                                old["rx1"].fractional_tracking_cfo_hz,
                            )
                        )
                    ...
                    if not conflicts:
                        retained.append(candidate)
                        continue
                    distance0, distance1, old = min(
                        conflicts, key=lambda item: min(item[0], item[1])
                    )
                    source_separation = abs(
                        (
                            candidate["rx0"].integer_epoch_sample
                            + candidate["rx0"].fractional_epoch_offset_samples
                        )
                        - (
                            old["rx0"].integer_epoch_sample
                            + old["rx0"].fractional_epoch_offset_samples
                        )
                    )
                    relation = (
                        "both_receivers"
                    ...
                    record = {
                        ...
                        "rejected": {
                            "rx0": _candidate(candidate["rx0"]),
                            "rx1": _candidate(candidate["rx1"]),
                        },
                        "retained_conflict": {
                            "rx0": _candidate(old["rx0"]),
                            "rx1": _candidate(old["rx1"]),
                        },
                    }
                    ...
                    for counter in (summary, bucket):
                        ...
                        counter["receiver_timing_within_one_sample"] += (
                            candidate["timing_residual_samples"] <= 1.0
                        )
                for counter in (summary, bucket):
        ...
        result = {
            ...
            "rules": {
                ...
                "distinct_source": (
                    "reject if either RX tracking CFO is within 5 kHz modulo 1/4.4 us"
                ),
            },
