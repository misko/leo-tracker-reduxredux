...
def load(
    ...
    *,
    ...
    if min_span_s <= 0 or min_observations < 4:
        raise ValueError("positive minimum span and at least four observations required")
    kernel = reference_kernel()
    before = file_digest(KERNEL_PATH)
    if track_count is not None and min_span_s == 7.0 and min_observations == 14:
        start, config, selected, eligible, reconstructed = kernel._tracks(
            session, bulk_root, track_count
        )
    else:
        start, config, selected, eligible, reconstructed = load_track_selection(
            kernel, session, bulk_root, track_count, min_span_s, min_observations
        )
...
def load_track_selection(kernel, session, bulk_root, limit, min_span_s, min_observations):
    """Select every eligible standard trajectory without the legacy 7 s floor."""
    store = kernel.ScannerTrackingInputStore(bulk_root)
    try:
        source = store.load(session)
    finally:
        store.close()
    if not kernel.timing_is_qualified_for_tle(source.timing):
        raise ValueError("qualified UTC required")
    start = source.timing.first_sample_estimate_utc_ns
    config = kernel.PersistentHopTrajectoryConfig(
        minimum_span_s=min_span_s, minimum_support=min_observations
    )
    trajectory = kernel.reconstruct_persistent_hop_trajectories(
        kernel.project_scanner_candidates(source), config=config
    )
    selected, seen = [], set()
    for hypothesis in trajectory.hypotheses:
        for tracklet_id in hypothesis.tracklet_ids:
            ...
            graph = kernel.persistent_hop_tracklet_graph(hypothesis, tracklet_id)
            rows = sorted(graph.observations, key=lambda row: row.support_center_utc_ns)
            times = np.asarray([(row.support_center_utc_ns - start) / 1e9 for row in rows])
            if len(times) >= min_observations and times[-1] - times[0] >= min_span_s:
                selected.append((tracklet_id, graph, rows, times, float(times[-1] - times[0])))
    count = len(selected) if limit is None else limit
    ordered = kernel._select_review_tracks(selected, count) if selected else []
    return start, config, ordered, len(selected), len(trajectory.tracklets)
