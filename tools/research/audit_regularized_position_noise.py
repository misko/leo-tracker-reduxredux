...
def _quantiles(values):
    return dict(zip(("min", "p25", "median", "p75", "max"),
                    np.quantile(values, (0, .25, .5, .75, 1)).tolist(), strict=True))
...
def run(args):
    ...
    for block_number in (1, 2, 3):
        ...
        for scan in manifest["scans"]:
            ...
            for track in evidence["tracks"]:
                ...
                track_rows.append({
                    "session_id": sid, "track_id": track["track_id"],
                    "candidate_id": selected["candidate_id"], "tau_s": selected["tau_s"],
                    "training_observations": len(residual), "one_second_bins": bins,
                    "rms_hz": scale, "sse_hz2": float(np.sum(residual**2)),
                })
            ...
            scan_rows.append({"session_id": sid, "training_observations": scan_n,
    ...
    largest = max(1, int(np.ceil(.1 * len(sse))))
    ...
    result = {
        ...
        "counts": {"scans": len(scan_rows), "tracks": len(track_rows),
                   "training_observations": n, "one_second_track_bins": bins,
        ...
        "heterogeneity": {
            ...
            "largest_10_percent_tracks_share_of_training_sse":
                float(np.sort(sse)[-largest:].sum() / sse.sum()),
        ...
        "scan_rows": scan_rows,
        "bindings": {"joint_source": _digest(joint_path),
    ...
    axes[0].hist(track_rms, bins=np.geomspace(max(1, track_rms.min()), track_rms.max(), 35),
                 color="#267b9f")
    axes[0].set(xscale="log", xlabel="Training track RMS (Hz)", ylabel="Tracks",
                title="Heavy residual tail")
    ordered_sse = np.sort(sse)
    axes[1].plot(np.arange(1, len(sse) + 1) / len(sse), np.cumsum(ordered_sse) / sse.sum(),
                 color="#a23b72")
    axes[1].plot([0, 1], [0, 1], color="black", alpha=.35, linestyle="--")
    axes[1].set(xlabel="Cumulative fraction of tracks", ylabel="Cumulative fraction of SSE",
                title="Residual influence concentration")
    axes[2].plot([row["rms_hz"] for row in scan_rows], "o-", markersize=3,
                 color="#d97904")
    axes[2].axhline(np.median([row["rms_hz"] for row in scan_rows]), color="black",
                    linestyle="--", linewidth=1)
    axes[2].set(xlabel="Chronological training scan", ylabel="Training RMS (Hz)",
    ...
    print(json.dumps({"counts": result["counts"], "dependence": result["dependence"],
                      "heterogeneity": result["heterogeneity"]}, indent=2))
