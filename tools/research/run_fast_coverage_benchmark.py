...
SOURCE_PATHS = (
    ...
    Path(__file__).with_name("parallel_coverage.py"),
)
    ...
    for city in args.cities:
        ...
        if args.mode in {"uniform", "both"}:
            for spacing in args.spacings_km:
                ...
                points = regional_circle_offsets(SEARCH_BOX_KM, radius, spacing)
                if args.single_point is not None:
                    points = np.asarray([args.single_point], dtype=float)
                metrics_before = evaluator.metrics()
                sites = site_grid(region, points)
                pruning = []
                parallel_metrics = None
                if args.uniform_policy == "exact-topk":
                    if args.workers > 1:
                        scores, pruning, parallel_metrics = evaluate_top_k_parallel(
                            evaluator,
                            sites,
                            top_k=args.top_k,
                        ...
                        scores, pruning = evaluator.evaluate_top_k(
                            sites,
                            top_k=args.top_k,
                            threshold_hz=args.primary_threshold_hz,
                        )
                else:
                    scores = evaluator.evaluate_points(sites)
                search_s = time.monotonic() - run_started
                search_metrics = evaluator.metrics()
                ...
                result = {
                    ...
                    "newly_scored_points": (
                        sum(row["fully_scored_cell_count"] for row in parallel_metrics["shards"])
                        if parallel_metrics
                        else search_metrics["fully_scored_cell_count"]
                        - metrics_before["fully_scored_cell_count"]
                    ),
                    "partially_scored_pruned_points": (
                        sum(row["early_abandoned_cell_count"] for row in parallel_metrics["shards"])
                        if parallel_metrics
                        else search_metrics["early_abandoned_cell_count"]
                        - metrics_before["early_abandoned_cell_count"]
                    ),
                    "preexisting_full_score_cache_hits": (
                        sum(row["cache_hits"] for row in parallel_metrics["shards"])
                        if parallel_metrics
                        else search_metrics["cache_hits"] - metrics_before["cache_hits"]
                    ),
        ...
        if args.mode in {"adaptive", "both"}:
            ...
            evaluated_scores = rank_coverage_cells(
                evaluator.cache.values(), args.primary_threshold_hz
            )
            write_gzip_json(directory / "evaluated-scores.json.gz", score_rows(evaluated_scores))
            finalist_started = time.monotonic()
            finalist_points = np.asarray(
                [[row.east_km, row.north_km] for row in ranked[: args.top_k]]
            )
            ...
            result = {
                ...
                "newly_scored_points": (
                    evaluator.metrics()["fully_scored_cell_count"]
                    - metrics_before["fully_scored_cell_count"]
                ),
                "partially_scored_pruned_points": 0,
                "preexisting_full_score_cache_hits": (
                    evaluator.metrics()["cache_hits"] - metrics_before["cache_hits"]
                ),
                "top_cells": score_rows(ranked[: args.top_k]),
                "evaluator_metrics": evaluator.metrics(),
                "finalists_digest": digest(directory / "finalists.json.gz"),
                "trace_digest": digest(directory / "trace.json"),
                "evaluated_scores_digest": digest(directory / "evaluated-scores.json.gz"),
            }
            write_json(directory / "result.json", result)
            summary_runs.append(
                {
...
def main() -> None:
    ...
    parser.add_argument(
        "--uniform-policy",
        choices=("full", "exact-topk"),
        default="full",
        help="score all cells or certify only the primary-threshold top K",
    )
    ...
    if not 0 < args.radius_km <= MAXIMUM_RADIUS_KM:
        parser.error("radius must be positive and no greater than 500 km")
    if args.workers > 1 and args.uniform_policy != "exact-topk":
        parser.error("workers > 1 requires exact-topk uniform policy")
