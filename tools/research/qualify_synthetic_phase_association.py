...
def simulate_unit(index, regime):
    ...
    truth = dict(
        theta0=rng.uniform(-0.8, 0.8, 2),
        omega=rng.uniform(-0.012, 0.012, 2),
        range_rate=rng.uniform(-3500, 3500, 2),
        range_accel=rng.uniform(-18, 18, 2),
    )
    ...
    rx_pos = np.array(
        [[0.0, 0.0], [BASELINE_M * np.cos(BASELINE_ANGLE), BASELINE_M * np.sin(BASELINE_ANGLE)]]
    )
    theta = truth["theta0"][:, None] + truth["omega"][:, None] * t
    unit = np.stack((np.cos(theta), np.sin(theta)), axis=-1)
    phase = np.empty((2, 2, len(t)))
    shared = np.vstack([rng.uniform(-np.pi, np.pi) + rng.normal(0, 0.3) * t for _ in range(2)])
    for rx in (0, 1):
        for source in (0, 1):
            geom = 2 * np.pi * RF[source] / C * (unit[source] @ rx_pos[rx])
            channel = 0.0
            if regime == "frequency_mismatch":
                channel = (rx * 2 - 1) * (source * 2 - 1) * 0.32 * t
            if regime == "independent_phase":
                channel = np.cumsum(rng.normal(0, 0.55, len(t)))
            phase[rx, source] = (
                rng.uniform(-np.pi, np.pi)
                + geom
                + shared[rx]
                + channel
                + rng.normal(0, 0.10, len(t))
            )
    dd = wrap((phase[1, 0] - phase[0, 0]) - (phase[1, 1] - phase[0, 1]))
    cal = np.zeros(len(t), bool)
    cal[rng.permutation(len(t))[:12]] = True
    return dict(
        index=index,
        regime=regime,
        time=t,
...
def fit_training(units):
    d, p = [], []
    for u in units:
        dr, pr = residuals(u, u["candidates"][u["label"]])
        d.extend(dr)
        p.extend(pr)
    ds = max(float(np.sqrt(np.mean(np.square(d)))), 1e-6)
    ps = max(float(np.sqrt(np.mean(np.square(p)))), 1e-6)
...
def candidate_losses(u, fit):
    ...
    for c in u["candidates"]:
        dr, pr = residuals(u, c)
        base.append(0.5 * np.sum((dr / fit["doppler_sigma"]) ** 2))
        phase = 0.5 * np.sum((pr / fit["phase_sigma"]) ** 2) if fit["phase_enabled"] else 0.0
        aug.append(base[-1] + phase)
        _, wrong = residuals(u, c, BASELINE_ANGLE + np.pi / 2)
        control.append(base[-1] + 0.5 * np.sum((wrong / fit["phase_sigma"]) ** 2))
...
def evaluate(units, fit):
    ...
    for u in units:
        ...
        def metrics(loss, label=int(u["label"])):
            loss = np.asarray(loss)
            prob = np.exp(-(loss - loss.min()))
            prob /= prob.sum()
            rank = int(np.argsort(loss).tolist().index(label) + 1)
            return rank, float(-np.log(max(prob[label], 1e-300)))

        br, bl = metrics(base)
        ar, al = metrics(aug)
        cr, cl = metrics(control)
        rows.append(
            dict(
                index=u["index"],
                label=u["label"],
                baseline_rank=br,
                augmented_rank=ar,
                orientation_control_rank=cr,
                baseline_log_loss=bl,
                augmented_log_loss=al,
...
def summarize(rows, fit):
    return dict(
        fit=fit,
        held_units=len(rows),
        coverage=(1.0 if fit["phase_enabled"] else 0.0),
        ...
        orientation_control_mean_log_loss=float(
            np.mean([r["orientation_control_log_loss"] for r in rows])
        ),
    )
...
def main():
    ...
    if a.freeze:
        a.seal.parent.mkdir(parents=True, exist_ok=True)
        a.seal.write_text(
            json.dumps(
                {
    ...
    train = set(order[:18].tolist())
    result = {
        "schema": "synthetic-phase-association/v1",
        "seed": SEED,
        "train_units": sorted(train),
        "held_units": sorted(set(range(36)) - train),
        "regimes": {},
    }
    for regime in REGIMES:
        units = [simulate_unit(i, regime) for i in range(36)]
        tr = [u for u in units if u["index"] in train]
        he = [u for u in units if u["index"] not in train]
        fit = fit_training(tr)
        rows = evaluate(he, fit)
        result["regimes"][regime] = {"summary": summarize(rows, fit), "rows": rows}
