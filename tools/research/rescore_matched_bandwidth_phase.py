...
def score(rows: list[dict], label: str) -> dict:
    ...
    for ax, (session, visit) in zip(axes, cases, strict=True):
        ...
        summary = {
            ...
            "visit": visit,
            "groups": {
                "train": [r["group_id"] for r in case if r["partition"] == "train"],
                "held": [r["group_id"] for r in case if r["partition"] == "held"],
            },
            "arms": {},
        }
...
def main():
    ...
    for label, path in (("v1", BASE / "results.json"), ("v2", BASE / "v2/results.json")):
        ...
        docs.append(score(doc["rows"], label))
    (OUT / "results.json").write_text(
        json.dumps(
            {"score_contract": "ordinary 2pi complex-vector phase; no IQ reread", "inputs": docs},
            indent=2,
        )
        + "\n"
    )
