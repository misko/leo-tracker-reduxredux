...
# ruff: noqa: E402 -- thread limits must be bound before numerical-library imports.
...
# Bind numerical libraries before importing NumPy/SciPy. The validation budget is
# wall time and must not consume an undeclared host-wide BLAS thread pool.
...
CHRONOLOGICAL_COARSE_RESULT_SHA256 = (
    "sha256:51ca427a4f4ea85d80be6cc4ddf714ced643a7290678fb543c42ed8633a9fdfd"
)
...
def _check_resources(deadline: float) -> None:
...
def _qualification(result, shared_outer_success: bool, nominal_outer_success: bool, tolerance):
    reasons = []
    if not shared_outer_success:
        reasons.append("shared-outer-optimizer-unconverged")
    if not nominal_outer_success:
        reasons.append("nominal-outer-optimizer-unconverged")
    for arm in ("selected_shared_result", "selected_nominal_result"):
        if not result[arm]["shared_fit"]["converged"]:
            reasons.append(arm.replace("_result", "") + "-inner-optimizer-unconverged")
    if not result["transfer_evaluation"]["shared_fit"]["converged"]:
        reasons.append("transfer-inner-optimizer-unconverged")
    audits = tuple(result["selected_shared_result"]["exact_replay_audits"]) + tuple(
        result["transfer_evaluation"]["exact_replay_audits"]
    )
    if not audits or any(
        audit.get("state") != "complete" or audit.get("maximum_error_hz", math.inf) > tolerance
        for audit in audits
    ):
        reasons.append("exact-replay-tolerance-failed")
    return {"state": "complete" if not reasons else "insufficient", "reasons": tuple(reasons)}
...
def _load_coarse_result(path, documents):
    ...
    if (
        ...
        or _file_digest(path) != CHRONOLOGICAL_COARSE_RESULT_SHA256
...
def main() -> int:
    ...
    checkpoint_path = args.checkpoint or args.output.with_suffix(".checkpoints.jsonl")
    ...
    def evaluate(east, north, branch_id, tokens=None, exact=False, fit_config=None):
        ...
        _append_checkpoint(
            checkpoint_path,
            {
                ...
                "full_catalogue_nominal_negative_log_evidence": full_score,
                "matched_nominal_negative_log_posterior": row[
                    "matched_nominal_negative_log_posterior"
                ],
                "matched_shared_negative_log_posterior": row[
                    "matched_shared_negative_log_posterior"
                ],
            },
        )
        return row
    ...
    result = {
        ...
        "provenance": {
            ...
            "numerical_core_sha256": _file_digest(
                Path(__file__).parents[2] / "src/leo/analysis/research/blind_shared_orbit.py"
            ),
    ...
    qualification = _qualification(
        result,
        bool(shared_answer.success),
        bool(nominal_answer.success),
        config.exact_audit_maximum_error_hz,
    )
    result["state"] = qualification["state"]
