...
def _retained_indices(
    ...
    *,
    ...
    # Sum the tiny omitted tail directly. Subtracting retained mass from a total
    # near one loses the precision needed for 1e-13-scale pruning certificates.
    suffix = np.empty(len(ordered) + 1, dtype=np.longdouble)
    suffix[-1] = 0
    suffix[:-1] = np.cumsum(ordered[::-1], dtype=np.longdouble)[::-1]
    eligible = np.flatnonzero(suffix <= allowed_mass)
    keep = int(eligible[0]) if len(eligible) else len(order)
    retained = order[:keep]
    omitted = float(suffix[keep])
    relative_bound = omitted * max_abs / outlier_probability
    log_bound = -np.log1p(-relative_bound) if relative_bound < 1 else np.inf
    while keep < len(order) and log_bound > log_error_budget:
        keep += 1
        retained = order[:keep]
        omitted = float(suffix[keep])
        relative_bound = omitted * max_abs / outlier_probability
...
def mixture_curve(
    ...
    *,
    ...
    if np.any(normalized <= 0) or bound > track_log_error_budget:
        # Numerical certification failure is never permission to loosen the
        # requested error bound. Fall back to the exact dense candidate kernel.
        retained = np.arange(len(candidate_log))
        factor = circular_factor(
            np.asarray(native_mean_hz)[retained],
            arm.grid_hz(),
            float(arm.sigma_hz),
        ...
        normalized = 1 + np.sum(weights[:, None] * (factor - 1), axis=0)
        omitted, bound = 0.0, 0.0
        if np.any(normalized <= 0):
            raise RuntimeError("dense circular factor mixture is nonpositive")
...
def verify_refinement(path: Path) -> tuple[dict, dict]:
    ...
    if (
        seal.get("schema") != "regional-five-block-refinement-seal/v1"
        or seal.get("result_digest") != "sha256:" + checksum
        or seal.get("result_checksum_digest") != digest(path.with_name("result.sha256"))
        or seal.get("position_truth_used") is not False
        or seal.get("all_fits_converged") is not True
        or seal.get("fits") != len(result.get("fits", []))
        or seal.get("observation_count") != result.get("observation_count")
        or seal.get("sessions") != result.get("sessions")
        or seal.get("partition_receipt_digest") != result.get("partition_receipt_digest")
        or seal.get("adapter_digest") != result.get("five_block_adapter_digest")
        or seal.get("wrapper_digest") != result.get("five_block_wrapper_digest")
        or seal.get("base_refiner_digest") != result.get("base_refiner_digest")
        or seal.get("numerical_core_digest") != result.get("numerical_source_digest")
    ...
    if not acquisition_seal.get("files") or acquisition_seal.get("schema") != (
        "regional-five-block-acquisition-content-seal-v1"
    ):
        raise ValueError("unexpected acquisition seal")
    ...
    acquisition = json.loads((run / "result.json").read_text())
    receipt = json.loads((run / "partition-receipt.json").read_text())
    if (
        result.get("source_result_digest") != digest(run / "result.json")
        or result.get("acquisition_file_digests") != acquisition_seal["files"]
        or acquisition.get("complete") is not True
        or acquisition.get("position_truth_used") is not False
        or acquisition.get("partition") != FIVE_BLOCK_PARTITION
        or receipt.get("partition") != FIVE_BLOCK_PARTITION
        or receipt.get("content_digest") != content_digest(receipt)
        or acquisition.get("partition_receipt_digest") != receipt.get("content_digest")
        or result.get("partition_receipt_digest") != receipt.get("content_digest")
    ):
        raise ValueError("refinement/acquisition partition lineage mismatch")
...
def build_state_cache(
    ...
    if len(sessions) not in (1, 5) or len(sessions) != len(set(sessions)):
        raise ValueError("expected an exact sealed single- or five-session cohort")
    tracks = []
    provenance = {}
    acquisition_receipt = json.loads(
        (Path(refinement["run"]) / "partition-receipt.json").read_text()
    )
    expected_partition_documents = {
        row["source_document_digest"]: row for row in acquisition_receipt["documents"]
    }
    for session in sessions:
        ...
        for episode_id, arc in loaded:
            ...
            if (
                episode is None
                or len(episode["members"]) != 1
                or episode["members"][0] != episode_id
            ):
    ...
    if len(loader.documents) != len(sessions):
        raise ValueError("partition loader did not account for every cohort document")
    for actual in loader.documents:
        expected = expected_partition_documents.get(actual["source_document_digest"])
        if expected is None:
            raise ValueError("evidence document absent from sealed partition receipt")
        expected_sources = {
            (episode["episode_id"], source["tracklet_id"]): (
                source["partition_digest"],
                source["full_training"],
                source["full_heldout"],
            )
    ...
    if (
        ...
        or observation_count != expected_observations
        or len(tracks) != refinement["track_count"]
        or observation_count != refinement["observation_count"]
    ):
        raise ValueError("sealed cohort track/observation accounting mismatch")
    return StateCache(
        region,
        config,
        tuple(tracks),
        {
            ...
            "sealed_selected": refinement["selected"],
            "sealed_training_score": refinement["training_score"],
            "sealed_heldout_score": refinement["heldout_score"],
            "partition_receipt_digest": refinement["partition_receipt_digest"],
        },
    )
...
def run(args) -> None:
    ...
    if guard_started - started > args.budget_seconds:
        raise TimeoutError("state-cache construction exhausted the run budget")
    if resource.getrusage(resource.RUSAGE_SELF).ru_maxrss > 1_800_000:
        raise MemoryError("state-cache construction exceeds 1.8GB")
    sealed = cache.provenance["sealed_selected"]
    uniform_parity = evaluate_position(
        cache,
        [sealed["east_km"], sealed["north_km"]],
        CircularArm(),
    )
    parity_errors = {
        "training": abs(
    ...
    if args.all_arms:
        ...
        arms.append(CircularArm(args.sigma_hz, args.outlier_probability, args.grid_size))
    ...
    result = {
        ...
        "complete": all(
            fit["converged"] and not fit["bound_hit"]
        ...
        "configuration": {
            ...
            "seed_source": "fits from the supplied sealed cohort refinement",
    ...
    failure_reasons = [
        f"{output['arm']} fit {index} did not converge"
        for output in outputs
        for index, fit in enumerate(output["fits"])
        if not fit["converged"]
    ] + [
        f"{output['arm']} fit {index} hit a local bound"
        for output in outputs
        for index, fit in enumerate(output["fits"])
        if fit["bound_hit"]
    ]
    result["qualification"] = "diagnostic" if result["complete"] else "insufficient"
    result["qualification_failures"] = failure_reasons
