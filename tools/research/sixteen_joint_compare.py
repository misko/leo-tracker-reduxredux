        ...
        rows.append(
            {
                ...
                "state_nodes": len(time_grid),
                "snapshot_digest": digest,
            }
        )
        print(
            f"cached {scan_index + 1}/16 {sid}: {len(evidence)} tracks, "
            f"{len(candidates)} candidates",
            flush=True,
        )
    manifest = {
        "schema": "frozen16-conditional-state-cache/v1",
...
def prediction_for_track(
    evidence, arrays, track, latitude_deg, longitude_deg, taus_s=TAUS
    ...
    return AdaptiveTrackPrediction(
        track["track_id"],
        tuple(track["observation_ids"]),
        times,
        np.asarray(track["measured_hz"]),
        np.asarray(track["training_mask"], dtype=bool),
        arrays["candidate_id"].astype(str),
        np.asarray(taus_s),
        prediction,
        visible,
    )
...
def score_location(cache: Path, sessions, latitude_deg: float, longitude_deg: float):
    ...
    for sid in sessions:
        ...
        predictions.extend(
            prediction_for_track(evidence, arrays, track, latitude_deg, longitude_deg)
...
def haversine_km(a, b):
    p1, p2 = np.deg2rad([a[0], b[0]])
    dl = np.deg2rad(b[1] - a[1])
...
def compare(cache: Path, scans_path: Path, output: Path, budget_s: float):
    ...
    sessions = [x["session_id"] for x in scans]
    seeds = []
    for scan in scans:
        for prior in ("sacramento", "reno"):
            for key in ("selected", "finest"):
                p = scan["priors"][prior][key]
                point = (p["latitude_deg"], p["longitude_deg"])
                if all(haversine_km(point, old) >= 20 for old in seeds):
                    seeds.append(point)
    ...
    for count in (16, 1, 2, 4, 8):
        ...
        retained = []
        for row in scored:
            if all(haversine_km(row[1], x[1]) >= 20 for x in retained):
                retained.append(row)
            if len(retained) == 4:
                break
        refined = []
        for _, seed, incumbent in retained:
            if time.monotonic() - started > budget_s:
                break
            fit = minimize(
                lambda x, count=count: objective(sessions[:count], tuple(x)),
                seed,
                method="Nelder-Mead",
                options={
                    "maxfev": 35,
                    ...
                    "initial_simplex": np.array(
                        [seed, (seed[0] + 0.02, seed[1]), (seed[0], seed[1] + 0.02)]
                    ),
                },
            )
            ...
            refined.append(
                {
                    "latitude_deg": best[1][0],
                    "longitude_deg": best[1][1],
                    "rmse_hz": best[0],
                    "seed": seed,
        ...
        results.append(
            {
                "scan_count": count,
                "track_count": sum(
                    x["priors"]["sacramento"]["accounting"]["eligible_track_count"] for x in active
                ),
                "observation_count": sum(
                    x["priors"]["sacramento"]["accounting"]["eligible_observation_count"]
                    for x in active
                ),
                "best": refined[0],
                "basins": refined,
    ...
    for row in results:
        row["best"]["error_km"] = haversine_km(
            (row["best"]["latitude_deg"], row["best"]["longitude_deg"]), truth
        )
    ordered = sorted(results, key=lambda x: x["scan_count"])
    result = {
        "schema": "frozen16-joint-comparison/v1",
        "position_truth_used_for_inference": False,
        ...
        "prior_support": "intersection: Sacramento 250 km and Reno 500 km",
        "optimizer_budget": (
            "Nelder-Mead, at most 35 evaluations per retained basin; incumbents preserved"
        ),
        "results": ordered,
    ...
    ax[0].set(xlabel="Scans accumulated", ylabel="Selection RMS (Hz)")
    ax[1].set(xlabel="Scans accumulated", ylabel="Evaluation-only error (km)")
    for a in ax:
        a.grid(alpha=0.25)
        a.set_xscale("log", base=2)
        a.set_xticks([1, 2, 4, 8, 16])
        a.get_xaxis().set_major_formatter("{x:g}")
...
def main():
    ...
    if args.build_cache:
        print(json.dumps(build_cache(args.selection, cache), indent=2))
    if args.cache_only:
        return
