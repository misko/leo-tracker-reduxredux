...
class _LazyCoarseElevation:
    def __init__(self, position: np.ndarray, site: Grid):
        self.position = position
        self.site = site
        self.values = np.empty(position.shape[:2], dtype=float)
        self.filled = np.zeros(position.shape[1], dtype=bool)

    def get(self, columns: np.ndarray) -> np.ndarray:
        missing = columns[~self.filled[columns]]
        if len(missing):
            missing = np.unique(missing)
            delta = self.position[:, missing] - self.site.ecef_km[0]
            distance = np.linalg.norm(delta, axis=-1)
            sine = np.sum(delta * self.site.up[0], axis=-1) / distance
            self.values[:, missing] = np.rad2deg(np.arcsin(np.clip(sine, -1.0, 1.0)))
            self.filled[missing] = True
...
class CoverageEvaluator:
    ...
    def _coarse_visible(
        ...
        if isinstance(shared_elevation, _LazyCoarseElevation):
            elevation = shared_elevation.get(endpoints)
            endpoint_lookup = np.full(len(track.coarse_node_indices), -1, dtype=int)
            endpoint_lookup[endpoints] = np.arange(len(endpoints))
            low_local = endpoint_lookup[low_local]
            high_local = endpoint_lookup[high_local]
            endpoints = np.arange(len(endpoints))
        else:
            elevation = (
    ...
    def evaluate_residual_points(self, sites: Grid) -> list[dict]:
        """Expose uncapped per-track residuals for alternative joint objectives.

        Ordinary point evaluation retains these details, so subsequent calls
        reuse them. A coverage-only top-K cache entry is explicitly recomputed
        rather than inventing track residuals from thresholded coverage.
        """
...
def refine_from_centres(
    evaluator: Callable[[np.ndarray], Sequence[CellScore]],
    initial_centres: np.ndarray,
    *,
    initial_spacing_km: float,
    radius_km: float,
    region_size_km: float = 5000.0,
    levels_km: Sequence[float] = (25.0, 12.5),
    basins: int | Sequence[int] = 32,
    threshold_hz: float = 200.0,
) -> tuple[list[CellScore], list[dict]]:
    """Refine truth-free exact-grid seeds onto finer nested regional lattices."""
    centres = np.asarray(initial_centres, dtype=float)
    ...
    for level, level_basins in zip(levels_km, schedule, strict=True):
        ...
        proposed = {
            (float(axis[i]), float(axis[j]))
            for east, north in centres
            for i in np.flatnonzero(np.abs(axis - east) <= previous / 2 + 1e-12)
            for j in np.flatnonzero(np.abs(axis - north) <= previous / 2 + 1e-12)
            if np.hypot(axis[i], axis[j]) <= radius_km + level / np.sqrt(2.0) + 1e-12
        }
