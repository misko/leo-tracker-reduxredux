...
GUARD_MS = 0.25
CASES = (
    ("scan-fw-9b88653c7a012fc2", 760, Feature103VisitAnalysisV3),
...
def circular_affine(
    time_s: np.ndarray, phase: np.ndarray, train: np.ndarray
) -> tuple[float, float]:
    ...
    intercept = float(
        np.angle(np.mean(np.exp(1j * (phase[train] - 2 * np.pi * hz * time_s[train]))))
    )
    return intercept, hz
...
def _design(
    ...
    for k in range(low, high + 1):
        ...
        out[use] = (
            states[sym[use] - 2]
            * np.exp(2j * np.pi * freqs[None, :] * offset[:, None])
            * np.exp(2j * np.pi * cfo * times[use, None] / fs)
            / np.sqrt(8)
        )
    return out
...
def coefficients(
    ...
    if np.count_nonzero(good) < 80:
        return np.full(8, np.nan + 1j * np.nan), 0.0, np.count_nonzero(good)
    a, *_ = np.linalg.lstsq(x[good], values[good], rcond=None)
    coh = float(
        np.linalg.norm(x[good] @ a) / max(np.linalg.norm(values[good]), np.finfo(float).tiny)
    )
...
def run() -> None:
    ...
    rows, binding = (
        [],
        {
            ...
            "protocol": hashlib.sha256(
                (ROOT / "reports/2026_09_23_matched_bandwidth_phase_protocol.md").read_bytes()
            ).hexdigest(),
            "source_sha256": {
                str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
                for p in (Path(__file__), ROOT / "src/leo/analysis/starlink/templates.py")
            },
            "target_rate_hz": TARGET_RATE,
            "pilot_relative_span_hz": 1_640_625.0,
            "guard_ms": GUARD_MS,
        },
    )
    ...
    try:
        for session, visit_index, model in CASES:
            ...
            with AdaptiveHopAnalysisInputStore(store).source(session) as source:
                ordinal = next(
                    i for i, v in enumerate(source.visits) if v.event.visit_index == visit_index
                )
                ...
                for gid in range(6):
                    ...
                    centered = raw * np.exp(
                        ...
                        * np.pi
                        * raw_times[:, None]
                        * np.array([c[1] for c in candidates])[None, :]
                        / fs
                    )
                    arms = {"full": (centered, raw_times)}
                    narrow = resample_poly(
                        centered,
                        up=1,
                        down=factor,
                        axis=0,
                    ...
                    arms["narrow_2p5"] = (
                        narrow,
                        start + np.arange(len(narrow), dtype=float) * factor,
                    )
                    item = {
                        "session_id": session,
                        "visit": visit_index,
                        "sample_rate_hz": fs,
                        "group_id": gid,
                        "partition": "train" if gid in train else "held",
                        "time_s": (gid + 0.5) * GROUP_MS / 1000,
                        "arms": {},
                    }
                    for name, (values, times) in arms.items():
                        keep = (times >= start + guard) & (times < stop - guard)
                        values, times = values[keep], times[keep]
                        a0, c0, n0 = coefficients(
                            values[:, 0], times, fs, candidates[0][0], 0.0, edge
                        )
                        a1, c1, n1 = coefficients(
                            values[:, 1], times, fs, candidates[1][0], 0.0, edge
                        )
                        q0, ctrl0, _ = coefficients(
                            values[:, 0], times, fs, candidates[0][0], 0.0, edge, roll=17
                        )
                        q1, ctrl1, _ = coefficients(
                            values[:, 1], times, fs, candidates[1][0], 0.0, edge, roll=17
                        )
                        cross = np.vdot(a0, a1)
                        ...
                        item["arms"][name] = {
                            "pilot_phase_rad": float(np.angle(cross)),
                            "pilot_vector_coherence": float(
                                abs(cross) / (np.linalg.norm(a0) * np.linalg.norm(a1))
                            ),
                            "exact_projection_rx0": c0,
                            "exact_projection_rx1": c1,
                            "rolled_projection_rx0": ctrl0,
                            "rolled_projection_rx1": ctrl1,
                            "usable_samples": min(n0, n1),
                            "broadband_cross_coherence": float(
                                abs(b) / max(denom, np.finfo(float).tiny)
                            ),
                            "broadband_cross_phase_rad": float(np.angle(b)),
                        }
                    rows.append(item)
    finally:
        store.close()
    ...
    for ax, (session, visit, _model) in zip(axes, CASES, strict=True):
        ...
        summary = {
            "session_id": session,
            "visit": visit,
            "groups": {"train": split_groups(6)[0], "held": split_groups(6)[1]},
            "arms": {},
        }
        for arm, color in (("full", "tab:blue"), ("narrow_2p5", "tab:orange")):
            t = np.array([r["time_s"] for r in case])
            p = np.array([r["arms"][arm]["pilot_phase_rad"] for r in case])
            train = np.array([r["partition"] == "train" for r in case])
            held = ~train
            intercept, hz = circular_affine(t, p, train)
            err = wrapped_phase_error(p, intercept + 2 * np.pi * hz * t)
            summary["arms"][arm] = {
                "train_phase_resultant": float(abs(np.mean(np.exp(1j * err[train])))),
                "held_phase_resultant": float(abs(np.mean(np.exp(1j * err[held])))),
                "held_phase_rms_rad": float(np.sqrt(np.mean(err[held] ** 2))),
                "training_only_slope_hz": hz,
                "median_exact_projection_rx0": float(
                    np.median([r["arms"][arm]["exact_projection_rx0"] for r in case])
                ),
                "median_exact_projection_rx1": float(
                    np.median([r["arms"][arm]["exact_projection_rx1"] for r in case])
                ),
                "median_broadband_cross_coherence": float(
                    np.median([r["arms"][arm]["broadband_cross_coherence"] for r in case])
                ),
            }
    ...
    (OUT / "results.json").write_text(
        json.dumps({"binding": binding, "rows": rows, "summaries": summaries}, indent=2) + "\n"
    )
