...
def build_report(session_id: str, output: Path) -> dict:
    sources = ScannerTrackingInputStore(Path("/srv/bulk/leo"))
    try:
    ...
    for hypothesis in trajectory.hypotheses:
        for tracklet_id in hypothesis.tracklet_ids:
            ...
            split = min(max(4, int(math.floor(len(t) * 0.6))), len(t) - 1)
            split_seed = canonical_digest({"session_id": session_id, "tracklet_id": tracklet_id})
            ranks = [
                canonical_digest({"seed": split_seed, "observation_id": row.observation_id})
                for row in rows
            ]
            cutoff = sorted(ranks)[split - 1]
            training_mask = np.asarray([rank <= cutoff for rank in ranks], dtype=bool)
...
def main() -> None:
    ...
    result = build_report(args.session_id, args.output)
    print(
        json.dumps(
            {
