...
def verdict(result: object, support_start: int) -> str:
    ...
    for c in evidence.candidates[:evidence.candidate_count]:
        ...
        if not c.fractional_complete or not supported:
            outcome = "unknown"
        if supported and c.fractional_complete and c.exact_score >= .175 and c.margin >= .025:
            return "detected"
    return outcome
...
def holdout(output: Path, bulk_root: Path) -> None:
                ...
                for index, visit in enumerate(visits):
                    ...
                    with NativeDwell(library, 2500000, edge, 512) as ref:
                        ...
                        assert (result.epoch, result.exact_score, result.margin) == (
                            ec.epoch, ec.exact_score, ec.margin,
                        )
                        high_result = ref.run(high, maximum=1, seeded=False)
                        high_verdict = verdict(high_result, 64)
                    row = {"session_id": sid, "input_manifest_sha256": session.manifest_sha256,
                           "rx": rx, "sweep": sweep, "visit": sweep*8+index,
                           "target": visit.target_index, "edge": edge,
                           "iq_sha256": hashlib.sha256(iq.tobytes()).hexdigest(),
                           "result": asdict(result), "same_coefficient_outcome": same,
    ...
    summary = {"cases": len(rows), "same_coefficient_mismatches": 0,
               "mean_wall_ms": float(np.mean(times)),
               "p99_wall_ms": float(np.percentile(times, 99)),
               "by_rx": {}, "rows_sha256": digest(output / "holdout.jsonl")}
    for rx in (0, 1):
        ...
        summary["by_rx"][str(rx)] = {
            ...
            "different_outcomes": [r["visit"] for r in subset
                                   if r["result"]["outcome"] != r["high_quality_outcome"]],
        }
