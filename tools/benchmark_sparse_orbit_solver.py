#!/usr/bin/env python3
"""Run the stabilized solver on sealed sparse density subsets, then score them."""
...
def run_job(job):
    ...
    try:
        ...
        fits = []
        config = SparseOrbitConfig()
        for start in STARTS:
            try:
                fit = fit_sparse_orbit(data, region, start, config, fitting_mask=selected)
                fits.append(fit)
...
def main():
    ...
    payload = {
        "schema": "sparse-orbit-stabilization-benchmark-v1",
        "plan": str(args.plan),
        "plan_sha256": _hash(args.plan),
        "solver_sha256": _hash(
            Path(__file__).parents[1] / "src/leo/analysis/research/sparse_orbit_solver.py"
        ),
