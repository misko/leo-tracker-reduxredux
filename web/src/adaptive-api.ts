...
export type AdaptiveCapture = LegacyAdaptiveCapture | HostAdaptiveCapture | EdgeAdaptiveCaptureV4 | DualRx10mCaptureV5 | Feature103CaptureV6;
...
function validateCapture(c: AdaptiveCapture): void {
  if (!c || c.kind !== "adaptive_hop_history_item" || ![1, 2, 3, 4, 5, 6].includes(c.schema_version)
  ...
  if (c.schema_version === 4 || c.schema_version === 5 || c.schema_version === 6) {
...
export async function getAdaptiveSessions(cursor: number, signal?: AbortSignal): Promise<AdaptivePage | null> {
  ...
  if (!page || ![1, 2, 3, 4, 5].includes(page.schema_version) || page.kind !== "adaptive_hop_history_page"
...
export async function getAdaptiveSession(sessionId: string, signal?: AbortSignal): Promise<AdaptiveDetail> {
  ...
  if (!detail || ![1, 2, 3, 4, 5, 6].includes(detail.schema_version) || detail.kind !== "adaptive_hop_session_detail") {
  ...
  detail.visits.forEach((v, i) => {
    if (!v || v.visit_index !== i || !integer(v.target_index, 7) || !integer(v.proposed_target_index, 7)
        ...
        || ((c.schema_version === 4 || c.schema_version === 5 || c.schema_version === 6) && ((c.allowed_target_mask & (1 << v.target_index)) === 0
