...
const document = {
  ...
  priors: [{ name: "sacramento", search_complete: false,
...
it("publishes a digest-bound PNG and machine-readable result with selection limitations", async () => {
  ...
  expect(image).toHaveAttribute("src", "/api/v1/scanner/tracking/scan-test/adaptive-tle-position/map.png?sha256=sha256%3Apng");
  expect(screen.getByRole("link", { name: "Download adaptive position JSON" })).toHaveAttribute("href", "/api/v1/scanner/tracking/scan-test/adaptive-tle-position");
