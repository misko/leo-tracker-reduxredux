...
A discrete fixed-interval smoother now marginalizes the final retained history
posterior back onto every earlier dwell. It may revise past identity
probabilities using later response, but it never rewrites forward receipts,
re-scores response, or refits receiver nuisance; ties and beam pruning remain
abstentions.
A digest-closed multi-dwell posterior now persists the positive retained
...
handoff histories into a solver-safe calibration product is now implemented.
The additive sequential-history product preserves the complete assignment
history and its posterior mass, exports only gauge-resolved satellite
frequency states, and deliberately refuses to infer which satellite will be
active in a later target interval. It is therefore calibration-transferable
but never navigation-eligible without a separate future-activity association.
...
episode per dwell, fixed `tau=0`, and the same complete candidate universe in
every dwell; truncation, missing visibility, simultaneous emitters, or changed
site/RF/model authority fail closed.
...
marginalizes the receiver columns. An optional time-local version uses one
...
| Sequential-history known-position frequency calibration | DONE, conditional synthetic first bridge | [`multi_dwell_joint_frequency_calibration.py`](../src/leo/analysis/multi_dwell_joint_frequency_calibration.py) and [`test_multi_dwell_joint_frequency_calibration.py`](../tests/analysis/test_multi_dwell_joint_frequency_calibration.py) validate the persisted history posterior against the exact graph/bank adapter lineage, merge only a common authenticated TLE member inventory, and reuse the Gaussian batch kernel per positive retained history. Satellite bias/drift means and joint covariance remain keyed to the original history-mode digest; null histories remain explicit, receiver-local offsets/drift stay opaque, and an unresolved frequency gauge makes every mode ineligible. The calibration result itself sets `association_result_relabelled=false` and `transferable_correction_emitted=false`; the separate projection below performs the solver-safe transfer. |
| Sequential-history satellite calibration product | DONE, solver-safe synthetic projection | [`satellite_pnt_multi_dwell_correction.py`](../src/leo/contracts/satellite_pnt_multi_dwell_correction.py), [`multi_dwell_satellite_correction.py`](../src/leo/analysis/multi_dwell_satellite_correction.py), and [`test_multi_dwell_satellite_correction.py`](../tests/analysis/test_multi_dwell_satellite_correction.py) preserve every sequential assignment history and posterior probability, bind its exact calibration and authenticated TLE inventory, retain cross-satellite bias/drift covariance, and export no receiver-local state or calibration coordinates. An opaque site commitment joins the access-controlled receipt, the TLE is causal and at most 24 hours old, and validity is fixed to 30 seconds. Gauge/evidence-qualified frequency states may be transferred for later modeling, but future activity is unresolved and every mode/product is intentionally `navigation_eligible=false`; the history is never reinterpreted as simultaneous activity. |
...
| Structural radio-only position control | DONE, truth-isolated negative-control boundary | [`blinded_radio_only_position.py`](../src/leo/analysis/blinded_radio_only_position.py) and [`test_blinded_radio_only_position.py`](../tests/analysis/test_blinded_radio_only_position.py) validate the exact radio-only challenge and seal `NO_RESULT` with all position mass unresolved. The control does not invent a prior-shaped position mode: receiver-relative CFO without orbit, direction, range, or range-rate geometry contains no map to geographic position. This closes the fourth positioning lane's structural boundary. The separate equal-row predictive comparison below is diagnostic and is not yet a calibrated navigation or identity decision. |
...
| Integrated portable qualification | PASS with explicit external-capability exclusions | `3199` tests pass and `192` are marker-deselected on the portable shard after excluding tests marked PostgreSQL, hardware, real-corpus, or legacy-oracle; the hardware qualification file whose unmarked unit cases import the unavailable optional `pluto_plus` package; and one pre-existing frozen-manifest AST digest mismatch. Repository-wide Ruff check/format, `uv lock --check`, focused strict MyPy, Markdown-link closure, and diff-check pass. The unconstrained run also confirms that PostgreSQL tests fail closed before connecting when `LEO_TEST_DATABASE_URL` is absent. No PostgreSQL, hardware, QNAP, or RF claim is made by this qualification. |
...
| Known-truth cross-family predictive audit | DONE, synthetic kernel plus deterministic opened-background diagnostic; no threshold | [`cross_family_predictive_calibration.py`](../src/leo/analysis/research/cross_family_predictive_calibration.py) defines scenario-equal paired-arm calibration. [`cross_family_qin_predictive_scoring.py`](../src/leo/analysis/research/cross_family_qin_predictive_scoring.py) applies it to sealed Qin evidence: a true causal-TLE `tau=0` curve plus one training offset is compared with training-only degree-1/2/3 radio polynomials on identical future rows, with training-fit covariance propagated. Attempt 1 is retained as [NO-GO determinism evidence](../reports/2026_08_27_satellite_pnt_cross_family_predictive_scoring_attempt1_determinism_audit.md); dense BLAS reductions changed last-bit digests by thread count. The corrected diagonal-plus-low-rank [attempt-2 result](../reports/2026_08_27_satellite_pnt_cross_family_predictive_scoring_attempt2.md) has one digest at 1/2/8 threads and remains correct on only 3/6 truth arms (50%); three pairs are below the 19-pair finite-rank floor. This is evidence of insufficient current discrimination, not a threshold, odds, gate, or identity verdict. |
| Paired orbit/radio known-truth inventory | DONE, frozen protocol and corrective execution complete | [`satellite-pnt-cross-family-injection-protocol-v1.json`](../config/analysis/satellite-pnt-cross-family-injection-protocol-v1.json) freezes the exact three authorized hard-null backgrounds, one causal response-free TLE and highest-center-elevation Starlink per background, paired orbit and center-matched radio-linear truth arms, identical 60/40 parity-isolated scoring, and the correct independent count of three backgrounds rather than six arms. Attempt 1 is preserved as [NO-GO digest evidence](../reports/2026_08_27_satellite_pnt_cross_family_injection_attempt1_digest_audit.md): post-digest float quantization prevented all six arm digests from reproducing. The scope-frozen corrective attempt 2 retains 9,000 opportunities and now closes all six arm plus three pair digests in [`test_cross_family_qin_injection_results.py`](../tests/analysis/test_cross_family_qin_injection_results.py). No threshold, odds, identity, correction, or positioning claim is made. |
| Response-free trajectory truth and Qin evidence kernels | DONE, executed on three opened hard-null backgrounds | [`trajectory_qin_injection.py`](../src/leo/analysis/research/trajectory_qin_injection.py) analytically integrates any frozen piecewise-linear CFO trajectory into exact Qin and reuses the public even/odd likelihood. [`cross_family_orbit_truth.py`](../src/leo/analysis/research/cross_family_orbit_truth.py) authenticates TLE bytes, reproduces the geometry-only winner, and bounds 1 ms orbit interpolation error before deriving the paired radio-linear truth. [`cross_family_injection_experiment.py`](../src/leo/analysis/research/cross_family_injection_experiment.py) retains every opportunity/no-result and exposes even-Qin training versus odd-Qin future rows. The [attempt-2 result](../reports/2026_08_27_satellite_pnt_cross_family_injection_attempt2_results.md) has 894–897 usable training rows and 597–600 future rows per arm; future truth-residual RMS is 54.96/57.19/56.75 Hz for the orbit arms and 57.32/54.26/53.52 Hz for the paired radio arms. These are front-end measurement diagnostics, not cross-family predictive discrimination. |
...
2, plus a conditional native-joint lane-3 first slice. A separate oracle lane
scores a response-free, precommitted geodetic particle bank under a bounded
continental prior. The first bank is only a conditional numerical fixture and
does not claim that its particles cover the prior. These slices keep discrete
...
- **2026-08-27:** a paired known-truth cross-family audit now applies those
  independently learned covariance scales to catalogue-orbit and
  radio-polynomial scores on the same frozen future rows. Cases are averaged
  within each truth arm of an independent paired-background scenario before
  sign accuracy is summarized. Both truth arms are mandatory in every
  background, so three reused backgrounds remain three—not six—independent
  units. This is calibration
  infrastructure only: it learns no threshold, emits no odds or identity gate,
  and has not been run on background IQ.
- **2026-08-27:** the paired cross-family development inventory is now frozen
  before any additional IQ access. The exact three authorized hard-null
  backgrounds each contribute one orbit-truth arm and one center-matched
  radio-linear arm, but remain only three independent units. Exact causal TLE
  bytes, geometry-only highest-elevation Starlinks, sample/UTC spans, seeds,
  parity-isolated 60/40 scoring, and no-claim boundaries are digest-bound. A
  pure trajectory-driven Qin kernel, TLE truth verifier, and paired evidence
  builder are qualified; the three response-free TLE curves reproduce with
  maximum 1 ms interpolation error below `0.000045 Hz`. The protocol grants no
  execution authority, so background IQ remains unopened pending a separate
  hash-bound execution amendment and runner qualification.
- **2026-08-27:** the hash-bound paired Qin execution ran on the three exact
  authorized hard-null spans without new RF collection. Attempt 1 completed but
  failed post-run semantic closure because its writer quantized floats after
  computing six arm digests; its files and exact NO-GO audit remain immutable.
  A separately frozen correction removed only that post-digest quantization and
  authorized one retry to new paths. Attempt 2 retains all `9,000` opportunities,
  uses even Qin only for training and odd Qin only for future scoring, reproduces
  all six arm and three pair digests from persisted bytes, and is numerically
  identical to attempt 1 at the original persisted precision. It still fits no
  threshold and produces no posterior odds, identity, correction, or position.
- **2026-08-27:** the frozen equal-row cross-family scorer was then run without
  reopening IQ. It fits the true causal-TLE `tau=0` curve plus one CFO offset
  and degree-1/2/3 radio polynomials on even-Qin training rows, propagates each
  training-fit covariance, and scores the identical usable odd-Qin future rows.
  Catalogue and radio uniform variance scales are learned separately with the
  scored background pair excluded (`1.616` and `1.629` on the full descriptive
  inventory). The primary line comparison identifies only `3/6` truth arms
  correctly (50%): the present two-second, `-16 dB` design does not have enough
  cross-family power for a threshold or catalogue-identity gate. The result
  remains a three-pair mechanistic diagnostic, below the frozen 19-pair floor.
- **2026-08-27:** the first persisted predictive score was subsequently marked
  NO-GO because dense BLAS reductions made last-bit result digests depend on
  thread count. The attempt-1 report, JSON, and exact digest variants remain
  immutable. A scope-frozen correction replaced only that dense calculation
  with its deterministic diagonal-plus-low-rank/Woodbury equivalent. Attempt 2
  produces one digest at 1, 2, and 8 OpenBLAS threads; it differs from attempt 1
  by at most `7.2e-15 Hz` in RMS and `3.1e-11` in leave-one-out NLL separation,
  with the same 3/6 scientific conclusion and no additional IQ access.
- **2026-08-27:** integrated portable qualification passes `3199` tests with
  `192` marker-deselected, alongside
  repository-wide Ruff formatting/checking, lock closure, focused strict MyPy,
  plan-link closure, and diff-check clean. External-capability tests remain
  explicitly outside this receipt: PostgreSQL refuses to connect without a
  safe test URL, the optional Pluto dependency and protected corpus are absent,
  and one unrelated frozen-manifest AST baseline already differs on main.
  None of those exclusions is converted into a satellite/PNT success claim.
...
- **2026-08-27:** the first conditional known-position joint-frequency batch
  ...
  unidentifiable gauge. The default uses one linear drift per hardware epoch;
  an optional dwell-local random walk now relaxes that equality across time and
  demonstrably reduces forced cross-dwell coupling as its process noise grows.
  The batch still does not revise the association probabilities.
...
- **2026-08-27:** a first broad-prior navigation kernel now scores an exact
  response-free geodetic particle bank frozen before target response. It uses
  the oracle frozen-identity lane, the same correlated Doppler evidence at
  every particle, and analytically marginalizes a proper common receiver-CFO
  prior. Its estimate is always partial and truth-free; the existing reveal
  boundary computes position error only after sealing. Numerically underflowed
...
- **2026-08-27:** the synthetic multi-dwell filter now has a narrow
  contract-level input bridge. It consumes one exact TLE-blind physical graph
  and one authenticated response-free SGP4 bank per chronological dwell,
  verifies the support join before copying response, and never ranks candidates
...
- **2026-08-27:** a known-position calibration bridge now consumes the
  ...
  active satellites.
- **2026-08-27:** an additive sequential-history correction contract now
  performs that distinction without relabelling the posterior. It publishes
  only gauge-resolved satellite frequency states and their covariance behind
  an opaque known-site commitment, preserves null and handoff histories, and
  excludes all receiver-local states. Because a historical assignment does
  not say which transmitter will be active later, the product is always
  `PARTIAL` and never navigation-eligible; target-time activity association
