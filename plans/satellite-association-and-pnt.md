...
| Known-truth cross-family predictive audit | DONE, synthetic calibration kernel; no threshold | [`cross_family_predictive_calibration.py`](../src/leo/analysis/research/cross_family_predictive_calibration.py) applies independently learned, evidence-disjoint covariance scales to paired catalogue-orbit and radio-polynomial scores on identical future rows. Each independent background is one scenario containing both truth arms; cases are aggregated within each arm before accuracy is summarized, so reusing one background cannot be miscounted as two independent experiments and duplicated fragments cannot manufacture sample size. Six tests cover paired-arm completeness, background-equal weighting, truth-authority consistency, calibration-target leakage, and nested score tampering. It fits no threshold, odds, gate, or identity verdict. |
