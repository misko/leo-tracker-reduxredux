...
| Paired orbit/radio known-truth inventory | DONE, frozen development protocol; not executed | [`satellite-pnt-cross-family-injection-protocol-v1.json`](../config/analysis/satellite-pnt-cross-family-injection-protocol-v1.json) freezes the exact three authorized hard-null backgrounds, one causal response-free TLE and highest-center-elevation Starlink per background, paired orbit and center-matched radio-linear truth arms, identical 60/40 parity-isolated scoring, and the correct independent count of three backgrounds rather than six arms. [`cross_family_injection_protocol.py`](../src/leo/analysis/research/cross_family_injection_protocol.py) closes hashes, sample/UTC coordinates, causality, selection semantics, and claim boundaries. The protocol explicitly authorizes no execution. |
| Response-free trajectory truth and Qin evidence kernels | DONE, synthetic and metadata/TLE qualified; no IQ execution | [`trajectory_qin_injection.py`](../src/leo/analysis/research/trajectory_qin_injection.py) analytically integrates any frozen piecewise-linear CFO trajectory into exact Qin and reuses the public even/odd likelihood. [`cross_family_orbit_truth.py`](../src/leo/analysis/research/cross_family_orbit_truth.py) authenticates TLE bytes, reproduces the geometry-only winner, and bounds 1 ms orbit interpolation error before deriving the paired radio-linear truth. [`cross_family_injection_experiment.py`](../src/leo/analysis/research/cross_family_injection_experiment.py) retains every opportunity/no-result and exposes even-Qin training versus odd-Qin future rows. The three frozen TLE truth curves reproduce with maximum interpolation errors below 0.000045 Hz; no background IQ has been opened by this slice. |
