...
| Known-truth cross-family predictive audit | DONE, synthetic kernel plus first opened-background diagnostic; no threshold | [`cross_family_predictive_calibration.py`](../src/leo/analysis/research/cross_family_predictive_calibration.py) defines scenario-equal paired-arm calibration. [`cross_family_qin_predictive_scoring.py`](../src/leo/analysis/research/cross_family_qin_predictive_scoring.py) now applies the design to the sealed Qin evidence: a true causal-TLE `tau=0` curve plus one training offset is compared with training-only degree-1/2/3 radio polynomials on identical future rows, with training-fit covariance propagated. Separate catalogue/radio variance scales are learned while holding out the scored background pair. The [result](../reports/2026_08_27_satellite_pnt_cross_family_predictive_scoring.md) is correct on only 3/6 truth arms (50%); three pairs are below the 19-pair finite-rank floor. This is evidence of insufficient current discrimination, not a threshold, odds, gate, or identity verdict. |
